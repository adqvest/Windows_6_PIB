...
def jobs_entry(all_files, scheduler):
    ...
    files_df.to_sql("LOG_TABLE_STORE_CRAWLER", con=engine, if_exists='append', index=False)
...
def job_start_log_by_bot(table_name, py_file_name, job_start_time):
    ...
    #print(py_file_name)
    print('select max(Schedular_Start_Time) as Time from LOG_TABLE_STORE_CRAWLER where Python_File_Name = "' + py_file_name + '"')
    max_sc_time = pd.read_sql('select max(Schedular_Start_Time) as Time from LOG_TABLE_STORE_CRAWLER where Python_File_Name = "' + py_file_name + '"', con=engine)['Time'][0].strftime("%Y-%m-%d %H:%M:%S")
    query      = 'Update LOG_TABLE_STORE_CRAWLER set Start_Time = "' + job_start_time.strftime("%Y-%m-%d %H:%M:%S") + '", Table_Name = "' + table_name + '", Relevant_Date="' + relevant_date + '" where Schedular_Start_Time = "' + max_sc_time + '" and Python_File_Name = "'+py_file_name+'"'
    #print(query)
...
def job_start_log(table_name, py_file_name,  job_start_time, scheduler = ''):
    ...
    log_Table.to_sql(name='LOG_TABLE_STORE_CRAWLER',con=engine,if_exists='append',index=False)
...
def job_end_log(table_name, job_start_time, no_of_ping):
    ...
    query      = 'Update LOG_TABLE_STORE_CRAWLER set No_Of_Ping=' + str(no_of_ping) + ', End_Time = ' + job_end_time + ', Execution_Time_Seconds = "' + str(job_execution_time) + '", Runtime = ' + job_end_time + ' where Start_Time = "' + job_start_time + '" and Table_Name = "'+table_name+'"'
    connection = engine.connect()
    connection.execute(query)
    connection.execute('commit')
    connection.close()
...
def job_error_log(table_name, job_start_time, error_type, error_msg, no_of_ping=-10,error_desc = ''):
    ...
    try:
        ...
        query = 'Update LOG_TABLE_STORE_CRAWLER set No_Of_Ping=' + str(no_of_ping) + ', Error_Type = "'+ error_type +'", Error_Msg = "'+ error_msg +'", Error_Desc = "'+ error_desc +'" where Start_Time = "' + job_start_time + '" and Table_Name = "'+table_name+'"'
