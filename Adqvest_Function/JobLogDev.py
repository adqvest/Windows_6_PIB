...
def job_start_log(table_name, py_file_name,  job_start_time, scheduler = ''):
    ...
    log_Table.to_sql(name='LOG_TABLE_NEW_DEV',con=engine,if_exists='append',index=False)
...
def job_end_log(table_name, job_start_time):
    ...
    query      = 'Update LOG_TABLE_NEW_DEV set End_Time = ' + job_end_time + ', Execution_Time_Seconds = "' + str(job_execution_time) + '", Runtime = ' + job_end_time + ' where Start_Time = "' + job_start_time + '" and Table_Name = "'+table_name+'"'
...
def job_error_log(table_name, job_start_time, error_type, error_msg, error_desc = ''):
    ...
    try:
        ...
        query = 'Update LOG_TABLE_NEW_DEV set Error_Type = "'+ error_type +'", Error_Msg = "'+ error_msg +'", Error_Desc = "'+ error_desc +'" where Start_Time = "' + job_start_time + '" and Table_Name = "'+table_name+'"'
        connection = engine.connect()
        connection.execute(query)
        connection.execute('commit')
        connection.close()
