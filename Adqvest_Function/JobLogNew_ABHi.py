...
def check_holidays(job_start_time):

  query = "Select * from AdqvestDB.NSE_BSE_MARKET_HOLIDAYS_YEARLY_DATA"
  holidays = pd.read_sql(query,con=engine)

  cond1 = (holidays['Relevant_Date']==job_start_time.date()).any()
  cond2 = (holidays['Relevant_Date']==(job_start_time.date()-datetime.timedelta(1))).any()

  if cond1==True:
      occasion = holidays[holidays['Relevant_Date']==job_start_time.date()]
      event = list(occasion['Holiday'].unique())[0]
      #print("Market Holiday On Account of ",event)
      reason = "Market Holiday On Account of "+event
      return [reason,"cond1"]

  elif cond2==True:
      occasion = holidays[holidays['Relevant_Date']==(job_start_time.date()-datetime.timedelta(1))]
      event = list(occasion['Holiday'].unique())[0]
      date1 = (job_start_time.date()-datetime.timedelta(1)).strftime("%Y-%m-%d")
      #print("Data Not Present for Holiday As on ",date1," On Account of ",event," Current Days Data Captured")
      reason = "Data Not Present for Holiday As on ",date1," On Account of ",event," Current Days Data Captured"
      return [reason,"cond2"]
  else:
    return []
...
def jobs_entry(all_files, scheduler):
    ...
    files_df.to_sql("LOG_TABLE_NEW_TEST", con=engine, if_exists='append', index=False)
...
def job_start_log_by_bot(table_name, py_file_name, job_start_time):
    ...
    #print(py_file_name)
    print('select max(Schedular_Start_Time) as Time from LOG_TABLE_NEW_TEST where Python_File_Name = "' + py_file_name + '"')
    max_sc_time = pd.read_sql('select max(Schedular_Start_Time) as Time from LOG_TABLE_NEW_TEST where Python_File_Name = "' + py_file_name + '"', con=engine)['Time'][0].strftime("%Y-%m-%d %H:%M:%S")
    query      = 'Update LOG_TABLE_NEW_TEST set Start_Time = "' + job_start_time.strftime("%Y-%m-%d %H:%M:%S") + '", Table_Name = "' + table_name + '", Relevant_Date="' + relevant_date + '" where Schedular_Start_Time = "' + max_sc_time + '" and Python_File_Name = "'+py_file_name+'"'
    #print(query)
...
def job_start_log(table_name, py_file_name,  job_start_time, scheduler = ''):
    ...
    log_Table.to_sql(name='LOG_TABLE_NEW_TEST',con=engine,if_exists='append',index=False)
...
def job_end_log(table_name, job_start_time, no_of_ping):
    ...
    query      = 'Update LOG_TABLE_NEW_TEST set No_Of_Ping=' + str(no_of_ping) + ', End_Time = ' + job_end_time + ', Execution_Time_Seconds = "' + str(job_execution_time) + '", Runtime = ' + job_end_time + ' where Start_Time = "' + job_start_time + '" and Table_Name = "'+table_name+'"'
    connection = engine.connect()
    connection.execute(query)
    connection.execute('commit')
    connection.close()
...
def job_error_log(table_name, job_start_time, error_type, error_msg, no_of_ping=-10,error_desc = ''):
    ...
    try:
        ...
        current_date = job_start_time
        job_start_time = job_start_time.strftime("%Y-%m-%d %H:%M:%S")
        #******* Cleaning error message/string *******
        error_type = cleaner.full_clean(error_type)
        error_msg = cleaner.full_clean(error_msg)

        if(("NSE" in table_name) | ("BSE" in table_name)):
            check = check_holidays(current_date)
            if ((check != [])):
                if(check[1]=="cond1"):
                    query = 'Update LOG_TABLE_NEW_TEST set No_Of_Ping=' + str(no_of_ping) + ', Error_Type = "'+ "Market Holiday" +'", Error_Msg = "'+ check[0] +'", Error_Desc = "'+ "Market Holiday" +'" where Start_Time = "' + job_start_time + '" and Table_Name = "'+table_name+'"'
                elif(check[1]=="cond2"):
                     query = ""
            else:
                 query = 'Update LOG_TABLE_NEW_TEST set No_Of_Ping=' + str(no_of_ping) + ', Error_Type = "'+ error_type +'", Error_Msg = "'+ error_msg +'", Error_Desc = "'+ error_desc +'" where Start_Time = "' + job_start_time + '" and Table_Name = "'+table_name+'"'
