...
class ThurroS3Upload:
    ...
    def get_links(self, table_name, limit=3):
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                elif table_name == 'BANK_BASEL_III_QUARTERLY_DATA_CORPUS':
                    query = text(f"""
            ...
            print(f"Error getting test links for: {table_name}")
            return []
    ...
    def multiple_tables_parallel(self, table_limits=None):
        ...
        if not table_limits:
            ...
            table_limits = {table: 100 for table in self.table_names}
        ...
        try:
            ...
            for table_name in self.table_names:
                ...
                links = self.get_links(table_name, limit)
            ...
            with ThreadPoolExecutor(max_workers=max_total_workers, thread_name_prefix="MultiTable") as executor:
                ...
                future_to_task = {
                    executor.submit(self.single_link_parallel, link_data, table_name): (link_data, table_name)
            ...
            print(f"Multi-table parallel processing error")
            traceback.print_exc()
    ...
    def single_link_parallel(self, link_data, table_name):
        ...
        try:
            ...
            local_path = self.download_parallel(url, filename, table_name, thread_name)
            if not local_path:
                return False
            
            k1, k2 = self.s3_folder_map[table_name]
            
            s3_key = self.s3_upload_parallel(local_path, filename, table_name, thread_name,k1,k2)
            
            if not s3_key:
                return False

            status = 'Done'
            success = self.db_update_parallel(link_id,filename,url, status, table_name, thread_name)
    ...
    def download_parallel(self, url, filename, table_name, thread_name):
        ...
        try:
            ...
            if table_name == 'BANK_BASEL_III_QUARTERLY_DATA_CORPUS':
    ...
    def s3_upload_parallel(self, local_path, filename, table_name, thread_name,k1,k2):
    ...
    def db_update_parallel(self, link_id,filename,url, status, table_name, thread_name):
        ...
        try:
            ...
            if table_name == 'CARE_RATINGS_DAILY_DATA_CORPUS':
                connection.execute(f"UPDATE {table_name} SET S3_Upload_Status='Done' WHERE File_ID = %s AND Generated_File_Name = %s",(link_id, filename))
                print(f"{table_name} Database updated: File_ID {link_id} = {status}")
                return True    

            if table_name == 'RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS':
                connection.execute(f"UPDATE {table_name} SET S3_Upload_Status='Done' WHERE File_ID = %s AND Generated_File_Name = %s",(link_id, filename))
                print(f"{table_name} Database updated: File_ID {link_id} = {status}")
                return True       

            if table_name == 'AMC_WISE_MF_FACTSHEETS_MONTHLY_DATA_CORPUS':
                connection.execute(f"UPDATE {table_name} SET S3_Upload_Status='Done' WHERE File_ID = %s AND Generated_File_Name = %s",(link_id, filename))
                print(f"{table_name} Database updated: File_ID {link_id} = {status}")
                return True            
...
def create_service():
    ...
    test_tables = [
        ...
        ('BANK_BASEL_III_QUARTERLY_DATA_CORPUS', 'BANK_BASEL_III_DISCLOSURES_CORPUS/', 'BANK_BASEL_III_DISCLOSURES_CORPUS_TO_BE_CHUNKED/'),
    ...
    service = ThurroS3Upload(test_tables)
    return service
...
def quick_parallel_test():
    ...
    try:
        ...
        if service.table_names:
            table_limits = {table: 250 for table in service.table_names}
            service.multiple_tables_parallel(table_limits)
