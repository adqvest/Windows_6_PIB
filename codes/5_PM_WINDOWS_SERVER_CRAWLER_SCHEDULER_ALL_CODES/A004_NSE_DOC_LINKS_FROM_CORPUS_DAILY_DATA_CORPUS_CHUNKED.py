...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA'
    ...
    try:
        ...
        folder_name = 'NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_TO_BE_CHUNKED'
        ...
        response = [ x for x in response if 'NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_TO_BE_CHUNKED/' in x]
        print(len(response))

        # response = response[1:800]
        print(len(response))
        # response = response[:2]
        # response = [x for x in response if re.search(r'Q1_FY26', x)]

        doc_ids = client.execute('''SELECT distinct Document_Id FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CORPUS_CHUNKED;''')
            ...
            try:
                try:
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA SET Chunking_Comments = 'File Not Available' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    elif 'Failed to load document (PDFium: Incorrect password error)' in str(e):
                        query = f"UPDATE NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA SET Chunking_Comments = 'Password Issue' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)  
                    elif 'Failed to load page' in str(e):
                        query = f"UPDATE NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA SET Chunking_Comments = 'Corrupt Document' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)       
                    else:
                        query = f"UPDATE NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA SET Chunking_Comments = '{str(e)}' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    # connection.commit()
                    time.sleep(2)
                ...
                if is_whole_doc_scanned:
                    query = f"UPDATE NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA SET Chunking_Comments = 'Scanned Document' WHERE File_Name = '{file_name}'"
                    print(query)
                    connection.execute(query)
                    continue
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if int(relevant_year[5:])>=18 and int(relevant_year[5:])<30:
                        ...
                        if (len(extracted_chunks)>0):
                            ...
                            for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                                ...
                                try:
                                    ...
                                    client.execute(f"INSERT INTO NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CORPUS_CHUNKED (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")
                                except:
                            ...
                            connection = engine.connect()
                            query='Update NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA set Chunking_Status="Done" where Company_Name="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                        ...
                        query='Update NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA set Chunking_Status="Not Relevant" where Company_Name="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                        connection = engine.connect()
                        connection.execute(text(query))
                        # connection.commit()
                        time.sleep(2)
                ...
                # query=f"Update NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA set Chunking_Comments='ISSUE' where File_Name = '{file_name}'"
                # print(query)
                # try:
                #     connection.execute(query)
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc)>1:
            ...
            for x in range(len(data)):
                ...
                link = data["File_Link"][x].replace("'", "''").replace("\\", "")
            ...
            for batch_num in range(total_batches):
                ...
                for doc_id, link in batch:
                    conditions.append(f"Document_Id = '{doc_id}', '{link}'")

                case_statement_link = f"multiIf({', '.join(conditions)}, Document_Link)"
                ...
                query = f"""
                    ALTER TABLE NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
        ...
        dcomp = client.execute('''SELECT distinct Document_Company as dcomp FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CORPUS_CHUNKED where Symbol = '';''')
        print(len(dcomp))
