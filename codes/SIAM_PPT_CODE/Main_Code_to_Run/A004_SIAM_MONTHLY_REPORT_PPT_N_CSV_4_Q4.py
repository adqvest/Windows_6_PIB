...
def round_excel(x, digits=0):
    ...
    z = abs(x) * 10 ** digits
    z = z + 0.5 + math.sqrt(2.220446049250313e-16)  # equivalent to R's .Machine$double.eps
    z = int(z)
    z = z / 10 ** digits
    return z * posneg
def table_preprocessing(data, frequency_normalizer='', period=3, unit='NA',
    ...
    print(data)
    total_year=len(data['Relevant_Date'])
    print('--------------total_year--------------->>>>>>>>>',total_year)
    if calculate_gr:
        growth_rate = round_excel(((data['Value'].iloc[-1] / data['Value'].iloc[0]) ** (1 / (total_year-1)) - 1) * 100, 1)
    ...
    data['Value'] = pd.to_numeric(data['Value'], errors='coerce')
    data['Value'] = data['Value'].apply(lambda x: round_excel(x, digits=0))
    data['Value'] = data['Value'].astype(float)
...
def round_and_format(x):
    return '{:,.2f}'.format(round_excel(x, 2))


def get_financial_yr(df):
    ...
    df['fiscal_year'] = df['Relevant_Date'].apply(get_fiscal_year)
    df['fiscal_year']=df['fiscal_year'].astype(str)
    df['Relevant_Date']=df['Relevant_Date'].astype(str)
    df['fiscal_year']=df['fiscal_year'].apply(lambda x : f'{x}')+'-'+df['Relevant_Date'].apply(lambda x : x.split('-',1)[1])
    df['Relevant_Date']=df['fiscal_year']
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try :
        ...
        r = robjects.r
        r.source("C:\\Users\\Administrator\\AdQvestDir\\codes\\SIAM_PPT_CODE\\Common_Function_and_Input_Files\\siam_common_function_quater_4.R") 

        #ALL FILES AND COMMON VARIABLE
        file = "C:/Users/Administrator/AdQvestDir/codes/SIAM_PPT_CODE/Common_Function_and_Input_Files/SIAM_Q4.csv"
        print(file)
        # Integrate PYTHON AND R
        r = robjects.r
        r.source("C:\\Users\\Administrator\\AdQvestDir\\codes\\SIAM_PPT_CODE\\Common_Function_and_Input_Files\\siam_common_function_quater_4.R") 
            ...
            for i in range(siam_df_ex.shape[0]):
                try:
                    if siam_df_ex['chart_creat_type'][i]!='table':
                        ...
                        data_pg=get_financial_yr(data_pg)
        ...
        value_columns=df_melted.columns[3:len(df_melted.columns)]
        df_melted['Unit']="In '000"
        
        # Apply the custom function to the DataFrame columns
        df_melted[value_columns] = df_melted[value_columns].applymap(round_and_format)
        current_datetime = datetime.now()
        max_date_siam=max(max_relevant_date_df['Max_Relevant_Date'])
        formatted_date = current_datetime.strftime("%d_%m_%Y_%H_%M")

        file_name_csv = "C:/Users/Administrator/AdQvestDir/codes/SIAM_PPT_CODE/Output_Files/SIAM_Q4_"+formatted_month_file+"_" + formatted_date + ".csv"
        file_name_ppt = "C:/Users/Administrator/AdQvestDir/codes/SIAM_PPT_CODE/Output_Files/SIAM_Q4_"+formatted_month_file+"_" + formatted_date + ".pptx"
                ...
                if siam_df['chart_creat_type'][i][1]=='table':
                    ...
                    for widget, columns in zip(widget_ids, column_names):
                        ...
                        for j in range(len(widget)):
                            ...
                            data_pg,average=niif_get_clean_data_frame(result,columns,div)
                            if 'FY' not in siam_df['slide_heading'][i][j]:
                                data_pg=get_financial_yr(data_pg)
                                print('----------------TABLE WITH FINANCIAL YEAR----------------')
                                print(data_pg)
                            print(average)
                            if average >2:
                                if j ==0:
