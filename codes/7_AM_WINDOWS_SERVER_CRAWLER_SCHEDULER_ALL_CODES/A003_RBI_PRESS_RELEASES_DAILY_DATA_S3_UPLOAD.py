"""
Created on Sun Jun 22 21:52:21 2025
@author: Santonu
"""
...
def run_program(run_by = 'Adqvest_Bot', py_file_name = None):
    ...
    try :
        ...
        #%%
        lookup_dict={'RBI_PRESS_RELEASES_DAILY_DATA_CORPUS':["RBI_PRESS_RELEASES_DAILY_CORPUS_TO_BE_CHUNKED/","RBI_PRESS_RELEASES_DAILY_CORPUS/"]}
        for Working_table,keys in lookup_dict.items():
            
            query = f"SELECT * FROM {Working_table} where S3_Upload_Status is Null and S3_Upload_Comments is null;"
            ...
            for index, row in raw_df.iterrows():
                ...
                filepath = os.path.join(r'C:\\Users\\Administrator\\AdQvestDir\\RBI', filename)
                try:
                ...
                #     connection=engine.connect()
                #     connection.execute(f"update {Working_table} set S3_Upload_Comments='Failed' where File_Name = '" +str(filename)+"'")
                #     connection.execute("commit")
                #     print('-' *80)
        ...
        df_links=pd.read_sql("select * from AdqvestDB.RBI_PRESS_RELEASES_DAILY_DATA_CORPUS where S3_Upload_Status is Null and S3_Upload_Comments != 'File Not Available'",engine)
