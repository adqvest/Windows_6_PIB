...
from  adqvest_chunking_functions import read_large_file_names_from_s3_folder,read_all_file_names_from_s3_folder,read_all_file_content,delete_file_from_s3
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        chunking_table="AdqvestDB.DEA_ECONOMY_REPORT_MONTHLY_DATA_CORPUS_CHUNKED"
        ...
        print(f'''SELECT distinct Document_Id FROM {chunking_table};''')
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if int(relevant_year[5:])>=18 and int(relevant_year[5:])<30:
                        ...
                        if (len(extracted_chunks)>0):
                            ...
                            connection = engine.connect()
                            query=f"Update {corpus_table} set Chunking_Status='Done' where File_ID='{str(file_id)}'"
        ...
        doc = client.execute(f'''SELECT distinct Document_Id as id FROM {chunking_table} where Published_Date is Null;''')
        print(len(doc))
