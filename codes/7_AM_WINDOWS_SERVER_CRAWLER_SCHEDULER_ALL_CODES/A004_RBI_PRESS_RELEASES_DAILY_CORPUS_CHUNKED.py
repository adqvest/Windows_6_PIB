...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'RBI_PRESS_RELEASES_DAILY_CORPUS_TO_BE_CHUNKED/'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'RBI_PRESS_RELEASES_DAILY_CORPUS_TO_BE_CHUNKED/' in x]
        response = response [1:]
        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM RBI_PRESS_RELEASES_DAILY_DATA_CORPUS_CHUNKED;''')
        ...
        for obj in response:
            ...
            try:
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute("update RBI_PRESS_RELEASES_DAILY_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name)+"'")
                        ...
                        # connection=engine.connect()
                        # connection.execute("update RBI_PRESS_RELEASES_DAILY_DATA_CORPUS set Chunking_Comment='Issue' where File_Name = '" +str(file_name)+"'")
                        # connection.execute("commit")
                        continue
                ...
                if 'Failed to load document (PDFium: Data format error)' in str(e):
                    connection=engine.connect() 
                    connection.execute(sqltext("UPDATE RBI_PRESS_RELEASES_DAILY_DATA_CORPUS SET Chunking_Comments='File Not Available' WHERE File_Name = :file_name"),{"file_name": file_name})
                # connection.execute("commit")
                else:
                    connection=engine.connect() 
                    connection.execute(sqltext("UPDATE RBI_PRESS_RELEASES_DAILY_DATA_CORPUS SET Chunking_Status='ISSUE' WHERE File_Name = :file_name"),{"file_name": file_name})
                # connection.execute("commit")
                continue
            print('_' * 150)    
        connection = engine.connect()
        query = "select * from AdqvestDB.RBI_PRESS_RELEASES_DAILY_DATA_CORPUS ORDER BY File_Id ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id FROM RBI_PRESS_RELEASES_DAILY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE RBI_PRESS_RELEASES_DAILY_DATA_CORPUS_CHUNKED
                    UPDATE 
            ...
            query = """
                    ...
                    AND Document_Link NOT ILIKE '%.html%' 
                    AND Document_Link NOT ILIKE '%.htm%';
                    """
            ...
            connection = engine.connect()
            query = f"select * from AdqvestDB.RBI_PRESS_RELEASES_DAILY_DATA_CORPUS ORDER BY File_ID ASC;"
            data = pd.read_sql(query, con=engine)

            doc = client.execute(f'''SELECT distinct Document_Id as id FROM RBI_PRESS_RELEASES_DAILY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
            print(len(doc))
            ...
            doc = client.execute(f'''SELECT distinct Document_Id as id FROM RBI_PRESS_RELEASES_DAILY_DATA_CORPUS_CHUNKED where Published_Date is Null;''')
            print(len(doc))
