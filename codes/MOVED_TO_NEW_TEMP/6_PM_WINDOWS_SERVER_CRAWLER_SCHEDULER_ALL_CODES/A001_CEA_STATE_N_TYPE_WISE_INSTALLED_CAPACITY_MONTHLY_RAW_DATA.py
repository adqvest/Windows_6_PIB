                ...
                for i in soup.findAll('a', {'target': '_blank'}, href=True):
                    if '.pdf' in i.get('href'):
                        ...
                        try:
                            ...
                            df['Region'] = df.Region.apply(lambda x : x[1] if x == str else x)
                            ...
                            try:
                                ...
                                df['Region'] = np.where(df.iloc[:, 0].str.lower().str.contains('region|islands', na=False), df.iloc[:, 0], np.nan)
                                df['Region'] = df['Region'].fillna('')
                                df['Region'] = df.Region.str.replace('Total', '').str.replace('(', '').str.replace(')', '').str.strip()
                        ...
                        df = df.reset_index(drop=True)
                        try:
                            str_idx = df[df.iloc[:, 0].str.lower().str.contains('state', na=False)].index
                            df=df.iloc[str_idx[0]:,:]
                            df = df.reset_index(drop=True)
                        ...
                        df = df.reset_index(drop=True)
                        index_list = df[df['State'].str.contains('List of Projects', case=False, na=False)].index.tolist()
                        if len(index_list) == 0:
                            index_list = [-1]
                        df=df[:index_list[0]]
                        ...
                        df['State'] = np.where(df.State.str.lower().str.contains('unallocated'), 'Unallocated', df.State)
                        df['Sector'] = np.where(df.State.str.lower().str.contains('unallocated'), 'Central', df.Sector)
                        ...
                        print(len(df))
                        if len(df) > 119:
                            raise Exception(f'Increase in Data count for {start_date.strftime("%b %Y")}')
