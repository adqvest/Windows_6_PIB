...
headers1 = {
    ...
    'Referer': 'https://aishe.gov.in/aishe/institutionAisheCode',
...
def get_main_pg_info_and_links(st_id,inst_id,ut_id,it_id,standalone):
    ...
    while retries < max_retries:
        try:
            r = requests.get('https://aishe.gov.in/aishe/institutionAisheCode', params=params, 
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try :
        ...
        if today.date()-db_max_date>= datetime.timedelta(30):
            update_status_table(f"UPDATE AISHE_CODES_MONTHLY_DATA_STATUS_TABLE set Relevant_Date='{str(pd.to_datetime(today.date(), format='%Y-%m-%d').date())}', Col_Status=NULL;")

        
        status_tbl=pd.read_sql("select State,Max(Relevant_Date) as Relevant_Date from  AISHE_CODES_MONTHLY_DATA_STATUS_TABLE where Col_Status is Null group by State;",engine)
        print(status_tbl)
        if len(status_tbl)>0:
            ...
            for st,st_id in state_code1.items():
                ...
                connection = engine.connect()
                if len(df_st)>0:
                    df_st.to_sql('AISHE_CODES_MONTHLY_DATA',index=False, if_exists='append', con=engine)
                    update_status_table(f"UPDATE AISHE_CODES_MONTHLY_DATA_STATUS_TABLE set Col_Status='Done' where State='{st.title()}';")
                    print(df_st.info())
                time.sleep(10)
