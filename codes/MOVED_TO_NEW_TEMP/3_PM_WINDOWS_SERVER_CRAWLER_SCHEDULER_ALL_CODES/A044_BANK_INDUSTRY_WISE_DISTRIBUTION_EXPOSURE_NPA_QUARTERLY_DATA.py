...
def get_correct_links(links_capital):
    links_capital=links_capital[links_capital['Bank_Industry_Wise_Distribution_Exposure_Npa_Status']!='COMPLETED']
...
#     connection.execute("update BANK_BASEL_III_QUARTERLY_LINKS set Bank_Industry_Wise_Distribution_Exposure_Npa_Status = 'COMPLETED' where Relevant_Date = '"+date+"' and Bank= '"+bank_name+"'")
def update_basel_iii(bank_name,date):
    engine = adqvest_db.db_conn()
    connection = engine.connect()
    connection.execute("update BANK_BASEL_III_QUARTERLY_LINKS set Bank_Industry_Wise_Distribution_Exposure_Npa_Status = 'COMPLETED' where Relevant_Date = '"+str(date)+"' and Bank= '"+bank_name+"'")
    ...
    try :
        ...
        def data_collection_sbi(tables,search_str,link,max_date,bank_type,bank,date,type_read,loop):
            try:
                ...
                df['Total_Gross_Npa_In_Millions'] = df['Total_Gross_Npa_In_Millions'].replace('', np.nan)
                df['Total_Gross_Npa_In_Millions'] = df['Total_Gross_Npa_In_Millions'].astype(float)*10
                ...
                if loop <2:
                    ...
                    df=data_collection_sbi(tables,search_str,link,max_date,bank_type,bank,date,'ilovepdf',2)
                else:
        ...
        if links_capital.empty:
            print('No new data for STATE BANK OF INDIA')
        else:
            print('New data for STATE BANK OF INDIA')
        ...
        if links_capital.empty:
            ...
            for i in range(len(links_capital)):
                ...
                try:
                    try:
                        tables = extract_tables_from_pdf(link,True,'stream',10,200,70)
                        df=data_collection_hdfc(tables,search_str,link,max_date,bank_type,bank,date,'camelot',1,False)
                    except:
                        df=data_collection_hdfc('',search_str,link,max_date,bank_type,bank,date,'camelot',1,False)
                    update_basel_iii(bank,date)
                except Exception as e:
                    catch_error_df=get_error_df(table_name,bank,e,date,catch_error_df)
        ...
        def data_collection_axis(tables,search_str,link,max_date,bank_type,bank,date,type_read,loop):
            try:
                ...
                if loop <2:
                    ...
                    end_text='total'
                    df=search(heading,end_text,path,sheets)
                    df=data_collection_axis(tables,search_str,link,max_date,bank_type,bank,date,'ilovepdf',2)
                else:
                    e='Nothing worked try manually'
                    catch_error_df=get_error_df(table_name,bank,e,date,catch_error_df)
            return df
        ...
        def data_collection_union(tables,search_str,link,max_date,bank_type,bank,date,type_read,loop):
            try:
                if type_read=='camelot':
                    df=get_desired_table(tables,search_str)
                else:
        ...
        search_str=['gross npa']
        if links_capital.empty:
            print('No new data for UNION BANK OF INDIA')
        else:
            print('New data for UNION BANK OF INDIA')
            for i in range(len(links_capital)):
                link=links_capital['Links'][i]
                bank_type = links_capital['Bank_Type'][i]
                date = links_capital['Relevant_Date'][i]
        ...
        def data_collection_pnb(tables,search_str,link,max_date,bank_type,bank,date,type_read,loop):
            try:
                if type_read=='camelot':
                    ...
                    df=tables
                    df.reset_index(drop=True,inplace=True)
                str_index=row_col_index_locator(df,['food processing'])[1]
                end_index=row_col_index_locator(df,['Construction'])[1]
                df=df.iloc[str_index-1:end_index+1]
                df.reset_index(drop=True,inplace=True)
                print(df)
                col=row_col_index_locator(df,['food processing'])[0]
                col1=row_col_index_locator(df,['amount of npa'])[0]
                col2=row_col_index_locator(df,['general provisions'])[0]
                col3=row_col_index_locator(df,['writeoffs'])[0]
                col4=row_col_index_locator(df,['specific provisions'])[0]
                df=df.iloc[1:,[col,col1,col2,col3,col4]]
                print(df)
                df.columns=['Industry','Total_Gross_Npa_In_Millions','Total_Provisions_For_Npa_In_Millions','Qtrly_Write_Offs_In_Millions','Qtrly_Provisions_For_Npa_In_Millions']
                df['Sub_Industry']=np.nan
                ...
                if loop<2:
                    ...
                    end_text='total'
                    df=search(heading,end_text,path,sheets)
                    npa_df= data_collection_pnb(tables,search_str,link,max_date,bank_type,bank,date,type_read,2)
                else:
                    e='Nothing worked try manually'
                    catch_error_df=get_error_df(table_name,bank,e,date,catch_error_df)
