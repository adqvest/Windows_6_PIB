...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        def get_dates(cond):
            ...
            query = "select min(Relevant_Date) as Min FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_MONTHLY_STATUS_HYBRID"
        ...
        def data_check():
            ...
            if(error_df.empty):
                ...
                for index,i in error_df.iterrows():
                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_MONTHLY_STATUS_HYBRID set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                    connection.execute("commit")

                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_MONTHLY_STATUS_HYBRID set State_Status = Null where State = '" +i["State"] +"'")
                    connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_MONTHLY_STATUS_HYBRID", con=engine)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_MONTHLY_STATUS_HYBRID", con=engine, index=False, if_exists='append')
                    ...
                    for st in state_info:
                        ...
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_MONTHLY_STATUS_HYBRID set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
                ...
                try:
                    ...
                    pass
            data_check()
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_MONTHLY_STATUS_HYBRID"
