...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_TRACTOR_MONTHLY_DATA'
    ...
    try:
        ...
        def clicking_tabular_summary(driver):
            ...
            while True:
                try:
                    no_of_ping += 1
                    driver.find_element(By.XPATH,"//div[@class = 'ui-selectonemenu-trigger ui-state-default ui-corner-right']//span[@class='ui-icon ui-icon-triangle-1-s ui-c']").click()
                    time.sleep(2)
                    no_of_ping += 1
                    driver.find_element(By.XPATH, "//li[@class='ui-selectonemenu-item ui-selectonemenu-list-item ui-corner-all'][@data-label='Tabular Summary']").click()
                    time.sleep(1)
                    break
                except:
                    limit = limit + 1
        ...
        def download_file(driver):
            global no_of_ping
            os.chdir(r"C:\Users\Administrator\Junk_Vahan_STATE_LEVEL_TRACTORS_MP")
            limit = 0
            while True:
                try:
                    no_of_ping += 1
        ...
        def get_states(driver):
            ...
            while True:
                try:
                    ...
                    state_tags=u.findAll('li')[1:]
                    for i,st in enumerate(state_tags[:]):
                        state_info[i] = st.text.strip()
        ...
        def get_fuels(driver):
            ...
            while True:
                try:
                    ...
                    required_fuels=['DIESEL/HYBRID', 'PETROL/HYBRID', 'STRONG HYBRID EV']
        ...
        def read_file(type_,state,rel_date,overall = False):
            os.chdir(r"C:\Users\Administrator\Junk_Vahan_STATE_LEVEL_TRACTORS_MP")
            key = 'VAHAN/STATE_LEVEL_TRACTORS/'
            os.chdir(r"C:\Users\Administrator\Junk_Vahan_STATE_LEVEL_TRACTORS_MP")
        ...
        def get_dates(cond):
            ...
            query = "select min(Relevant_Date) as Min FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID"
        ...
        def data_check():
            ...
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP  group by RTO_Office_Raw,Relevant_Date",engine)
            if (data_df.empty==False):
                ...
                if(error_df.empty):
                    ...
                    for index,i in error_df.iterrows():
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                        connection.execute("commit")

                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID set State_Status = Null where State = '" +i["State"] +"'")
                        connection.execute("commit")


                        connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                        connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    download_file_path = r"C:\Users\Administrator\Junk_Vahan_STATE_LEVEL_TRACTORS_MP"
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID", con=engine)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID", con=engine, index=False, if_exists='append')
                    ...
                    for st in state_info:
                        ...
                        vahan_maker_cat_fuel_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP", con=engine, index=False, if_exists='append')
                        ...
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
                ...
                try:
                    ...
                    pass
            data_check()
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID"
            connection.execute(query)
            connection.execute('commit')

            df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP", con=engine)
            if df.empty==False:
                

                #maker vs category
                date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP", con=engine)
                ...
                query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID where Relevant_Date>='" + date_vahan + "'"
                connection.execute(query)
                connection.execute('commit')

                try:
                    df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP", con=engine)
                    df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID', con=engine, index=False, if_exists='append')

                except:
                    print('ERROR 3')
                    raise Exception('error in VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP')


            query = "drop table VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP"
            connection.execute(query)
            connection.execute('commit')
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP')"
        ...
        query = "SELECT * FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_TRAC_MONTHLY_STATUS_HYBRID"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [1,8,15,22,29,25, 28]):   #1,4,11,12,16,22,27


                print('B')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_TRACTOR_MONTHLY_DATA limit 0"
                connection.execute(query)
                connection.execute('commit')
