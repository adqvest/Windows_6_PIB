...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_3W_MONTHLY_HYBRID'
    ...
    try:
        ...
        def get_dates(cond):
            ...
            query = "select min(Relevant_Date) as Min FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_3W_MONTHLY_STATUS_HYBRID"
            ...
            if (data_df.empty==False):
                ...
                if(error_df.empty):
                    ...
                    for index,i in error_df.iterrows():
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_3W_MONTHLY_STATUS_HYBRID set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                        connection.execute("commit")

                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_3W_MONTHLY_STATUS_HYBRID set State_Status = Null where State = '" +i["State"] +"'")
                        connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_3W_MONTHLY_STATUS_HYBRID", con=engine)
                    print(maker_df)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_3W_MONTHLY_STATUS_HYBRID", con=engine, index=False, if_exists='append')
                    ...
                    for st in state_info:
                        ...
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_3W_MONTHLY_STATUS_HYBRID set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
                ...
                try:
                    ...
                    pass
            data_check()
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_3W_MONTHLY_STATUS_HYBRID"
            ...
            if df.empty==False:
                ...
                query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_3W_MONTHLY_HYBRID where Relevant_Date>='" + date_vahan + "'"
                connection.execute(query)
                connection.execute('commit')
                ...
                try:
                    ...
                    if (df.empty==False):
                        df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_3W_MONTHLY_HYBRID', con=engine, index=False, if_exists='append')
        ...
        query = "SELECT * FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_3W_MONTHLY_STATUS_HYBRID"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [1,8,15,22,29,25, 28]):
