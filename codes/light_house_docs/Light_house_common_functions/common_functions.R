...
#PG_ADMIN
Adqvest_Test_Thurro=read.delim("C:\\Users\\Administrator\\AdQvestDir\\codes\\R_Db_Properties\\Postgress_properties_test.txt", 
                              sep = "", header = T)
...
filter_data <- function(df, num_months) {
  max_rel_date <- max(df$Relevant_Date)
  cutoff_date <- max_rel_date %m-% months(num_months)
  return(df[df$Relevant_Date >= cutoff_date, ])
}
...
get_val_pie=function(df){
 ...
 val_pp= c("","","21.9","35.2","78.1","122.5","151.9","250.4")
...
light_house_economic_table=function(df,font_type= "Times New Roman", 
                                        ...
                                        row_bracket=c(),notes='',spc=1,space=2,suffix=0.1){
  ...
  ft = flextable(df) 
  ft = theme_alafoli(ft) %>% 
    set_formatter(i =1:nrow(df),j=2:ncol(df)-1,part = "body", align = "right")
  
  ft <- colformat_double(
    ft,
    na_str = "n.a."  # Example NA string
  )
  ...
  if (length(row_unit) != 0){
    for (m in names(row_unit)){
      ...
      for (i in row_unit[[m]]){ 
        ...
        ft=colformat_double(ft,i=row_formula,j=2:ncol(df)-1,suffix=val,digits=1,na_str = "n.a.")
      }
    }
  }
  
  ft=ft %>% 
    width(j=names(df)[length(df)-1],1.28) %>% 
    width(j=2:length(df)-1,1.28) %>% 
    width(j='Variable',var_col_width) %>% 
    width(j=' ',suffix)
  ...
  if (length(row_bracket)!=0){
    for (m in row_bracket){
      ...
      for (cl in 2:ncol(df)-1){
        # print(cl)
        formatted_value <- formatC(df[row_indx, cl], format = "f", big.mark = ",", digits = 0)
        neg <- sprintf("(%s)", formatted_value)
        ft <- compose(ft, i = row_indx, j = cl, value = as_paragraph(as_chunk(neg)))
      ...
      ft <- ft %>% 
        ...
        fontsize(i = NULL, j = NULL, size = 13, part = "header") %>% 
        fontsize(i = NULL, j = NULL, size = 13, part = "body") %>% 
      ...
      line_spacing(i = NULL, j = NULL, space = space, part = "footer") 
      # ft <- ft %>%
      #   add_footer_lines('   Please update Sensex P/E') %>%
      #   fontsize(i = NULL, j = NULL, size = spc, part = "footer") %>%
      #   color(i = 2, j = NULL, color = "red", part = "footer")
      ft <- ft %>% 
        fontsize(i = NULL, j = length(df), size = 8, part = "body")
      # ft <- ft %>%
      #   add
