...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'ANAROCK_HOSPITALITY_MARKET_VIEWPOINTS_CORPUS_TO_BE_CHUNKED'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'ANAROCK_HOSPITALITY_MARKET_VIEWPOINTS_CORPUS_TO_BE_CHUNKED/' in x]
        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED;''')
        ...
        for obj in response:
            ...
            try:
                ...
                try:
                    ...
                    connection.execute("update ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection=engine.connect()
                                connection.execute("update ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name)+"'")
                                connection.execute("commit")
                                continue      
                        ...
                        connection=engine.connect()
                        connection.execute("update ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '" +str(file_name)+"'")
                        connection.execute("commit")
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                client.execute(f"INSERT INTO ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")   

                                # client.execute(f"INSERT INTO ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED (Document_Id,Chunk_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.str.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{datetime.datetime.now(india_time)}');")   
                                # client.execute(f"INSERT INTO ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED (document_id,document_company,document_company_small_case,document_type,document_year,document_date,page_number,document_content,document_content_small_case,document_content_modify,embedding,embedding_small_case) VALUES ('{file_id}','{company_name}','{company_name_small}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''','''{chunk_small}''','''{new_chunk}''',{chunk_embed},{chunk_embed_small});") 
                                connection=engine.connect()
                                connection.execute("update ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS set Chunking_Status='Done' where File_Name = '" +str(file_name)+"'")
                                connection.execute("commit")
                            except:
        ...
        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
