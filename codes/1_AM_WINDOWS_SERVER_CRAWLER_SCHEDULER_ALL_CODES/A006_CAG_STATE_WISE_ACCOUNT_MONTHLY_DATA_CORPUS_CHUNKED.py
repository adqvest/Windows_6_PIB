# -*- coding: utf-8 -*-
"""
Created on Fri Mar 21 14:34:43 2025
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_TO_BE_CHUNKED/'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_TO_BE_CHUNKED/' in x]
        response = response [1:]
        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_CHUNKED;''')
        ...
        for obj in response:
            ...
            try:
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute("update CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name)+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '" +str(file_name)+"'")
                        # connection.execute("commit")
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                # client.execute(f"INSERT INTO CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_CHUNKED (Document_Id,Chunk_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.str.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{datetime.datetime.now(india_time)}');")   
                                client.execute(f"INSERT INTO CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_CHUNKED (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")   
                                connection=engine.connect() 
                                connection.execute(sqltext("UPDATE CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS SET Chunking_Status='Done' WHERE File_Name = :file_name"),{"file_name": file_name.replace('.pdf','')})
                                # connection.commit()
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = sqltext("UPDATE CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS SET Chunking_Comments= :error WHERE File_Name = :file_name")
                        connection.execute(query, {"file_name": file_name.replace('.pdf',''),"error": 'File Not Available'})
                        print(query)
                        # connection.execute(query)
                    else:
                        query = sqltext("UPDATE CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS SET Chunking_Comments= :error WHERE File_Name = :file_name")
                        connection.execute(query, {"file_name": file_name.replace('.pdf',''),"error": str(e)})
                        print(query)
                        # connection.execute(query)
                    # connection.commit()
                    time.sleep(2)
            ...
            #     print("****************** SOMETHING WRONG WITH FILE ******************")
            #     connection=engine.connect() 
            #     connection.execute(sqltext("UPDATE CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS SET Chunking_Comments='ISSUE' WHERE File_Name = :file_name"),{"file_name": file_name.replace('.pdf','')})
            #     # connection.commit()
            #     continue
        
        connection = engine.connect()
        query = "select * from AdqvestDB.CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS ORDER BY File_Id ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM CAG_STATE_WISE_ACCOUNT_MONTHLY_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
