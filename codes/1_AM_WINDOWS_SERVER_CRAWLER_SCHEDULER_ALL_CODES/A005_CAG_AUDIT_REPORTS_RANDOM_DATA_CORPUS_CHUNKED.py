...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'CAG_AUDIT_REPORT_FILES_TO_BE_CHUNKED/'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'CAG_AUDIT_REPORT_FILES_TO_BE_CHUNKED' in x]
        # response = response [600:]
        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS_CHUNKED;''')
            ...
            try:
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute(text(f"UPDATE CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS SET Chunking_Status='Not Relevant' WHERE File_ID = '{file_id}';"))
                                # connection.commit()
                                continue      
                        ...
                        connection = engine.connect()
                        connection.execute(text(f"UPDATE CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS SET Chunking_Comments='ISSUE' WHERE File_ID = '{file_id}';"))
                        # connection.commit()
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                client.execute(f"INSERT INTO CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS_CHUNKED (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")
                                # client.execute(f"INSERT INTO CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS_CHUNKED (document_id,document_company,document_company_small_case,document_type,document_year,document_date,page_number,document_content,document_content_small_case,document_content_modify,embedding,embedding_small_case) VALUES ('{file_id}','{company_name}','{company_name_small}','{report_type}','{relevant_year}','{document_date}',{page},'{chunk}','{chunk_small}','{new_chunk}',{chunk_embed},{chunk_embed_small});") 
                                # client.execute(f"INSERT INTO CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS_CHUNKED (Document_Id,Chunk_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.str.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{datetime.datetime.now(india_time)}');")   
                                connection = engine.connect()
                                connection.execute(text(f"UPDATE CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS SET Chunking_Status='Done' WHERE File_ID = '{file_id}';"))
                                # connection.commit()
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE File_ID = '{file_id}'"
                        print(query)
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for x in range(len(data)):
                ...
                link = data["Report_Link"][x].replace("'", "\\'")  # Escape single quotes
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM CAG_AUDIT_REPORTS_RANDOM_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
