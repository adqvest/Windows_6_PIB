...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        download_file_path = "C:\\Users\\Administrator\\AdQvestDir\\Junk folder\\"
        ...
        for option in select_to_month.find_elements_by_tag_name("option"):
            if option.text == cur_month:
                option.click()
                break
        driver.find_element_by_xpath('//a[text()=' + str(cur_day) + ']').click()
        ...
        for i in items_list_new:
            ...
            #     max_rel_date=pd.read_sql('select max(Relevant_Date) as Max where Commodity='+i+' from AdqvestDB.MCX_SPOT_PRICE_DAILY_DATA',engine).Max[0]
