...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'MOCI_EXPORT_6_DIGIT_COMMODITY_N_COUNTRY_WISE_MONTHLY_DATA'
    ...
    try :
        ...
        #                 
                        engine = adqvest_db.db_conn()
                        connection = engine.connect()
                        connection.execute("Delete from MOCI_EXPORT_6_DIGIT_COMMODITY_N_COUNTRY_WISE_MONTHLY_DATA where Relevant_Date='"+str(end_date(int(yr),int(mon)))+"' and HSCode='"+str(import_200['HSCode'].iloc[i])+"'")
                        connection.execute('commit')
                        time.sleep(3)
                        df_final['Country']= df_final['Country'].str.replace("'","")
                        engine = adqvest_db.db_conn()
                        df_final.to_sql("MOCI_EXPORT_6_DIGIT_COMMODITY_N_COUNTRY_WISE_MONTHLY_DATA", if_exists="append", index=False, con=engine)
        ...
        dates=pd.read_sql("select distinct Relevant_Date as Max from MOCI_EXPORT_6_DIGIT_COMMODITY_WISE_MONTHLY_DATA order by Relevant_Date desc",engine)['Max']
        print(dates)
        for date in dates[2:10]:
            HSCode_200=pd.read_sql("select distinct HSCode as HSCode,Commodity from MOCI_EXPORT_6_DIGIT_COMMODITY_WISE_MONTHLY_DATA where Relevant_Date='"+str(date)+"' order by Monthly_Value_INR_In_Lakhs_Current_Year desc limit 500",engine)
