# -*- coding: utf-8 -*-
"""
Created on Tue Aug 10 15:42:15 2021
...
#%%
        ...
        done = pd.read_sql("Select Distinct Page from AdqvestDB.MEFCC_PROPOSALS_YEARLY_DATA_PARIVESH_FINAL_PIT",con=engine)
        done_cols = pd.read_sql("Select * from AdqvestDB.MEFCC_PROPOSALS_YEARLY_DATA_PARIVESH_FINAL_PIT limit 1",con=engine)
        headers = {"user-agent" : "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36"}
...
#        output.columns = [x.replace("Moefcc")]
        ...
        if bool1:
          ...
          output.to_sql(name = "MEFCC_PROPOSALS_YEARLY_DATA_PARIVESH_FINAL",con = engine,if_exists = 'append',index = False)
    ...
    #            output = output[list(done_cols.columns)]
              ...
              continue
        check.to_sql(name = "MEFCC_PROPOSALS_YEARLY_DATA_PARIVESH_FINAL_PIT_NOT_FOUND",con = engine,if_exists = 'append',index = False)
        print("Data Uploaded")
        connection = engine.connect()
