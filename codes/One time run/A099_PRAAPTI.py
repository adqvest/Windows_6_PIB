...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'PRAAPTI_MONTHLY_DATA'
    ...
    try :
        ...
        site_date = "https://praapti.in/state-dashboard/uttar-pradesh"

        r = requests.get(site_date,headers=headers)

        soup = BeautifulSoup(r.text,'lxml')

        site_date = soup.find_all("div",{"class":"col-md-12 StatePage_Heading"})[0].text.strip()

        site_date = "-".join([site_date.split()[-2],site_date.split()[-1][-2:]])

        site_date = datetime.datetime.strptime(site_date, '%b-%y')

        site_date = last_day_of_month(site_date).date()

        db_date = pd.read_sql("Select max(Relevant_Date) as RD from AdqvestDB.PRAAPTI_MONTHLY_DATA",con=engine)['RD'][0]

        if site_date > db_date:
            ...
            years = [site_date.year]
            years = [str(x) for x in years]
            i = str(site_date.month)
            for y in years:
                print(y)
                for st,co in zip(st_codes,codes1):
                    ...
                    discom_combo = soup4.find_all("p",{'class':'GenCoNameText'})
                    entity_name = soup4.find_all("p",{'class':'GenCoNameText'})
                    discoms = [x['onclick'] for x in discom_combo]
                    discoms = [x.split("getDiscomCode1")[1].split(";")[0] for x in discoms]
                    entity_name = [x.text for x in entity_name]
                    entity_name = [re.match(r"[^[]*\[([^]]*)\]", x).groups()[0] for x in entity_name]
                    #    discoms = [x.split('(', 1)[1].split(')')[0] for x in discoms]
                    for vals,na in zip(discoms,entity_name):
                        print(vals,na)
                        discom1 = eval(vals)[0]
                        discom2 = eval(vals)[1]
                        print(st)
                ...
                cols = ['Invoice_No', 'Invoice_Date', 'Amount_Billed_To_Discom_Cr',
                       ...
                       'Percentage_Pending_Payment_Status', 'Due_Since_Days', 'State', 'State_Code',
                       'Discom', 'Entity','Entity_2','Relevant_Date','Runtime']

                main1.columns = cols


                main1.to_sql("PRAAPTI_MONTHLY_DATA",con=engine,if_exists='replace',index=False)
