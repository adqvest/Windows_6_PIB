...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'AMC_COMPANY_WISE_ABRIDGED_ANNUAL_REPORT_TO_BE_CHUNKED'
        ...
        response_org = [ x for x in response_org if 'AMC_COMPANY_WISE_ABRIDGED_ANNUAL_REPORT_TO_BE_CHUNKED/' in x]
        response_org=[i for i in response_org if i.split('_')[-1].replace('.pdf','')!='12920']
        response_org=[i for i in response_org if i.split('_')[-1].replace('.pdf','')!='12960']
        req_quarter=['FY22']
        ...
        execution_list = ["ICICI Prudential Mutual Fund"]
        ...
        for company in execution_list:
            ...
            doc_ids = client.execute(f'''SELECT distinct document_id FROM thurro_pdf_documents_vector_db_amc_company_wise_abridged_annual_reports;''')
                            ...
                            if (len(extracted_chunks)>0):
                                ...
                                for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                                    ...
                                    try:
                                        ...
                                        client.execute(f"INSERT INTO thurro_pdf_documents_vector_db_amc_company_wise_abridged_annual_reports (document_id,document_company,document_company_small_case,document_type,document_year,document_date,document_link,document_content,document_content_small_case,document_content_modify,embedding,embedding_small_case) VALUES ('{file_id}','{company_name}','{company_small}','{report_type}','{relevant_year}','{document_date}','{link}','''{chunk}''','''{chunk_small}''','''{new_chunk}''',{chunk_embed},{chunk_embed_small});") 
