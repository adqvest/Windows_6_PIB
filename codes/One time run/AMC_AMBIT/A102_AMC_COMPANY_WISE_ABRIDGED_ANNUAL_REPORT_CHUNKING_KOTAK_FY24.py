...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        req_quarter=['FY24','FY23','FY22']
        execution_list = ["Kotak Mahindra Mutual Fund"]
        req_file_id=['2652','2653','2654']
                    ...
                    if str(file_id) not in doc_ids :
                            ...
                            if len(text)>0:
                                for i, chunk in enumerate(text):
                                    ...
                                    # try:
                                    if len(table)>0:
                                        table_content_str=table[0][1][0]
                                        # print(table_content_str)
                                        table_content_str = table_content_str.replace("\\", "\\\\")  # Escaping backslashes
                                        table_content_str = table_content_str.replace("\n", "\\n")  # Escaping newlines
                                        table_content_str = table_content_str.replace("\r", "\\r")  # Escaping carriage returns
                                        table_content_str = table_content_str.replace("'","\\'") # Escaping single quotes
                     
                                        # table1 = [str(x) for x in table1]
                                        # table_content_str = "[" + ", ".join(f"'{x}'" for x in table1) + "]"



                                        table_embed = str(create_embeddings(table[0][1])).replace('[[','[').replace(']]',']')
                                        table_embed_small = str(create_embeddings(table[0][1][0].lower())).replace('[[','[').replace(']]',']')
                                    
                                    else:
                                        table_content_str=''
                                        # table_content_str = json.dumps(table1)  
                                        # table_content_str = "[" + ", ".join(f"'{x}'" for x in table1) + "]"
