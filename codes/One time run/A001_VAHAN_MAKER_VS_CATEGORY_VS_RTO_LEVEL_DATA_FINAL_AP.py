...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        def get_dates(cond):
            ...
            query = "select distinct Relevant_Date FROM VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_CLEAN_FINAL where Relevant_Date>='2022-01-01'"
        ...
        def data_check():
            ...
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP  group by RTO_Office_Raw,Relevant_Date",engine)
            ...
            if(error_df.empty):
                ...
                for index,i in error_df.iterrows():
                    connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                    connection.execute("commit")

                    connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP set State_Status = Null where State = '" +i["State"] +"'")
                    connection.execute("commit")


                    connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    download_file_path = r"C:\Users\Administrator\Junk_Vahan_Full_RTO_AP"

                    os.chdir(r"C:\Users\Administrator\Junk_Vahan_Full_RTO_AP")
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP", con=engine)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP", con=engine, index=False, if_exists='append')

                    #########################################
                    query = "select min(Relevant_Date) as Min FROM VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP"
                    ...
                    for st in state_info:
                        ...
                        for x in rto_info.keys():
                            ...
                            vahan_maker_cat_fuel_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP", con=engine, index=False, if_exists='append')


                            connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP set RTO_Status = 'Done' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")

                            connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP set Runtime = '"+datetime.datetime.now(india_time).strftime("%Y-%m-%d %H:%M:%S")+"' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")
                        ...
                        connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
                ...
                try:
                    ...
                    pass
            data_check()
            query = "delete from VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP"
            connection.execute(query)
            connection.execute('commit')

            #maker vs category
            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP", con=engine)
            ...
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_FINAL_AP where Relevant_Date>='" + date_vahan + "'"
            connection.execute(query)
            connection.execute('commit')


            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP where Relevant_Date>='" + date_vahan + "'", con=engine)
                df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_FINAL_AP', con=engine, index=False, if_exists='append')
                #ClickHouse

            except:
                print('ERROR 3')
                raise Exception('error in VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP')


            query = "drop table VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP"
            connection.execute(query)
            connection.execute('commit')
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP')"
        ...
        query = "SELECT * FROM VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_AP"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [1,11,21]):
                ...
                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2_AP select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_FINAL_AP limit 0"
                connection.execute(query)
                connection.execute('commit')
