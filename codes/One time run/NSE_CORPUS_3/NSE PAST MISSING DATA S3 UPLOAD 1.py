...
#%%     
        ...
        # status_empty_data=pd.read_sql(f"select * from NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS where S3_Upload_Status is Null and Runtime >'2025-06-06 18:17:03' order by File_Id asc;", con=engine)
        status_empty_data=pd.read_sql(f"select * from NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS where Runtime >= '2025-06-06 12:30:00' order by File_Id asc;", con=engine)
        ...
        status_empty_data=status_empty_data[~status_empty_data['Links'].str.contains('.zip')].reset_index(drop=True)
        status_empty_data=status_empty_data[status_empty_data['Links']!='-'].reset_index(drop=True)
        status_empty_data = status_empty_data[:1500]
