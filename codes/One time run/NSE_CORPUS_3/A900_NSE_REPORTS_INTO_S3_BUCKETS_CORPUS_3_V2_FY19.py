...
#%%     
        ...
        # status_empty_data=pd.read_sql(f"select * from NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS where S3_Upload_Status is null and Relevant_Quarter in ('{req_quarter}') and File_Id>={min_file_id} and File_Id<={max_file_id} order by Company_Name", con=engine)
        status_empty_data=pd.read_sql(f"select * from NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS where S3_Upload_Status is null and S3_Upload_Comments is not null and Runtime<'2025-03-05 18:17:03'  and Relevant_Quarter in ('Q4 FY19','Q3 FY19','Q2 FY19','Q1 FY19') order by File_Id asc;", con=engine)
        # status_empty_data=pd.read_sql(f"select * from NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS where S3_Upload_Status is null and Relevant_Quarter in ('{req_quarter}')  order by Company_Name", con=engine)
        ...
        for x in range(len(status_empty_data)):
            ...
            try:
                if status_val=='Done':
                    query=f"Update NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS set S3_Upload_Status='Done',S3_Upload_Comments=NULL where File_Name='{file_name}'"
                    connection.execute(query)
                    time.sleep(2)
                    print('Done updates')
                else:
                    query=f"Update NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS set S3_Upload_Comments='{status_val}' where File_Name='{file_name}'"
                    connection.execute(query)
                    time.sleep(2)
                    print(status_val)
            except:
