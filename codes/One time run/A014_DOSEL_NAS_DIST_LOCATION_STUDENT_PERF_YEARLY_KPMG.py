...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try :
        ...
        def get_data(driver,hover_val):
            for i in range(3):
                time.sleep(2)
                element_to_hover_over = driver.find_element("xpath", '//*[@id="perAccordioncollapseTwo"]/div/div[1]/div[1]/div')
                time.sleep(2)
                ActionChains(driver).move_to_element_with_offset(element_to_hover_over, hover_val[0], 0).perform()
                time.sleep(2)
            print('language---->',hover_val[0])
            for i in range(3):
                element_to_hover_over = driver.find_element("xpath", '//*[@id="perAccordioncollapseTwo"]/div/div[2]/div[2]/div')
                time.sleep(5)
                ActionChains(driver).move_to_element_with_offset(element_to_hover_over, hover_val[1], 0).perform()
                html_content = driver.page_source
            for i in range(3):
                element_to_hover_over = driver.find_element("xpath", '//*[@id="perAccordioncollapseTwo"]/div/div[3]/div[1]/div')
                time.sleep(5)
                    ...
                    if district.has_attr('onclick'):
                        ...
                        if loop==1:
                            try:
                                ...
                                time.sleep(2)
                                button = driver.find_element(By.XPATH, '//h2[@id="perAccordionTwo"]//button')
                                button.click()
                                loop+=1
                            except:
                                time.sleep(1)
                        ...
                        try:
                            ...
                            print(df)
                            df.to_sql(name='DOSEL_NAS_DIST_LOCATION_STUDENT_PERF_YEARLY_KPMG',con=engine,if_exists='append',index=False)
