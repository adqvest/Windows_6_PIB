# -*- coding: utf-8 -*-
"""
Created on Mon Dec 20 14:52:25 2021
...
#%%
#        i = 0
        query = 'Select count(Distinct Relevant_Date) as Unit,State from AdqvestDB.SCHOOL_ENROLMENT_BY_CATEGORY_YEARLY_DATA_Temp_Abhi group by State;'       
        restates = pd.read_sql(query,con=engine)
        restates = restates[restates['Unit'] <8 ]

        query = 'Select Timeperiod,State from AdqvestDB.SCHOOL_ENROLMENT_BY_CATEGORY_YEARLY_DATA_Temp_Abhi group by State,Timeperiod;'       
        restatestp = pd.read_sql(query,con=engine)
        restatestp = restatestp[restatestp['State'].isin(restates['State'].to_list())]
        r = requests.Session()
...
#            print(result)
        ...
        states = [x for x in states if x['state_name'] in restates['State'].to_list()]
        #%%
        for st in states:
            print(st)
...
#                print(result)
            ...
            checker = restatestp[restatestp['State']==st_name].iloc[:,0].to_list()
            years = [x for x in years if x not in checker]
...
#                data = {"mapId":"96","dependencyValue":"{\"year\":\""+str(yrs)+"\",\"state\":\""+str(st_code)+"\",\"dist\":\""+str(dt_code)+"\",\"block\":\""+str('none')+"\"}","isDependency":"Y","paramName":"civilian","paramValue":"","schemaName":"national","reportType":"T"}
                data = {"dependencyValue":"{\"year\":\""+str(yrs)+"\",\"state\":\""+str(st_code)+"\",\"dist\":\""+str('all')+"\",\"block\":\""+str('none')+"\"}",
                        "isDependency": "Y",
                        "mapId": "96",
                        "paramName": "civilian",
                        "paramValue": "",
...
#                        output = pd.concat([output,df])
                        ...
                        location = df.iloc[df[df.iloc[:,0].str.lower().str.contains("location")].index[0]][0]
...
#                    print(output.head())
                    ...
                    output.to_sql("SCHOOL_ENROLMENT_BY_CATEGORY_YEARLY_DATA_Temp_Abhi", index=False, if_exists='append', con=engine)
                    print("UPLOADED -->",st_name)
                except:
                    k+=1
                    if k>10:
