...
def getApprovedDate(row):
              
  if row['Approved_On'] != None:
      return row['Approved_On']
  else:
      try:
          d = pd.read_html(row['code_url'])

          req = [i for i in d if 'Approval Date' in i.columns][0]
          return parse(req['Approval Date'][0]).date()
      except:
          return None
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'HARYANA_RERA_WEEKLY_DATA_ONE_TIME'
    ...
    try :
        ...
        # query = 'Select max(Relevant_Date) as RD from AdqvestDB.HARYANA_RERA_WEEKLY_DATA'
        # max_date = pd.read_sql(query ,con=engine)['RD'][0]

        # if (today.date()-max_date).days >= 7:
        overall_df = pd.DataFrame()
        for e in ['1','2']:
          a = pd.read_html("https://haryanarera.gov.in/admincontrol/registered_projects/" + e)
          r = requests.get("https://haryanarera.gov.in/admincontrol/registered_projects/" + e)
    ...
    #                        merge_df.to_csv(r"C:\Users\Administrator\AdQvestDir\codes\One time run\CHECK.csv")
          ...
          output_df['Relevant_Date'] = pd.to_datetime(today.strftime("%Y-%m-%d"))
          ...
          #final_required['Plot_Area_In_Sq_Meters'] = final_required['Plot_Area_In_Sq_Meters'].fillna(0)
          #final_required['Total_Plots_In_Project'] = final_required['Total_Plots_In_Project'].fillna(0)
          #final_required['Cost_Of_Land_In_Lakhs'] = final_required['Cost_Of_Land_In_Lakhs'].fillna(0)
          #final_required['Estimated_Cost_Of_Construction_Of_Apartments_In_Lakhs'] = final_required['Estimated_Cost_Of_Construction_Of_Apartments_In_Lakhs'].fillna(0)
          #final_required['Estimated_Cost_Of_Infrastructure_In_Lakhs'] = final_required['Estimated_Cost_Of_Infrastructure_In_Lakhs'].fillna(0)
          final_required['Booked_Or_Sold'] = final_required['Booked_Or_Sold'].fillna(0)
          final_required['Total_Plot_Or_Aparment_In_Project'] = final_required['Total_Plot_Or_Aparment_In_Project'].fillna(0)
          final_required['Towers_Under_Construction_For_Booked_Apartments'] = final_required['Towers_Under_Construction_For_Booked_Apartments'].fillna(0)
          final_required['Yet_To_Be_Booked'] = final_required['Yet_To_Be_Booked'].fillna(0)

          #final_required = final_required.astype({"Plot_Area_In_Sq_Meters":"int","Total_Plots_In_Project":"int","Cost_Of_Land_In_Lakhs":"int","Estimated_Cost_Of_Construction_Of_Apartments_In_Lakhs":"int","Estimated_Cost_Of_Infrastructure_In_Lakhs":"int","Other_Costs_In_Lakhs":"int"})
          final_required = final_required.astype({"Booked_Or_Sold":"int","Total_Plot_Or_Aparment_In_Project":"int","Towers_Under_Construction_For_Booked_Apartments":"int","Yet_To_Be_Booked":"int"})

          final_required = final_required.astype({"Plot_Area_In_Sq_Meters":"float","Total_Plots_In_Project":"float","Cost_Of_Land_In_Lakhs":"float","Estimated_Cost_Of_Construction_Of_Apartments_In_Lakhs":"float", "Estimated_Cost_Of_Infrastructure_In_Lakhs":"float","Other_Costs_In_Lakhs":"float","Carpet_Area_In_Sq_Meters":"float"})

          final_required.insert(1,'Approved_On',None)

          final_required['Approved_On'] = [x.split('DATED')[-1].replace(':','').strip() if 'DATED' in x.upper() else None for x in final_required['Registration_Certificate_Number']]

          final_required['Approved_On'] = [None if x== None else x.replace('.208','.2018')  for x in final_required['Approved_On']]

          final_required['Approved_On'] = [None if x== None else parse(x).date()  for x in final_required['Approved_On']]

          final_required['Approved_On'] = final_required.apply(lambda row: getApprovedDate(row), axis = 1)  

          # print(final_required)
          # print(final_required.dtypes)

          overall_df = pd.concat([overall_df,final_required])

        overall_df.to_sql(name = "HARYANA_RERA_WEEKLY_DATA_ONE_TIME",if_exists = 'append',index = False,con =  engine)
