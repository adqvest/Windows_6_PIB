# -*- coding: utf-8 -*-
"""
Created on Fri Dec  4 14:31:02 2020
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'CAG_SCHOOLS_VILLAGE_LEVEL_DATA_UPDATES'
...
#        %%
       ...
       'Content-Type': 'application/json;charset=UTF-8'
        ...
        df1 = pd.read_sql("Select * from CAG_SCHOOLS_LEVEL2",con = engine)

        df1 = df1[df1['Percent']<=20]
        df1 = df1.sort_values("State")
        all_states=  list(df1['State'].unique())

        st_1 = list(df1[df1['State']==all_states[0]]['District'].unique())
        st_2 = list(df1[df1['State']==all_states[1]]['District'].unique())
        st_3 = list(df1[df1['State']==all_states[2]]['District'].unique())
        st_4 = list(df1[df1['State']==all_states[3]]['District'].unique())

        url1 = "https://src.udiseplus.gov.in/locateSchool/state/"
        url2 = "https://src.udiseplus.gov.in/locateSchool/getDistrict"
        url3 = "https://src.udiseplus.gov.in/locateSchool/getBlock?"
        ...
        for vals in reqd1:
            #for st in reqd1:
            data1 = {'stateId':vals['stateId']}
            r = requests.post(url2,data=data1)
            no_of_ping += 1
            res = r.text.strip('][').split(', ')
            src = u"[%s]" % res[0]
        ...
        for ents in reqd2:
            ...
            for values in ents:
                try:
                    ...
                    main_url = "https://src.udiseplus.gov.in/locateSchool/getBlock?districtId="+str(values['districtId'])+"&yearId="+str(values['yearId'])+""
                    print(main_url)
                    time.sleep(1)
...
#                    no_of_ping += 1
        ...
        for vals in final:
            print(vals)
            time.sleep(1)
            result_url = "https://src.udiseplus.gov.in/locateSchool/searchSchool"
...
#                       columns =['School_Name', 'Village_Name','District_Name','Block_Name','State_Name','Electricity_Availability','Total_Students'])
                output = pd.DataFrame({'School_Name':school_name, 'Village_Name':village_name,'District_Name':district_name,'Block_Name':block_name,'State_Name':state_name,'Pincode':avail,"UDISE_Code":udise_code}, index=[0])
        ...
        #        output = pd.concat([output,df])
                ...
                output = output[['School_Name',"UDISE_Code",'State_Name', 'District_Name', 'Block_Name','Village_Name', 'Electricity_Availability','Total_Students',"Relevant_Date","Runtime"]]

                print(output.head())
                print(len(output))
                output.to_sql(name='CAG_SCHOOLS_VILLAGE_LEVEL_UDISE_CODE',con=engine,if_exists='append',index=False)
#
                print('TRY BLOCK UPLOADED')
                del output
#                except:
#                    print('''
