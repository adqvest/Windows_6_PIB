...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        def read_file(type_,state,rto,rel_date,overall = False):
            vahan_df = pd.read_excel(os.listdir()[0], engine='openpyxl')
        ...
        def main_function():
            ...
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_FINAL where Relevant_Date>='" + date_vahan + "'"
            connection.execute(query)
            connection.execute('commit')
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2')"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [11]):
                ...
                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_Temp_Data2 select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_FINAL limit 0"
                connection.execute(query)
                connection.execute('commit')
