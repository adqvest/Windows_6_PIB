...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'VAHAN_TABULAR_SUMMARY'
    ...
    try:
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        driver.find_element_by_xpath("//div[@id='j_idt31']//div[@class = 'ui-selectonemenu-trigger ui-state-default ui-corner-right']").click()
        ...
        def get_dates(cond):
            ...
            query = "select distinct Relevant_Date FROM VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA where Relevant_Date >= '2018-01-01' "
            maker_vs_fuel_db_data = pd.read_sql(query, engine)

            query = "select distinct Relevant_Date FROM VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA where Relevant_Date >= '2018-01-01' "
            maker_vs_cat_db_data = pd.read_sql(query, engine)


            distinct_date = set(maker_vs_fuel_db_data['Relevant_Date']).intersection(set(maker_vs_cat_db_data['Relevant_Date']))

            distinct_date = list(distinct_date)

            query = "select min(Relevant_Date) as Min FROM VAHAN_TABULAR_SUMMARY_STATE_DONE"
            ...
            if(cond):
                ...
                dates_df = dates_df[:3] #change this later
        ...
        def data_check():
            ...
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,RTO_Code,State,Relevant_Date from AdqvestDB.VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA_Temp_Data  group by RTO_Office_Raw,Relevant_Date",engine)
            ...
            for date in rel_date:
                ...
                error_df_1 = pd.concat([error_df_1,rel_date_df])

            error_df_1 = error_df_1[["RTO_Office_Raw","RTO_Code","State","Relevant_Date"]]

            error_df_2 = pd.DataFrame()
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data  group by RTO_Office_Raw,Relevant_Date",engine)
            rel_date = data_df["Relevant_Date"].unique().tolist()
            for date in rel_date:
                rel_date_df = data_df[data_df["Relevant_Date"] == date]
                rel_date_df = rel_date_df.nlargest(10,"RTO")
                ...
                error_df_2 = pd.concat([error_df_2,rel_date_df])
            error_df_2 = error_df_2[["RTO_Office_Raw","RTO_Code","State","Relevant_Date"]]


            error_df_3 = pd.DataFrame()
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data  group by RTO_Office_Raw,Relevant_Date",engine)
            rel_date = data_df["Relevant_Date"].unique().tolist()
            for date in rel_date:
                rel_date_df = data_df[data_df["Relevant_Date"] == date]
                rel_date_df = rel_date_df.nlargest(10,"RTO")
            ...
            error_df = pd.concat([error_df_1,error_df_2,error_df_3])
            ...
            if(error_df.empty):
                ...
                for index,i in error_df.iterrows():
                    connection.execute("update VAHAN_TABULAR_SUMMARY_STATE_DONE set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                    connection.execute("commit")

                    connection.execute("update VAHAN_TABULAR_SUMMARY_STATE_DONE set State_Status = Null where State = '" +i["State"] +"'")
                    connection.execute("commit")


                    connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")

                    connection.execute("delete from VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")

                    connection.execute("delete from VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA_Temp_Data where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")
                raise Exception
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    download_file_path = r"C:\Users\Administrator\Junk_Vahan"

                    os.chdir(r"C:\Users\Administrator\Junk_Vahan")
                    ...
                    for row in set(remove_state['State'].to_list()):
                        print(row)
                        if row=='Jammu & Kashmir':
                            row='Jammu and Kashmir'
                        ...
                        for x in rto_info.keys():
                            ...
                            for i in range(len(dates_df)):
                                ...
                                if('no records found' in str(soup).lower()):
                                    ...
                                    time.sleep(0.5)
                                    vahan_df_maker_vs_cat = read_file(type_ = "Maker_Vs_Cat",state = state,rto = rto,rel_date = dates_df["Relevant_Date"][i])
                                ...
                                if('no records found' in str(soup).lower()):
                                    ...
                                    driver = download_file(driver)
                                    time.sleep(0.5)
                                    vahan_df_maker_vs_fuel = read_file(type_ = "Maker_Vs_Fuel",state = state,rto = rto,rel_date = dates_df["Relevant_Date"][i])
                                ...
                                if('no records found' in str(soup).lower()):
                                    ...
                                    driver = download_file(driver)
                                    time.sleep(0.5)
                                    vahan_df_fuel_vs_cat = read_file(type_ = "Fuel_Vs_Cat",state = state,rto = rto,rel_date = dates_df["Relevant_Date"][i])
                                    print(vahan_df_fuel_vs_cat)
                                    vahan_fuel_cat_rto_level = pd.concat([vahan_fuel_cat_rto_level,vahan_df_fuel_vs_cat])

                            if(vahan_maker_cat_rto_level.empty == False):
                                vahan_maker_cat_rto_level = vahan_maker_cat_rto_level[vahan_maker_cat_rto_level["Total"]!=0]
                            vahan_maker_cat_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data", con=engine, index=False, if_exists='append')
                            if(vahan_fuel_maker_rto_level.empty == False):
                                vahan_fuel_maker_rto_level = vahan_fuel_maker_rto_level[vahan_fuel_maker_rto_level["Total"]!=0]
                            vahan_fuel_maker_rto_level.to_sql(name = "VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data", con=engine, index=False, if_exists='append')
                            if(vahan_fuel_cat_rto_level.empty == False):
                                vahan_fuel_cat_rto_level = vahan_fuel_cat_rto_level[vahan_fuel_cat_rto_level["Total"]!=0]
                            vahan_fuel_cat_rto_level.to_sql(name = "VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA_Temp_Data", con=engine, index=False, if_exists='append')

                            connection.execute("update VAHAN_TABULAR_SUMMARY_STATE_DONE set RTO_Status = 'Done' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")
                ...
                try:
                    ...
                    pass
            data_check()
            query = "delete from VAHAN_TABULAR_SUMMARY_STATE_DONE"
            connection.execute(query)
            connection.execute('commit')

            #maker vs category
            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data", con=engine)
            ...
            # query = "delete from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA where Relevant_Date>='" + date_vahan + "'"
            # connection.execute(query)
            # connection.execute('commit')
            # try:
            #     query="ALTER TABLE AdqvestDB.VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA DELETE WHERE Relevant_Date>='" + date_vahan + "'"
            ...
            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data where Relevant_Date>='" + date_vahan + "'", con=engine)
                df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA', con=engine, index=False, if_exists='append')
                #ClickHouse
                try:
                ...
                print('ERROR 3')
                raise Exception('error in VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA')


            # query = "drop table VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data"
            # connection.execute(query)
            # connection.execute('commit')
            ...
            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data", con=engine)
            date_vahan = date_vahan['Date'][0]
            date_vahan = datetime.date(date_vahan.year, date_vahan.month, 1)
            date_vahan = date_vahan.strftime('%Y-%m-%d')

            # query = "delete from VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA where Relevant_Date>='" + date_vahan + "'"
            # connection.execute(query)
            # connection.execute('commit')

            # try:
            #     query="ALTER TABLE AdqvestDB.VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA DELETE WHERE Relevant_Date>='" + date_vahan + "'"
            #     client.execute(query)
            # except:
            #     print('ClickHouse delete')
            #     pass
            ...
            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data where Relevant_Date>='" + date_vahan + "'", con=engine)
                df.to_sql(name='VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA', con=engine, index=False, if_exists='append')
                try:
                    client.execute("INSERT INTO VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA VALUES", df.values.tolist())
                except:
                    pass
            except:
                print('ERROR 3')
                raise Exception('error in VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA')


            # query = "drop table VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data"
            # connection.execute(query)
            # connection.execute('commit')

            #fuel vs category

            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA_Temp_Data", con=engine)
            date_vahan = date_vahan['Date'][0]
            date_vahan = datetime.date(date_vahan.year, date_vahan.month, 1)
            date_vahan = date_vahan.strftime('%Y-%m-%d')

            # query = "delete from VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA where Relevant_Date>='" + date_vahan + "'"
            # connection.execute(query)
            # connection.execute('commit')
            ...
            try:
                df = pd.read_sql("select * from VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA_Temp_Data where Relevant_Date>='" + date_vahan + "'", con=engine)
                df.to_sql(name='VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA', con=engine, index=False, if_exists='append')
            except:
                print('ERROR 3')
                raise Exception('error in VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA')


            # query = "drop table VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA_Temp_Data"
            # connection.execute(query)
            # connection.execute('commit')
            ...
            vahan_table_list = ["VAHAN_MAKER_WISE_RTO_LEVEL_MONTHLY_PRORATA_DATA","VAHAN_MAKER_WISE_INDIA_LEVEL_DAILY_DATA","VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA","VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA","VAHAN_MAKER_VS_CATEGORY_OVERALL_DAILY_DATA","VAHAN_MAKER_VS_FUEL_OVERALL_DAILY_DATA"]
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data', 'VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data','VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA_Temp_Data')"
        ...
        def rto_count():
            query = "select distinct Relevant_Date from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA where Relevant_Date between '2021-10-31' and '2022-01-31'"

            dates = pd.read_sql(query, con=engine)

            dates = dates['Relevant_Date'].tolist()
            rto_intersection = {}
            for i, d in enumerate(dates):
                print(d)
                d = d.strftime('%Y-%m-%d')

                query = "select distinct RTO_Code from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA  where Relevant_Date='" + d + "'"
                rto = pd.read_sql(query, con=engine)
                rto['RTO_Code'] = rto['RTO_Code'].str.strip().str.upper()

                if(i==0):
                    rto_intersection = set(rto['RTO_Code'])
                else:
                    new = set(rto['RTO_Code'])
                    rto_intersection = rto_intersection.intersection(new)

            rto_table = pd.DataFrame(list(rto_intersection), columns=['RTO_Code'])
            new_df = pd.read_sql("select  RTO_Office_Raw, RTO_Code from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA where Relevant_Date='2022-01-31'", con=engine)
            new_df = new_df.drop_duplicates()

            rto_table = rto_table.merge(new_df, on='RTO_Code', how='left')
            if(rto_table['RTO_Office_Raw'].isnull().any()):
                raise Exception("RTO_Office_Raw contains null")

            rto_table['Relevant_Date'] = today.date()
            rto_table['Runtime'] = pd.to_datetime(today.strftime("%Y-%m-%d %H:%M:%S"))

            query = "delete from VAHAN_TABULAR_SUMMARY_RTOS"
            connection.execute(query)
            connection.execute('commit')

            rto_table.to_sql(name = "VAHAN_TABULAR_SUMMARY_RTOS",con = engine,index = False,if_exists = 'append')
            print("rto list uploaded")
        ...
        def get_overall_data():
            ...
            while True:
                try:
                    ...
                    connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_OVERALL_DAILY_DATA")
                    connection.execute("commit")
                    connection.execute("delete from VAHAN_MAKER_VS_FUEL_OVERALL_DAILY_DATA")
                    connection.execute("commit")
                    connection.execute("delete from VAHAN_FUEL_VS_CATEGORY_OVERALL_DAILY_DATA")
                    connection.execute("commit")

                    #########################################
                    start_date = yesterday.date() - relativedelta(months=+16)

                    end_date =   yesterday.date()

                    date_list = []
                    while start_date <= end_date:
                        if(calendar.monthrange(start_date.year,start_date.month)[1] == start_date.day):
                    ...
                    dates_df["Year"] = dates_df["Relevant_Date"].apply(lambda x: datetime.datetime.strftime(x,"%Y"))
                    dates_df.reset_index(drop = True,inplace = True)
                    dates_df = dates_df[1:len(dates_df)]
                    dates_df.reset_index(drop = True,inplace = True)
                    ...
                    for i in range(len(dates_df)):
                        ...
                        if('no records found' in str(soup).lower()):
                            ...
                            driver = download_file(driver)
                            time.sleep(0.5)
                            vahan_df_maker_vs_cat = read_file(type_ = "Maker_Vs_Cat",state = "",rto = "",rel_date = dates_df["Relevant_Date"][i],overall = True)
                            print(vahan_df_maker_vs_cat)
                            vahan_maker_cat_rto_level = pd.concat([vahan_maker_cat_rto_level,vahan_df_maker_vs_cat])
                        ...
                        if('no records found' in str(soup).lower()):
                            ...
                            driver = download_file(driver)
                            time.sleep(0.5)
                            vahan_df_maker_vs_fuel = read_file(type_ = "Maker_Vs_Fuel",state = "",rto = "",rel_date = dates_df["Relevant_Date"][i],overall = True)
                            print(vahan_df_maker_vs_fuel)
                            vahan_fuel_maker_rto_level = pd.concat([vahan_fuel_maker_rto_level,vahan_df_maker_vs_fuel])
                        driver.refresh()
                        ...
                        if('no records found' in str(soup).lower()):
                            ...
                            driver = download_file(driver)
                            time.sleep(0.5)
                            vahan_df_fuel_vs_cat = read_file(type_ = "Fuel_Vs_Cat",state = "",rto = "",rel_date = dates_df["Relevant_Date"][i],overall = True)
                            print(vahan_df_fuel_vs_cat)
                            vahan_fuel_cat_rto_level = pd.concat([vahan_fuel_cat_rto_level,vahan_df_fuel_vs_cat])
                        driver.refresh()
                    if(vahan_maker_cat_rto_level.empty == False):
                    ...
                    vahan_maker_cat_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_OVERALL_DAILY_DATA", con=engine, index=False, if_exists='append')
                    if(vahan_fuel_maker_rto_level.empty == False):
                        vahan_fuel_maker_rto_level = vahan_fuel_maker_rto_level[vahan_fuel_maker_rto_level["Total"]!=0]

                    vahan_fuel_maker_rto_level.to_sql(name = "VAHAN_MAKER_VS_FUEL_OVERALL_DAILY_DATA", con=engine, index=False, if_exists='append')
                    if(vahan_fuel_cat_rto_level.empty == False):
                        vahan_fuel_cat_rto_level = vahan_fuel_cat_rto_level[vahan_fuel_cat_rto_level["Total"]!=0]

                    vahan_fuel_cat_rto_level.to_sql(name = "VAHAN_FUEL_VS_CATEGORY_OVERALL_DAILY_DATA", con=engine, index=False, if_exists='append')
        ...
        #getting list of rtos
        #rto_count()
        #getting overall data
        #get_overall_data()

        query = "SELECT * FROM VAHAN_TABULAR_SUMMARY_STATE_DONE"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [1,8,18,25]):

                print('B')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA limit 0"
                connection.execute(query)
                connection.execute('commit')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data select * from AdqvestDB.VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA limit 0"
                connection.execute(query)
                connection.execute('commit')

                query = "create table AdqvestDB.VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA_Temp_Data select * from AdqvestDB.VAHAN_FUEL_VS_CATEGORY_RTO_LEVEL_DATA limit 0"
                connection.execute(query)
                connection.execute('commit')
