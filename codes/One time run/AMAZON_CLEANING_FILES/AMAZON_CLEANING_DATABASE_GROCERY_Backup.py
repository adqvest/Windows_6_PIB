...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try :
        ...
        tables = ['AMAZON_GROCERY_AND_GOURMET_FOODS']
        for table_name in tables:
            table = 'CLEAN_DATA_FOR_MANNA'
            date = pd.read_sql("select max(Relevant_Date) as Relevant_Date from AmazonDB."+table,con=engine2)['Relevant_Date'][0]
            ...
            while start_date <= end_date :
                ...
                if df.empty:
                    try:
                        start_date = pd.read_sql("Select min(Relevant_Date) as RD from AdqvestDB.AMAZON_GROCERY_AND_GOURMET_FOODS where Relevant_Date > '"+str(start_date)+"'",con=engine1)['RD'][0]
                        df = pd.read_sql("Select * from AdqvestDB."+table_name+" where Relevant_Date = '"+str(start_date)+"'",con=engine1)
                        if df.empty:
...
#                    break
                ...
                try:
                    df.to_sql(name="CLEAN_DATA_FOR_MANNA",con=engine2,if_exists='append',index=False)
                except:
                    df.to_sql(name="CLEAN_DATA_FOR_MANNA",con=engine2,if_exists='append',index=False)
