    ...
    try :
        ...
        for table_name in tables:
            ...
            while start_date <= end_date :
                ...
                if df.empty:
                    try:
                        start_date = pd.read_sql("Select min(Relevant_Date) as RD from AdqvestDB.AMAZON_BEAUTY where Relevant_Date > '"+str(start_date)+"'",con=engine1)['RD'][0]
                        df = pd.read_sql("Select * from AdqvestDB."+table_name+" where Relevant_Date = '"+str(start_date)+"'",con=engine1)
                        if df.empty:
                ...
                try:
                    if "SHOES_AND_HANDBAGS" in table_name:
                        print("Shoes and Handbags")
                        df = only_qty(df.copy())
                    else:
...
#                    break
                    ...
                    raise Exception("Data Pipeline Wrong")
                ...
                if len(value) == 3:
                    ...
                    sku1 = ['SKU_0']
#                    sku1.append(str(sku1[0][:-1])+str(int(sku1[0][-1])))
                    sku1.append(str(sku1[0][:-1])+str(int(sku1[0][-1])+1))
                    sku1.append(str(sku1[0][:-1])+str(int(sku1[0][-1])+2))

                    df[str(sku1[0][:-1])+str(int(sku1[0][-1]))] = np.nan
                    df[str(sku1[0][:-1])+str(int(sku1[0][-1])+1)] = np.nan
                    df[str(sku1[0][:-1])+str(int(sku1[0][-1])+2)] = np.nan
                ...
                if len(value) == 3:
                    ...
                    sku2 = ['SKU_Units0']
#                    sku2.append(str(sku2[0][:-1])+str(int(sku2[0][-1])))
                    sku2.append(str(sku2[0][:-1])+str(int(sku2[0][-1])+1))
                    sku2.append(str(sku2[0][:-1])+str(int(sku2[0][-1])+2))

                    df[str(sku2[0][:-1])+str(int(sku2[0][-1]))] = np.nan
                    df[str(sku2[0][:-1])+str(int(sku2[0][-1])+1)] = np.nan
                    df[str(sku2[0][:-1])+str(int(sku2[0][-1])+2)] = np.nan
#
#                if 'SHOES_AND_HANDBAGS' in table_name:
#                    print("SKU Unavailable for Shoes & Hangbags")
#                else:
                ...
                for sku in sku_float:
                    try:
                        ...
                        df[sku] =  pd.to_numeric(df[sku],errors='ignore')
