...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try :
        ...
        tables = ["AMAZON_HOME_AND_KITCHEN"]
        for table_name in tables:
            ...
            while start_date <= end_date :
                ...
                if df.empty:
                    try:
                        start_date = pd.read_sql("Select min(Relevant_Date) as RD from AdqvestDB.AMAZON_HOME_AND_KITCHEN where Relevant_Date > '"+str(start_date)+"'",con=engine1)['RD'][0]
                        df = pd.read_sql("Select * from AdqvestDB."+table_name+" where Relevant_Date = '"+str(start_date)+"'",con=engine1)
                        if df.empty:
