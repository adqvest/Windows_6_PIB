...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'CMOTS_NSE_CORPUS_TO_BE_CHUNKED'

        not_tried_files=pd.read_sql("select File_Name  from  AdqvestDB.CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS where  S3_Upload_Status = 'Done' and  Chunking_Status is null and Chunking_Comments is  null and Relevant_Date<'2025-11-25';",con=engine)
        ...
        response = [ x for x in response if 'CMOTS_NSE_CORPUS_TO_BE_CHUNKED/' in x]
        print(len(response))
            ...
            try:
                try:
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = 'File Not Available' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    elif 'Failed to load document (PDFium: Incorrect password error)' in str(e):
                        query = f"UPDATE CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = 'Password Issue' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)  
                    elif 'Failed to load page' in str(e):
                        query = f"UPDATE CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = 'Corrupt Document' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)       
                    else:
                        query = f"UPDATE CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    # connection.commit()
                    time.sleep(2)
                ...
                if is_whole_doc_scanned:
                    query = f"UPDATE CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = 'Scanned Document' WHERE File_Name = '{file_name}'"
                    print(query)
                    connection.execute(query)
                    continue
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if int(relevant_year[5:])>=18 and int(relevant_year[5:])<30:
                        ...
                        if (len(extracted_chunks)>0):
                            ...
                            connection = engine.connect()
                            query='Update CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS set Chunking_Status="Done" where Company_Name="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                        ...
                        query='Update CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS set Chunking_Status="Not Relevant" where Company_Name="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                        connection = engine.connect()
                        connection.execute(text(query))
                        # connection.commit()
                        time.sleep(2)
                ...
                # query=f"Update CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '{file_name}'"
                # print(query)
                # try:
                #     connection.execute(query)
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.CMOTS_NSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS ORDER BY File_ID ASC;"
