...
def run_program(run_by = 'Adqvest_Bot', py_file_name = None):
    ...
    try :
        ...
        url='https://aishe.gov.in/aishe/gotoAisheReports'
        r = requests.get(url)
        robot.add_link(url)
        soup = BeautifulSoup(r.content,'lxml')
        links=soup.find_all('tr')
        date_link={'20'+i.find_all('td')[1].text.strip().replace(' - English','').split('-')[1]:'https://aishe.gov.in/'+i.find('a',href=True)['href'] for i in links if i.find('a',href=True)!=None}
       ...
       #%%
        ...
        if len(date_link)>0:
            for current_date,link in date_link.items():
                print(link)
                print(current_date)
                file_name=f"AISHEA_REPORTS_{str(current_date)}.pdf"
