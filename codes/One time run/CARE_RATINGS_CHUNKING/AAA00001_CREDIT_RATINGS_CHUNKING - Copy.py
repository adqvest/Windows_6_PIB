...
doc_links = client.execute('''SELECT distinct document_link FROM thurro_pdf_documents_vector_db_care_ratings;''')
doc_links = [link for (link,) in doc_links]
print(len(doc_links))

doc_ids = client.execute('''SELECT distinct document_id FROM thurro_pdf_documents_vector_db_care_ratings;''')
...
for i in range(len(care_ratings)):
    ...
    if(link not in doc_links):
        ...
        max_doc_ids = client.execute('''SELECT max(document_id) as max_id FROM thurro_pdf_documents_vector_db_care_ratings;''')
