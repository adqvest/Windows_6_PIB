...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 10 12:42:43 2025
...
folder_name = 'CARE_Ratings/'

insert_query = "insert into CARE_RATINGS_DAILY_FILES_LINKS_CHUNKING select *, NULL as Chunked from CARE_RATINGS_DAILY_FILES_LINKS where Links not in (select distinct Links from CARE_RATINGS_DAILY_FILES_LINKS_CHUNKING) order by Relevant_Date;"
engine.execute(insert_query)

query = "select * from CARE_RATINGS_DAILY_FILES_LINKS_CHUNKING where Chunked is null"
care_ratings = pd.read_sql(query,con=engine)



doc_links = client.execute('''SELECT distinct document_link FROM thurro_pdf_documents_vector_db_care_ratings_smallcase;''')
doc_links = [link for (link,) in doc_links]
print(len(doc_links))

doc_ids = client.execute('''SELECT distinct document_id FROM thurro_pdf_documents_vector_db_care_ratings_smallcase;''')
...
for i in range(len(care_ratings)):
    ...
    link = care_ratings['Links'][i]
    file_name = care_ratings['File_Name'][i]
    ...
    if(link not in doc_links):
        ...
        max_doc_ids = client.execute('''SELECT max(document_id) as max_id FROM thurro_pdf_documents_vector_db_care_ratings_smallcase;''')
        ...
        try:
            ...
            if('NoSuchKey' in str(e)):
                try:
                    ...
                    files_content, file_name = read_all_file_content(folder_name, file_name)
                    update_query = f"update CARE_RATINGS_DAILY_FILES_LINKS_CHUNKING set Chunked = 'Failed' where Links = '{link}' and Relevant_Date = '{care_ratings['Relevant_Date'][i]}'"
                    print(update_query)
        ...
        try:
            company_name,report_type,relevant_year,file_id,text = extract_text_from_pdf(files_content,file_name_updated,file_name)
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                client.execute(f"INSERT INTO thurro_pdf_documents_vector_db_care_ratings_smallcase (document_id,document_company,document_type,document_year,document_date,document_link,document_content,document_content_modify,embedding) VALUES ('{file_id}','{company_name}','{report_type}','{relevant_year}','{document_date}','{link}','''{chunk}''','''{new_chunk}''',{chunk_embed});") 
                                print('Document chucked and inserted into clickhouse')
                                update_query = f"update CARE_RATINGS_DAILY_FILES_LINKS_CHUNKING set Chunked = 'Yes' where Links = '{link}' and Relevant_Date = '{care_ratings['Relevant_Date'][i]}'"
                                print(update_query)
                                connection.execute(text(update_query))
                                connection.execute("commit")
                                print('sql table updated')
