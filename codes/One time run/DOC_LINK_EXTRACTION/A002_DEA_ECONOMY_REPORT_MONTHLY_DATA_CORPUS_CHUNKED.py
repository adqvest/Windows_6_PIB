"""
Created on Wed Jul  2 02:14:26 2025
...
from  adqvest_chunking_funcs import read_large_file_names_from_s3_folder,read_all_file_names_from_s3_folder,read_all_file_content,delete_file_from_s3

engine = adqvest_db.db_conn()
connection = engine.connect()

from clickhouse_driver import Client
client = Client('ec2-52-27-243-167.us-west-2.compute.amazonaws.com',
                user='default',
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'DEA_ECONOMY_REPORT_MONTHLY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        connection = engine.connect()
        folder_name = 'DEA_MONTHLY_ECONOMY_CORPUS_TO_BE_CHUNKED'
        corpus_table="DEA_ECONOMY_REPORT_MONTHLY_DATA_CORPUS"
        chunking_table="DEA_ECONOMY_REPORT_MONTHLY_DATA_CORPUS_CHUNKED"


        response = read_large_file_names_from_s3_folder(folder_name)
        response = [ x for x in response if f'{folder_name}/' in x]
        print(len(response))

        response = response[1:]
        # response = [x for x in response if re.search(r'Q4_FY25', x)]

        doc_ids = client.execute(f'''SELECT distinct Document_Id FROM {chunking_table};''')
        ...
        for obj in response:
            ...
            try:
                try:
                    ...
                    query=f"Update {corpus_table} set Chunking_Comments='{error_msg}' where File_Name='{file_name}'"
                    print(query)
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if int(relevant_year[5:])>=10 and int(relevant_year[5:])<30:
                        ...
                        if (len(extracted_chunks)>0):
                            ...
                            for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                                ...
                                try:
                                    ...
                                    client.execute(f"INSERT INTO {chunking_table} (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")
                                except:
                            ...
                            connection = engine.connect()
                            query=f"Update {corpus_table} set Chunking_Status='Done' where Source='{company_name}' and File_ID='{str(file_id)}' and Relevant_Quarter='{relevant_year}'"
                        ...
                        # query='Update NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS set Chunking_Status="Not Relevant" where Source="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                        query=f"Update {corpus_table} set Chunking_Status='Not Relevant' where Source='{company_name}' and File_ID='{str(file_id)}' and Relevant_Quarter='{relevant_year}'"
                    ...
                    connection = engine.connect()
                    query=f"Update {corpus_table} set Chunking_Status='Done' where Source='{company_name}' and File_ID='{str(file_id)}' and Relevant_Quarter='{relevant_year}'"
                    # query='Update NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS set Chunking_Status="Done" where Source="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                    connection.execute(text(query))
                    # connection.commit()
                    time.sleep(2)
                    print('Done updates')
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.{corpus_table} ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute(f'''SELECT distinct Document_Id as id , Page_Number as pn FROM {chunking_table} where Document_Link = '' ;''')
        print(len(doc))
        ...
        connection = engine.connect()
        query = f"select * from AdqvestDB.{corpus_table} ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute(f'''SELECT distinct Document_Id as id FROM {chunking_table} where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute(f'''SELECT distinct Document_Id as id FROM {chunking_table} where Published_Date ='1970-01-01 00:00:00' ;''')
        print(len(doc))
        ...
        dcomp = client.execute(f'''SELECT distinct Document_Company as dcomp FROM {chunking_table} where Symbol = '';''')
        print(len(dcomp))
        ...
        if len(dcomp) > 0:
            ...
            data = data[data['Source'].isin(company)]
            data.reset_index(drop = True, inplace = True)
            print(len(data))
            
            update_data = []
            for x in range(len(data)): 
                company_name = data["Source"].iloc[x] 
