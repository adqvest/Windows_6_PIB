# -*- coding: utf-8 -*-
"""
Created on Tue May 13 09:15:24 2025
...
host = 'ec2-52-27-243-167.us-west-2.compute.amazonaws.com'
user_name = 'default'
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'NSE_DOC_LINKS_FROM_CORPUS_TO_BE_CHUNKED'
        ...
        response = [ x for x in response if 'NSE_DOC_LINKS_FROM_CORPUS_TO_BE_CHUNKED/' in x]
        print(len(response))

        response = response[1:]
        # response = [x for x in response if re.search(r'Q4_FY25', x)]

        doc_ids = client.execute('''SELECT distinct Document_Id FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CHUNKED;''')
        ...
        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc)>1:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CHUNKED where Published_Date ='1970-01-01 00:00:00' ;''')
        print(len(doc))
        ...
        dcomp = client.execute('''SELECT distinct Document_Company as dcomp FROM NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA_CHUNKED where Symbol = '';''')
        print(len(dcomp))
