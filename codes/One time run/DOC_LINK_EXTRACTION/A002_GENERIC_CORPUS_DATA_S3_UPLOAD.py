...
def run_program(run_by = 'Adqvest_Bot', py_file_name = None):
    ...
    try :
        ...
        #%%
        # lookup_dict={'DEA_ECONOMY_REPORT_MONTHLY_DATA_CORPUS':["DEA_MONTHLY_ECONOMY_CORPUS_TO_BE_CHUNKED/","DEA_MONTHLY_ECONOMY_CORPUS/"]}
        lookup_dict={'NSE_DOC_LINKS_FROM_CORPUS_DAILY_DATA':["NSE_DOC_LINKS_FROM_CORPUS_TO_BE_CHUNKED/","NSE_DOC_LINKS_FROM_CORPUS/"]}
        for Working_table,keys in lookup_dict.items():
            
            query = f"SELECT * FROM {Working_table} where S3_Upload_Status is Null;"
            ...
            for index, row in raw_df.iterrows():
                ...
                if pdf_content:
                    ...
                    connection=engine.connect()
                    connection.execute(f"update {Working_table} set S3_Upload_Status='Failed' where File_Name = '" +str(filename)+"'")
                    connection.execute("commit")
                    print('-' *80)
