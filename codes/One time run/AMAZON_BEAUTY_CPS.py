...
#        df_catg = df_catg[df_catg['Sub_Category_3']=='Talcum Powders']
        ...
        df_1 = pd.DataFrame(df_1, columns=[tuple[0] for tuple in cols])
        print(df_1)
        df_1['Marker'] = '1'
#        a = df_catg.merge(df,on=['Done','Category', 'Sub_Category_1', 'Sub_Category_2','Sub_Category_3', 'Sub_Category_4'],how='inner')
#        df_catg = df_catg[~df_catg.index.isin(df_1.index)]
        a = pd.merge(df_1, df_catg, on=['Category', 'Sub_Category_1', 'Sub_Category_2','Sub_Category_3', 'Sub_Category_4'], how='right')
        df_catg = a[a['Marker'].isnull()]
        ...
        #    catg_df = pd.concat([catg_df,a])
            ...
            main1 = main1[['Pdt', 'Brand', 'Category', 'Sub_Category_1', 'Sub_Category_2','Sub_Category_3', 'Sub_Category_4','Value','Relevant_Month','Combination','Relevant_Date']]
            print(main1.groupby(['Relevant_Month'])['Value'].sum())
            main1['Runtime'] = pd.to_datetime(today.strftime("%Y-%m-%d %H:%M:%S"))

            client.execute("INSERT INTO AdqvestDB.AMAZON_BEAUTY_CPS_SCORES VALUES", main1.values.tolist())
            del r
            del df
            del main1
