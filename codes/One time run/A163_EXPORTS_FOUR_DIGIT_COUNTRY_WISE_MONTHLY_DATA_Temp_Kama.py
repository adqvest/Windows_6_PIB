...
#%%
        ...
        def get_export_hscode():
            ...
            return export_top_500
        def set_null(HSCodes):
            for code in HSCodes:
                query="update EXPORTS_FOUR_DIGIT_COUNTRY_WISE_HSCODE_STATUS set Status='NULL' where HSCode='"+str(code)+"'"
                engine.execute(query)
        # last_date=pd.read_sql("select min(Relevant_Date) as Min from EXPORTS_DATA_HSN_MONTHLY",engine)['Min'][0]
        done_data=pd.read_sql("Select max(Max_Relevant_Date_DB) as MAX from EXPORTS_FOUR_DIGIT_COUNTRY_WISE_HSCODE_STATUS where Status='DONE'",engine)['MAX'][0]


        if str(done_data)<str(prev_month):
            HSCodes=pd.read_sql("Select distinct HSCode as HSCode from EXPORTS_FOUR_DIGIT_COUNTRY_WISE_HSCODE_STATUS where Status ='DONE'",engine)
            HSCodes=HSCodes['HSCode']
            HSCodes=list(HSCodes)
            set_null(HSCodes)
        # date1 = last_date
        date2 = pd.read_sql("select max(Relevant_Date) as Max from EXPORTS_DATA_HSN_MONTHLY",engine)['Max'][0]
        connection.close()
        max_date = []
        max_db_date=pd.to_datetime('2018-02-28').date()
        today=pd.read_sql("select min(Relevant_Date) as Max from EXPORTS_FOUR_DIGIT_COUNTRY_WISE_MONTHLY_DATA_Temp_Kama",engine)['Max'][0]
        ...
        for date1 in all_dates:
            ...
            r = session.get("https://tradestat.commerce.gov.in/meidb/comq.asp?ie=e", verify=False)
            ...
            req = session.post("https://tradestat.commerce.gov.in/meidb/com.asp?ie=e", data=main_data, headers=headers,
                               verify=False)
            ...
            export_top_500=get_export_hscode()
            print(len(export_top_500))
            New_HSCode=list(master.HSCode)
            HSCode=list(export_top_500.HSCode)
            ...
            for code in range(export_top_500.shape[0]):
                data_date=end_date(Latest_Year,Latest_Month)
                print('Checking for date '+str(data_date)+'')
                print('checking for HSCode ---->' +str(export_top_500['HSCode'].iloc[code]))
                data=pd.read_sql("select * from EXPORTS_FOUR_DIGIT_COUNTRY_WISE_MONTHLY_DATA where HSCode='"+str(export_top_500['HSCode'].iloc[code])+"' and Relevant_Date='"+str(data_date)+"'",engine)
                if data.empty:
                    
                
                    connection.close()
                    print(export_top_500['HSCode'].iloc[code])
                    ...
                    try:
                        r=session.get("https://tradestat.commerce.gov.in/meidb/comcntq.asp?ie=e",verify=False)
                    except:
                        r=session.get("https://tradestat.commerce.gov.in/meidb/comcntq.asp?ie=e",verify=False)
                    #            no_of_ping +=1
                    main_soup=BeautifulSoup(r.content)
                    l1=main_soup.findAll("input")
                    main_data={l1[1]['name']:l1[1]['value'],
                    'Mm1':str(Latest_Month),'yy1':str(Latest_Year),'hscode':str(export_top_500['HSCode'].iloc[code]),'sort':'0',l1[3]['name']:l1[3]['value'],
                    l1[6]['name']:l1[6]['value']}
                    try:
                        req=session.post("https://tradestat.commerce.gov.in/meidb/comcnt.asp?ie=e",data=main_data,headers=headers,verify=False)
                    except:
                        req=session.post("https://tradestat.commerce.gov.in/meidb/comcnt.asp?ie=e",data=main_data,headers=headers,verify=False)
                    #            no_of_ping +=1
                    master_soup=BeautifulSoup(req.text,'lxml')
                    ...
                    if flag!=1:
                        ...
                        for yr in [Latest_Year]:
                            ...
                            for mon in [Latest_Month]:
                                ...
                                'Mm1':str(mon),'yy1':str(yr),'hscode':str(export_top_500['HSCode'].iloc[code]),'sort':'0',l1[3]['name']:l1[3]['value'],
                                l1[6]['name']:l1[6]['value']}
                                time.sleep(5)
                                try:
                                ...
                                'Mm1':str(mon),'yy1':str(yr),'hscode':str(export_top_500['HSCode'].iloc[code]),'sort':'0',l1[4]['name']:l1[4]['value'],
                                l1[6]['name']:l1[6]['value']}
                                time.sleep(5)
                                try:
                                ...
                                'Mm1':str(mon),'yy1':str(yr),'hscode':str(export_top_500['HSCode'].iloc[code]),'sort':'0',l1[5]['name']:l1[5]['value'],
                                l1[6]['name']:l1[6]['value']}
                                time.sleep(5)
                                try:
                                ...
                                df_final['HSCode']=str(export_top_500['HSCode'].iloc[code])
                                df_final['Commodity']=export_top_500['Commodity'].iloc[code]
                                ...
                                if (df_final['Monthly_Value_INR_In_Lakhs_Current_Year'] == 0).all():
                                    print('All the value for '+str(export_top_500['HSCode'].iloc[code])+' are zero')
                                    connection.close()
                                else:
                                    ...
                                    connection.execute("Delete from EXPORTS_FOUR_DIGIT_COUNTRY_WISE_MONTHLY_DATA where Relevant_Date='"+str(end_date(int(Latest_Year),int(Latest_Month)))+"' and HSCode='"+str(export_top_500['HSCode'].iloc[code])+"'")
                                    connection.execute('commit')
                                    print('deleted the data for hscode '+str(export_top_500['HSCode'].iloc[code])+' and date -->',str(end_date(int(Latest_Year),int(Latest_Month))))

                                    df_final.to_sql("EXPORTS_FOUR_DIGIT_COUNTRY_WISE_MONTHLY_DATA", if_exists="append", index=False, con=engine)
                                    print(df_final.head(2))

                                    # df_final.to_sql(table_name, if_exists="append", index=False, con=engine)
                                    print('done for:',export_top_500['HSCode'].iloc[code])
                                    print('done for these much HSCode:',count)
                                    time.sleep(3)
                    else:
                        print('No data')
                else:
                    print('Data is already there for HSCode '+str(export_top_500['HSCode'].iloc[code])+' for ' +str(data_date))                    
