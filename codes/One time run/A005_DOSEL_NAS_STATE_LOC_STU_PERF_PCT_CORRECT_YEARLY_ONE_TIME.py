...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'DOSEL_NAS_STATE_LOC_STU_PERF_PCT_CORRECT_YEARLY_ONE_TIME'
    ...
    try :
        ...
        def get_data(driver,hover_val_x,hover_val_y,tag_val):
            for i in range(3):
                time.sleep(2)
                element_to_hover_over = driver.find_element("xpath", '/html/body/section/div[2]/main/div/div[2]/div[3]/div[3]/div/div/div['+str(tag_val)+']/div/div/div[2]/div/div[2]/div/div/div/div[1]/div/div/figure/div')
                time.sleep(2)
            ...
            for i in range(3):
                element_to_hover_over = driver.find_element("xpath", '/html/body/section/div[2]/main/div/div[2]/div[3]/div[3]/div/div/div['+str(tag_val)+']/div/div/div[2]/div/div[2]/div/div/div/div[2]/div/div/figure/div')
                time.sleep(5)
                ActionChains(driver).move_to_element_with_offset(element_to_hover_over, hover_val_x[1], hover_val_y[1]).perform()
                html_content = driver.page_source
            for i in range(3):
                element_to_hover_over = driver.find_element("xpath", '/html/body/section/div[2]/main/div/div[2]/div[3]/div[3]/div/div/div['+str(tag_val)+']/div/div/div[2]/div/div[2]/div/div/div/div[3]/div/div/figure/div')
                time.sleep(5)
                ...
                if button.has_attr('onclick'):
                    ...
                    if loop==1:
                        try:
                            ...
                            time.sleep(2)
                            for bt in range(2):
                            #/html/body/section/div[2]/main/div/div[2]/div[3]/div[3]/div/div/div[16]/div/div/div[2]/div/div[2]/h2/button
                                button = driver.find_element(By.XPATH, '/html/body/section/div[2]/main/div/div[2]/div[3]/div[3]/div/div/div['+str(tag_val_cls)+']/div/div/div[2]/div/div[2]/h2/button')
                            button.click()
                            loop+=1
                        except:
                            time.sleep(1)
                    ...
                    try:
                        ...
                        if len(df)<4:
                            ...
                            hover_val_y=[0,10,0]
                            df= get_data(driver,hover_val_x,hover_val_y,tag_val_cls)
                        df['Subject'] = ['Language', 'Language', 'Maths', 'Maths', 'EVS', 'EVS']
                        ...
                        df.columns=['Location','Pct','Subject','State_Name','Class','Relevant_Date','Runtime']
                        df=df[['Class','State_Name','Subject','Location','Pct','Relevant_Date','Runtime']]
                        df.reset_index(drop=True,inplace=True)
                        df = df.pivot_table(index=['Class','State_Name','Subject','Relevant_Date','Runtime'], columns=['Location' ], values='Pct', aggfunc='sum')
                        df.reset_index(inplace=True)
                        df = df.rename(columns={'Rural': 'Rural_Pct'})
                        df = df.rename(columns={'Urban': 'Urban_Pct'})  
                        df=df[['Class','State_Name','Subject','Rural_Pct','Urban_Pct','Relevant_Date','Runtime']] 
                        print(df)
                        engine = adqvest_db.db_conn()
                        connection = engine.connect()
                        connection.execute("Delete from DOSEL_NAS_STATE_LOC_STU_PERF_PCT_CORRECT_YEARLY_ONE_TIME where State_Name='"+state_name+"' and Class='"+class_name[i]+"'")
                        connection.execute('commit')
                        time.sleep(3)
                        df.to_sql(name='DOSEL_NAS_STATE_LOC_STU_PERF_PCT_CORRECT_YEARLY_ONE_TIME',con=engine,if_exists='append',index=False)
                        count=count+1
