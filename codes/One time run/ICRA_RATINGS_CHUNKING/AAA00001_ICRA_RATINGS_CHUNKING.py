...
doc_links = client.execute('''SELECT distinct document_link FROM thurro_pdf_documents_vector_db_icra_ratings;''')
doc_links = [link for (link,) in doc_links]
print(len(doc_links))

doc_ids = client.execute('''SELECT distinct document_id FROM thurro_pdf_documents_vector_db_icra_ratings;''')
...
for i in range(len(icra_ratings)):
    ...
    if(link not in doc_links):
        ...
        max_doc_ids = client.execute('''SELECT max(document_id) as max_id FROM thurro_pdf_documents_vector_db_icra_ratings;''')
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                client.execute(f"INSERT INTO thurro_pdf_documents_vector_db_icra_ratings (document_id,document_company,document_type,document_year,document_date,document_link,document_content,document_content_modify,embedding) VALUES ('{file_id}','{company_name}','{report_type}','{relevant_year}','{document_date}','{link}','''{chunk}''','''{new_chunk}''',{chunk_embed});") 
