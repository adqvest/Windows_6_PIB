...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 10 12:27:26 2025
...
def clean_company(text,new_mapping={}):
    ...
    def Merge(dict1, dict2):
        res = {**dict1, **dict2}
        return res
    mapping = {'limited':'ltd','private':'pvt','pte':'pvt','international':'intl',
            'privatelimited':'pvt ltd','bnak':'bank','corpn':' corporation','housing development finance corp':'hdfc'}
    mapping = Merge(mapping, new_mapping)
    for i in mapping.items():
        text = re.sub(r'(?<!\S)' + i[0] + '+(?!\S)', i[1], text, flags=re.IGNORECASE)
    text = text.replace('  ',' ')
    text = re.sub(r'  +',' ', text).strip()
    text = text.replace('private','').replace('ltd','').replace('pvt','').strip()
...
insert_query = "insert into ICRA_DAILY_FILES_LINKS_CHUNKING select *, NULL as Chunked from ICRA_DAILY_FILES_LINKS where Links not in (select distinct Links from ICRA_DAILY_FILES_LINKS_CHUNKING) order by Relevant_Date;"
engine.execute(insert_query)

query = "select * from ICRA_DAILY_FILES_LINKS_CHUNKING where Chunked is null"
icra_ratings = pd.read_sql(query,con=engine)

doc_links = client.execute('''SELECT distinct document_link FROM thurro_pdf_documents_vector_db_icra_ratings_smallcase;''')
doc_links = [link for (link,) in doc_links]
print(len(doc_links))

doc_ids = client.execute('''SELECT distinct document_id FROM thurro_pdf_documents_vector_db_icra_ratings_smallcase;''')
doc_ids = [str(x).replace('(','').replace(',)','') for x in doc_ids]
print(len(doc_ids))

for i in range(len(icra_ratings)):
    company_name = icra_ratings['Company_Name'][i]
    company_name = clean_company(company_name)
    link = icra_ratings['Links'][i]
    file_name = icra_ratings['File_Name'][i]
    year = icra_ratings['Relevant_Date'][i].year
    month = icra_ratings['Relevant_Date'][i].month
    if(month >= 4):
        year = year+1
    if(month < 4):
        year = year
    ...
    if(link not in doc_links):
        ...
        max_doc_ids = client.execute('''SELECT max(document_id) as max_id FROM thurro_pdf_documents_vector_db_icra_ratings_smallcase;''')
        ...
        try:
            ...
            if('NoSuchKey' in str(e)):
                try:
                    dbfunctions.url_to_s3(link, folder_name,file_name)
                    files_content, file_name = read_all_file_content(folder_name, file_name)
                    updated_link = link.replace('%','%%')
                    update_query = f"update ICRA_DAILY_FILES_LINKS_CHUNKING set Chunked = 'Failed' where Links = '{updated_link}' and Relevant_Date = '{icra_ratings['Relevant_Date'][i]}'"
                    print(update_query)
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                client.execute(f"INSERT INTO thurro_pdf_documents_vector_db_icra_ratings_smallcase (document_id,document_company,document_type,document_year,document_date,document_link,document_content,document_content_modify,embedding) VALUES ('{file_id}','{company_name}','{report_type}','{relevant_year}','{document_date}','{link}','''{chunk}''','''{new_chunk}''',{chunk_embed});") 
                                print('Document chucked and inserted into clickhouse')
                                updated_link = link.replace('%','%%')
                                update_query = f"update ICRA_DAILY_FILES_LINKS_CHUNKING set Chunked = 'Yes' where Links = '{updated_link}' and Relevant_Date = '{icra_ratings['Relevant_Date'][i]}'"
                                print(update_query)
                                connection.execute(text(update_query))
                                connection.execute("commit")
                                print('sql table updated')
