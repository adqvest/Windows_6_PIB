    ...
    try:
        ...
        def get_fuels(driver):
            ...
            while True:
                try:
                    try:
                        driver.find_element_by_xpath("//*[@id='filterLayout']/div[1]/a/span").click()
                    except:
                        pass
                    driver.find_element_by_xpath('//*[@id="filterLayout-toggler"]/span/a').click()
                    time.sleep(0.5)
                    #no_of_ping += 1
                    fuel_info = {}
        ...
        def get_states(driver):
            ...
            while True:
                try:
                    ...
                    state_option = soup.findAll('select', attrs={'id':'j_idt43_input'})[0].findAll('option')
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        driver.find_element_by_xpath("//div[@id='j_idt43']//span[@class='ui-icon ui-icon-triangle-1-s ui-c']").click()
                    except:
                        driver.find_element_by_xpath("//div[@id='j_idt43']//div[@class = 'ui-selectonemenu-trigger ui-state-default ui-corner-right']").click()
        ...
        def get_dates(cond):
            ...
            query = "select distinct Relevant_Date FROM VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_CLEAN_FINAL where Relevant_Date>='2019-01-01' and Relevant_Date <='2019-12-31'"
            maker_vs_fuel_db_data = pd.read_sql(query, engine)

            distinct_date = set(maker_vs_fuel_db_data['Relevant_Date'])
            ...
            #dates_df = dates_df.append(pd.DataFrame({"Relevant_Date":[min_date]})) #Uncomment this line
        ...
        def get_vehicle_class(driver):
            ...
            while True:
                try:
                    ...
                    required_vehicle_class=['AGRICULTURAL TRACTOR','TRACTOR (COMMERCIAL)','POWER TILLER','POWER TILLER (COMMERCIAL)']
        ...
        def data_check():
            ...
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP  group by RTO_Office_Raw,Relevant_Date",engine)
            ...
            if(error_df.empty):
                ...
                for index,i in error_df.iterrows():
                    connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                    connection.execute("commit")

                    connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP set State_Status = Null where State = '" +i["State"] +"'")
                    connection.execute("commit")


                    connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    download_file_path = r"C:\Users\Administrator\Junk_Vahan_Full_RTO_Tractors_AP"

                    os.chdir(r"C:\Users\Administrator\Junk_Vahan_Full_RTO_Tractors_AP")
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP", con=engine)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP", con=engine, index=False, if_exists='append')

                    #########################################
                    query = "select min(Relevant_Date) as Min FROM VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP"
                    ...
                    for st in state_info:
                        ...
                        for x in rto_info.keys():
                            ...
                            for i in range(len(dates_df)):
                                ...
                                driver,vehicle_class_info,length1=get_vehicle_class(driver)

                                for vc in vehicle_class_info.items():
                                    driver.find_element_by_xpath("//*[@id='VhClass']/tbody/tr["+str(vc[1])+"]/td/label").click()

                                for jk in fuel_info.items():
                                    print(jk)
                                    driver.find_element_by_xpath("//*[@id='fuel']/tbody/tr["+str(jk[1])+"]/td/label").click()

                                    driver.find_element_by_xpath("//*[@id='j_idt87']/span[2]").click()
                                    # driver.find_element_by_xpath("//*[@id='j_idt66']/span[2]").click()
                                    time.sleep(1)
                                ...
                                for jk in other_fuels:
                                    element = driver.find_element_by_xpath("//*[@id='fuel']/tbody/tr["+str(jk)+"]/td/label")
                                ...
                                if('no records found' in str(soup).lower()):
                                    ...
                                    print(vahan_df_maker_vs_cat_vs_fuel)
                                    vahan_maker_cat_fuel_rto_level = pd.concat([vahan_maker_cat_fuel_rto_level,vahan_df_maker_vs_cat_vs_fuel])
                                print(vahan_maker_cat_fuel_rto_level['Relevant_Date'].unique())
                            ...
                            print(vahan_maker_cat_fuel_rto_level['Relevant_Date'].unique())
                            vahan_maker_cat_fuel_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP", con=engine, index=False, if_exists='append')

                            print('Check 4')
                            print(state, rto)
                            connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP set RTO_Status = 'Done' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")
                            print('Check 5')
                            connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP set Runtime = '"+datetime.datetime.now(india_time).strftime("%Y-%m-%d %H:%M:%S")+"' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")
                        ...
                        connection.execute("update VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
                ...
                try:
                    ...
                    pass
            data_check()
            query = "delete from VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP"
            connection.execute(query)
            connection.execute('commit')

            #maker vs category
            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP", con=engine)
            ...
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_FINAL_TRACTORS_AP where Relevant_Date>='" + date_vahan + "'"  #Uncomment these 3 lines
            connection.execute(query)
            connection.execute('commit')


            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP where Relevant_Date>='" + date_vahan + "'", con=engine)
                df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_FINAL_TRACTORS_AP', con=engine, index=False, if_exists='append')
                #ClickHouse

            except:
                print('ERROR 3')
                raise Exception('error in VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP')


            query = "drop table VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP"
            connection.execute(query)
            connection.execute('commit')
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP')"
        ...
        def rto_count():
            query = "select distinct Relevant_Date from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA where Relevant_Date>='" + datetime.date(today.year - 2, today.month, today.day).strftime("%Y-%m-%d") + "'"
            ...
            new_df = pd.read_sql("select  RTO_Office_Raw, RTO_Code from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA where Relevant_Date=(select max(Relevant_Date) from VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA where Relevant_Date <= '2020-02-29')", con=engine)
        ...
        query = "SELECT * FROM VAHAN_TABULAR_SUMMARY_FUEL_AND_RTO_STATE_DONE_TRACTORS_AP"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [1,11,21]):
                ...
                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTORS_Temp_Data2_AP select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_RTO_LEVEL_DATA_FINAL_TRACTORS_AP limit 0"
                connection.execute(query)
                connection.execute('commit')
