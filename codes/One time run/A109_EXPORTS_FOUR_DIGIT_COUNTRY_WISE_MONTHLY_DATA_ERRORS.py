...
#        export_top_500=exports.sort_values(by='Value',ascending=False).iloc[1:501]
        q = "Select Distinct Four_Digit_HSCode as HSCode,Four_Digit_Commodity as Commodity from EXPORTS_CLEAN_DATA_HSN_MONTHLY"
...
#              HScode.append(export_top_500['HSCode'].iloc[code])
        q = "Select max(Relevant_Date) as Relevant_Date,HSCode,Commodity from EXPORTS_FOUR_DIGIT_COUNTRY_WISE_MONTHLY_DATA group by HSCode,Commodity"
        export_top_500,cols = client.execute(q,with_column_types=True)
        export_top_500 = pd.DataFrame(export_top_500, columns=[tuple[0] for tuple in cols])
        export_top_500.reset_index(drop=True,inplace=True)
...
#          date1+=relativedelta(months=1)
        #all_dates = [datetime.date(2022,1,31),datetime.date(2022,2,28)]
        #all_dates = [ldm(today.date())]
        max_date = pd.read_sql("Select max(Relevant_Date) as RD from AdqvestDB.EXPORTS_DATA_HSN_MONTHLY",engine)['RD'][0]
        if date2 == max_date:
      ...
      #            max_rel_date=pd.read_sql("select max(Relevant_Date) as MAX from AdqvestDB.EXPORTS_DATA_COUNTRY_WISE_MONTHLY where HSCode="+export_top_500['HSCode'].iloc[code],engine)['MAX'][0]
                  max_rel_date = date1#export_top_500['Relevant_Date'].iloc[0]
    #              date1 = max_rel_date
      ...
      #            Latest_Date = date1+relativedelta(months=1)
                  ...
                  # while (Latest_Month<today.month) | (Latest_Year<today.year):
    ...
    #                             print(prev_mon)
    #                             data1={l1[1]['name']:l1[1]['value'],
    #                               'Mm1':str(prev_mon),'yy1':str(prev_yr),'hscode':str(export_top_500['HSCode'].iloc[code]),'sort':'0',l1[3]['name']:l1[3]['value'],
    #                              l1[6]['name']:l1[6]['value']}
    #                             req=session.post("https://tradestat.commerce.gov.in/meidb/comcnt.asp?ie=e",data=data1,headers=headers,verify=False)
    ##                             no_of_ping +=1
    #                             inr_soup=BeautifulSoup(req.text,'lxml')
    ...
    #
    #                             data2={l1[1]['name']:l1[1]['value'],
    #                               'Mm1':str(prev_mon),'yy1':str(prev_yr),'hscode':str(export_top_500['HSCode'].iloc[code]),'sort':'0',l1[4]['name']:l1[4]['value'],
    #                              l1[6]['name']:l1[6]['value']}
    #                             req=session.post("https://tradestat.commerce.gov.in/meidb/comcnt.asp?ie=e",data=data2,headers=headers,verify=False)
    ##                             no_of_ping +=1
    #                             usd_soup=BeautifulSoup(req.text,'lxml')
    ...
    #
    #                             data3={l1[1]['name']:l1[1]['value'],
    #                               'Mm1':str(prev_mon),'yy1':str(prev_yr),'hscode':str(export_top_500['HSCode'].iloc[code]),'sort':'0',l1[5]['name']:l1[5]['value'],
    #                              l1[6]['name']:l1[6]['value']}
    #                             req=session.post("https://tradestat.commerce.gov.in/meidb/comcnt.asp?ie=e",data=data3,headers=headers,verify=False)
    ##                             no_of_ping +=1
    #                             qty_soup=BeautifulSoup(req.text,'lxml')
