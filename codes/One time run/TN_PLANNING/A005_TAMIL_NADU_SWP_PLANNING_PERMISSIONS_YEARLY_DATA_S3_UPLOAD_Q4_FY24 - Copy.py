...
def upload_to_s3(url,filename):
    ...
    if response.status_code == 200:
        ...
        if file_size > size_threshold:
            S3_upload(sanitized_filename,'TAMIL_NADU_SWP_PLANNING_PERMISSIONS_YEARLY_DATA')
            S3_upload(sanitized_filename,'TAMIL_NADU_SWP_PLANNING_PERMISSIONS_YEARLY_DATA_TO_BE_CHUNKED')
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    #add table name
    table_name = 'TAMIL_NADU_SWP_PLANNING_PERMISSIONS_YEARLY_DATA_S3_Upload'
...
#%%     
        ...
        status_empty_data=pd.read_sql(f"select * from TAMIL_NADU_SWP_PLANNING_PERMISSIONS_YEARLY_DATA where S3_Upload_Status is null and S3_Upload_Comments is  null and  Relevant_Quarter in ('Q1 FY23','Q2 FY23','Q3 FY23','Q4 FY23') order by File_Id asc;", con=engine)
        ...
        for x in range(len(status_empty_data)):
            ...
            try:
                if status_val=='Done':
                    query=f"Update TAMIL_NADU_SWP_PLANNING_PERMISSIONS_YEARLY_DATA set S3_Upload_Status='Done',S3_Upload_Comments=NULL where File_Name='{file_name}'"
                    connection.execute(query)
                    time.sleep(2)
                    print('Done updates')
                else:
                    query=f"Update TAMIL_NADU_SWP_PLANNING_PERMISSIONS_YEARLY_DATA set S3_Upload_Comments='{status_val}' where File_Name='{file_name}'"
                    connection.execute(query)
                    time.sleep(2)
                    print(status_val)
            except:
