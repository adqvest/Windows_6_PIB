...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        def read_file(type_,state,rto,rel_date):
        ...
        def get_dates(cond):
            ...
            query = "select distinct Relevant_Date FROM VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Adv where Relevant_Date >= '2018-01-01' "
            maker_vs_fuel_db_data = pd.read_sql(query, engine)

            query = "select distinct Relevant_Date FROM VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA_Adv where Relevant_Date >= '2018-01-01' "
            ...
            query = "select min(Relevant_Date) as Min FROM VAHAN_TABULAR_SUMMARY_STATE_DONE_Adv"
            ...
            dates_df = pd.DataFrame({"Relevant_Date":distinct_date})
            ...
            if(cond):
                ...
                dates_df = dates_df[:1]
            del_date = (dates_df["Relevant_Date"].min() - pd.DateOffset(months=1)).date()
            dates_df.reset_index(drop = True,inplace = True)

            return dates_df,del_date
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_TABULAR_SUMMARY_STATE_DONE_Adv", con=engine)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_TABULAR_SUMMARY_STATE_DONE_Adv", con=engine, index=False, if_exists='append')

                    #########################################
                    query = "select max(Relevant_Date) as Max FROM VAHAN_TABULAR_SUMMARY_STATE_DONE_Adv"
                    ...
                    dates_df,del_date = get_dates(cond)
                    #########################################
                    ...
                    for st in state_info:
                        ...
                        for x in rto_info.keys():
                            ...
                            connection.execute("update VAHAN_TABULAR_SUMMARY_STATE_DONE_Adv set RTO_Status = 'Done' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")

                            connection.execute("update VAHAN_TABULAR_SUMMARY_STATE_DONE_Adv set Runtime = '"+datetime.datetime.now(india_time).strftime("%Y-%m-%d %H:%M:%S")+"' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")

                        connection.execute("update VAHAN_TABULAR_SUMMARY_STATE_DONE_Adv set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
                    break
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data', 'VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data')"
        ...
        today = datetime.datetime.now(india_time)
        query = "SELECT * FROM VAHAN_TABULAR_SUMMARY_STATE_DONE_Adv"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [1,3, 13, 18, 25, calendar.monthrange(today.year, today.month)[1]]):

                print('B')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_Temp_Data select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_RTO_LEVEL_DATA_Adv limit 0"
                connection.execute(query)
                connection.execute('commit')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Temp_Data select * from AdqvestDB.VAHAN_MAKER_VS_FUEL_RTO_LEVEL_DATA_Adv limit 0"
                connection.execute(query)
                connection.execute('commit')
