...
def Upload_Data_MySQL(data):
        data.to_sql('ENAM_MANDIS_AGMARKNET_PAST_DAILY_DATA_4_UPLOAD_STAGING', con=engine, if_exists='append', index=False)
...
def update_ch_condition_based(table_name):
    query = 'select * from AdqvestDB.ENAM_MANDIS_AGMARKNET_PAST_DAILY_DATA_4_UPLOAD_STAGING;'
    df = pd.read_sql(query,engine)
    df['Act_Runtime'] = df.Act_Runtime.replace(np.nan, 0)
    df['Act_Runtime'] = df.Act_Runtime.replace(0, None)

    df_list=[]
    for src in ['ENAM','Non-ENAM']:
        query1=f"select State,max(Relevant_Date) as Relevant_Date from AdqvestDB.{table_name} where Source ='{src}' group by State order by State asc;"
        a,cols = client.execute(query1,with_column_types=True)
        df1 = pd.DataFrame(a, columns=[tuple[0] for tuple in cols])
        df1['Relevant_Date']=df1['Relevant_Date'].apply(lambda x:str(x))
         
        df_dates=df1.Relevant_Date.to_list()
        df_states=df1.State.to_list()
        if len(df_states)>0:
            for st,dt in zip(df_states,df_dates):
                print(st,dt)
                df2=df[((df['State']==st)==True) & ((df['Source']==src)==True) & ((df['Relevant_Date']>pd.to_datetime(dt, format='%Y-%m-%d').date())==True)]
                df_list.append(df2)

    df_f=pd.concat(df_list)
    client.execute("INSERT INTO AdqvestDB."+table_name+" VALUES",df_f.values.tolist())
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        engine.execute('Delete from ENAM_MANDIS_AGMARKNET_PAST_DAILY_DATA_4_UPLOAD_STAGING')
            ...
            if len(all_dates)>0:
                ...
                for date1 in all_dates:
                    ...
                    if df.empty == False:
                        ...
                        df['Unit']=df['Unit'].replace({'1 Kg':'Kg'})
                        ...
                        df['Price_In_Rs_Modal_Price']=np.where((df["Unit"].isin(["Kg"])),df['Price_In_Rs_Modal_Price']/100,df['Price_In_Rs_Modal_Price'])
                        df['Price_In_Rs_Min_Price']=np.where((df["Unit"].isin(["Kg"])),df['Price_In_Rs_Min_Price']/100,df['Price_In_Rs_Min_Price'])
                        df['Price_In_Rs_Max_Price']=np.where((df["Unit"].isin(["Kg"])),df['Price_In_Rs_Max_Price']/100,df['Price_In_Rs_Max_Price'])
                        df['Commodity_Arrivals']=np.where((df["Unit"].isin(["Kg"])),df['Commodity_Arrivals']/100,df['Commodity_Arrivals'])
                        df['Unit']=np.where((df["Unit"].isin(["Kg"])),'Quintal',df['Unit'])
...
#%%
            ...
            if len(all_dates)>0:
                ...
                for date1 in all_dates:
                    ...
                    if df.empty == False:
                        ...
                        df['Price_In_Rs_Max_Price']=np.where((df["Unit"].isin(["Kg"])),df['Price_In_Rs_Max_Price']/100,df['Price_In_Rs_Max_Price'])
                        df['Commodity_Arrivals']=np.where((df["Unit"].isin(["Kg"])),df['Commodity_Arrivals']/100,df['Commodity_Arrivals'])
                        df['Commodity_Traded']=np.where((df["Unit"].isin(["Kg"])),df['Commodity_Traded']/100,df['Commodity_Traded'])
                        df['Unit']=np.where((df["Unit"].isin(["Kg"])),'Quintal',df['Unit'])
        ...
        update_ch_condition_based(new_table)
