...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try :
        ...
        if (today.date()-max_date).days >= 7:
          ...
          r = requests.get("https://haryanarera.gov.in/admincontrol/registered_projects/1", timeout = 120)
          robot.add_link("https://haryanarera.gov.in/admincontrol/registered_projects/1")
          ...
          df1=pd.DataFrame()
          #a[1]
          ##Added by Nidhi: 06-June-2023 a[1] changed to a[4]
          a[4]
          df1 = a[4]
          df2 = pd.DataFrame()
          ...
          print(df2.columns)
          # added kama
          column_list=["Registration_Certificate_Number", "Project_ID", "Project_Name", "Builder", "Project_Location",
                        "Project_District","Registered_With","URL"]
          if len(column_list)!=len(df2.columns):
            df2 = df2.drop('Registration Up-to', axis=1, errors='ignore')
           # ended
  ...
  #                        merge_df.to_csv(r"C:\Users\Administrator\AdQvestDir\codes\One time run\CHECK.csv")
          ...
          final_required['Project_ID'] = ['-' + re.sub('\D+-','',x) for x in final_required['Project_ID']]
          ...
          #Added by Nidhi 29-05-2023###
          for i in range(len(final_required['Approved_On'])):
            print(final_required['Approved_On'][i])
            if final_required['Approved_On'][i] == None:
                pass
            else:
                final_required['Approved_On'][i] = dparser.parse(final_required['Approved_On'][i],fuzzy=True).date()
