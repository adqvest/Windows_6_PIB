...
def run_program(run_by='Adqvest_Bot', py_file_name=None):    
    ...
    table_name = 'ICRA_DAILY_DATA_CORPUS,ICRA_DAILY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        doc_ids = client.execute('''SELECT distinct Document_Id FROM ICRA_DAILY_DATA_CORPUS_CHUNKED;''')
        ...
        for obj in response:
            ...
            table_file_name = file_name.replace("'", "")
            ...
            try:
                ...
                try:
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE ICRA_DAILY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE Generated_File_Name = '{str(table_file_name)}'"
                        print(query)
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection=engine.connect()
                                connection.execute("update ICRA_DAILY_DATA_CORPUS set Chunking_Status='Not Relevant' where Generated_File_Name = '" +str(table_file_name)+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update ICRA_DAILY_DATA_CORPUS set Chunking_Comments='ISSUE' where Generated_File_Name = '" +str(table_file_name)+"'")
                        connection.execute("commit")
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                # client.execute(f"INSERT INTO ICRA_DAILY_DATA_CORPUS_CHUNKED (Document_Id,Chunk_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.str.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{datetime.datetime.now(india_time)}');")   
                                client.execute(f"INSERT INTO ICRA_DAILY_DATA_CORPUS_CHUNKED (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")   
                                connection=engine.connect() 
                                connection.execute(text("UPDATE ICRA_DAILY_DATA_CORPUS SET Chunking_Status='Done' WHERE Generated_File_Name = :file_name"),{"file_name": table_file_name})
                                # connection.commit()
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.ICRA_DAILY_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM ICRA_DAILY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE ICRA_DAILY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM ICRA_DAILY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM ICRA_DAILY_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
