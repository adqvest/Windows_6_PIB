...
def create_embeddings(chunked_text):
    ...
    # Embed the documents
    doc_embds = vo.embed(
        chunked_text, model="voyage-3", input_type="document"
    ).embeddings
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS,SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        folder_name = 'SEBI_LEGAL_CIRCULAR_CORPUS_TO_BE_CHUNKED'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'SEBI_LEGAL_CIRCULAR_CORPUS_TO_BE_CHUNKED/' in x]
        response = [ x for x in response]
        response = response[1:]
        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED;''')
        ...
        for obj in response:
            ...
            try:
                try:
                    ...
                    title = pd.read_sql(f"select Report_Title from AdqvestDB.SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS where File_Name='{file_name}' order by Relevant_Date DESC",engine)['Report_Title'][0]
                    title = str(title).replace("'", "\\'")

                except Exception as e:
                    print(e)
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE File_Name = '{str(file_name)}'"
                        print(query)
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute("update SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name)+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '" +str(file_name)+"'")
                        connection.execute("commit")
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk, page_number in zip(extracted_chunks, range(len(extracted_chunks))):
                            ...
                            # Build combined text: Title + pagewise chunk
                            # title is already fetched earlier from SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS
                            combined_text = (str(title) + " " + chunk).strip()

                            # Embedding must be based on the same text that goes into Document_Content
                            try:
                                chunk_embed = str(create_embeddings(combined_text)).replace('[[','[').replace(']]',']')

                                # Store combined_text as Document_Content
                                client.execute(
                                    f"""INSERT INTO SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED
                                        (Document_Id,Document_Company,Document_Type,Document_Category,Title,
                                         Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking)
                                        VALUES ('{file_id}','{company_name.title()}','{report_type}','{Report_Category}',
                                                '{title}','{relevant_year}','{document_date}',{page},
                                                '''{combined_text}''',{chunk_embed},'{timestamp}');"""
                                )
                            except:
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc)>0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED where Published_Date is Null;''')
        print(len(doc))
