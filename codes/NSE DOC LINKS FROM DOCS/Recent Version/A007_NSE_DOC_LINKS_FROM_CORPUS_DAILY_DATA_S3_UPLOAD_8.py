...
def download_file_content(pdf_url,filename,table_name,engine):
    try:
        ...
        session.verify = True
        pdf_url = pdf_url.replace("\x02", "-").rstrip("\\").strip()
        response=httpx.get(pdf_url,headers=headers,timeout=60)
...
def run_program(run_by = 'Adqvest_Bot', py_file_name = None):
    ...
    try :
        ...
        for Working_table,keys in lookup_dict.items():
            ...
            query = f"SELECT * FROM {Working_table} where S3_Upload_Status is Null AND S3_Upload_Comments is Null LIMIT 1000  OFFSET 1000;"
