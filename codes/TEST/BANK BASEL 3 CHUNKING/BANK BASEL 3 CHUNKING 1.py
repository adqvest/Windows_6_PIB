...
folder_name = 'BANK_BASEL_III_DISCLOSURES_CORPUS'

response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
response = [ x for x in response if 'BANK_BASEL_III_DISCLOSURES_CORPUS/' in x]
print(len(response))
# response = response[:1]

doc_ids = client.execute(f'''SELECT distinct Document_Id FROM BANK_BASEL_III_DISCLOSURES_QUARTERLY_DATA_CORPUS_CHUNKED;''')
doc_ids = [str(x).replace('(','').replace(',)','') for x in doc_ids]
doc_ids = [d.strip("'").strip() for d in doc_ids]
print(len(doc_ids))
...
for obj in response:
    ...
    try:
        try:
            ...
            document_id_alpha = 'T'+file_id
            print(document_id_alpha)
        except:
            query=f"Update BANK_BASEL_III_QUARTERLY_LINKS set Chunking_Comments='ISSUE' where File_ID='{'T'+str(file_id)}'"
            print(query)
            connection.execute(sqltext(query))
            print('Update Done as Issue')
            continue
        if str('T'+file_id) not in doc_ids :
            print(relevant_year)
            ...
            if int(relevant_year[5:])>=18 and int(relevant_year[5:])<30:
                ...
                if (len(extracted_chunks)>0):
                    ...
                    try:
                        ...
                        query=f"Update BANK_BASEL_III_QUARTERLY_LINKS set Chunking_Status='Done',Chunking_Comments=Null where File_ID='{'T'+str(file_id)}'"
                        print(query)
                        connection.execute(sqltext(query))
                        print('Done Updates')
                    except:
                    ...
                    if p==len(extracted_chunks):
                        print('==========================================================')
                        query=f"Update BANK_BASEL_III_QUARTERLY_LINKS set Chunking_Comments='No page extracted',Chunking_Status=Null where  File_ID='{'T'+str(file_id)}'"
                        print(query)
                        connection.execute(sqltext(query))
                        print('NO PAGES Updates')

                else:
                    query=f"Update BANK_BASEL_III_QUARTERLY_LINKS set Chunking_Comments='No page extracted' where  File_ID='{'T'+str(file_id)}'"
                    print(query)
                    connection.execute(sqltext(query))
                    print('NO PAGES Updates')
        ...
        query=f"Update BANK_BASEL_III_QUARTERLY_LINKS set Chunking_Comments='{error_msg}' where File_ID='{'T'+file_id}'"
        print(query)
        connection.execute(sqltext(query))
        print('ISSUE Updates')