...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    from clickhouse_driver import Client
    host = 'ec2-13-235-31-0.ap-south-1.compute.amazonaws.com'
    user_name = 'default'
    ...
    try:
        ...
        folder_name = 'GENERAL_INSURANCE_CORPUS_5'
        ...
        response = [ x for x in response if 'GENERAL_INSURANCE_CORPUS_5/' in x]
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute("update GEN_INSURANCE_QUARTERLY_DATA_CORPUS set Chunking_Status='Not Relevant' where Cleaned_File_Name = '" +str(file_name_without_extension)+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update GEN_INSURANCE_QUARTERLY_DATA_CORPUS set Chunking_Comments='Issues' where Cleaned_File_Name = '" +str(file_name_without_extension)+"'")
                        connection.execute("commit")
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                client.execute(f"INSERT INTO GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED (document_id,document_company,document_company_small_case,document_type,document_year,document_date,page_number,document_content,document_content_small_case,document_content_modify,embedding,embedding_small_case) VALUES ('{file_id}','{company_name}','{company_small}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''','''{chunk_small}''','''{new_chunk}''',{chunk_embed},{chunk_embed_small});") 
                                connection=engine.connect()
                                connection.execute("update GEN_INSURANCE_QUARTERLY_DATA_CORPUS set Chunking_Status='Done' where Cleaned_File_Name = '" +str(file_name_without_extension)+"'")
                                connection.execute("commit")
                            except:
