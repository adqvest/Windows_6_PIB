...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        # folder_name = 'GENERAL_INSURANCE_CORPUS_TO_BE_CHUNKED_4'
        ...
        # response = [ x for x in response if 'GENERAL_INSURANCE_CORPUS_TO_BE_CHUNKED_4/' in x]
        
        # response = response[6001:12000]
        # print(len(response))
        
        # doc_ids = client.execute(f'''SELECT distinct document_id FROM GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED_3;''')
        ...
        #                         connection.execute("update GEN_INSURANCE_QUARTERLY_DATA_CORPUS set Chunk_Status='Not Relevant' where Cleaned_File_Name = '" +str(file_name_without_extension)+"'")
        ...
        #                 connection=engine.connect()
        #                 connection.execute("update GEN_INSURANCE_QUARTERLY_DATA_CORPUS set Chunk_Status='Issues' where Cleaned_File_Name = '" +str(file_name_without_extension)+"'")
        #                 connection.execute("commit")
        #                 continue
        ...
        #                         client.execute(f"INSERT INTO GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED_3 (document_id,document_company,document_company_small_case,document_type,document_year,document_date,document_content,document_content_small_case,document_content_modify,embedding,embedding_small_case) VALUES ('{file_id}','{company_name}','{company_small}','{report_type}','{relevant_year}','{document_date}','''{chunk}''','''{chunk_small}''','''{new_chunk}''',{chunk_embed},{chunk_embed_small});") 
        #                         connection=engine.connect()
        #                         connection.execute("update GEN_INSURANCE_QUARTERLY_DATA_CORPUS set Chunk_Status='Done' where Cleaned_File_Name = '" +str(file_name_without_extension)+"'")
        #                         connection.execute("commit")
        #                     except:
        ...
        query = "select * from AdqvestDB.GEN_INSURANCE_QUARTERLY_DATA_CORPUS ORDER BY Company ASC;"
        ...
        doc = client.execute(f'''SELECT distinct(document_id) FROM GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED_3 WHERE document_link = '';''')
        len(doc)


        companies = doc[700:1200]
        ...
        for x in range(len(data_1)):
            ...
            print('URL : ',url_value)
            print("Doc ID ",document_id_value)

            client.execute(f'''ALTER TABLE GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED_3 UPDATE document_link = '{url_value}' WHERE document_id = '{document_id_value}';''')
            client.execute(f'''ALTER TABLE GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED_3 UPDATE vector_db_status = 'LINK UPDATED' WHERE document_id = '{document_id_value}';''')
            ...
            if count % 900 == 0:
                ...
                client.execute(f'''OPTIMIZE TABLE GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED_3 FINAL;''')
