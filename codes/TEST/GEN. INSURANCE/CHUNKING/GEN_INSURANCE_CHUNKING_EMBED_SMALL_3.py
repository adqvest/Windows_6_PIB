...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        # doc_ids = client.execute(f'''SELECT distinct document_id FROM GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED;''')
        ...
        doc = client.execute(f'''SELECT distinct(document_id) FROM GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED WHERE document_link = '';''')
        len(doc)


        companies = doc[200:400]
        ...
        for x in range(len(data_1)):
            ...
            client.execute(f'''ALTER TABLE GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED UPDATE document_link = '{url_value}' WHERE document_id = '{document_id_value}';''')
            client.execute(f'''ALTER TABLE GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED UPDATE vector_db_status = 'LINK UPDATED' WHERE document_id = '{document_id_value}';''')
            ...
            if count % 900 == 0:
                ...
                client.execute(f'''OPTIMIZE TABLE GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED FINAL;''')
