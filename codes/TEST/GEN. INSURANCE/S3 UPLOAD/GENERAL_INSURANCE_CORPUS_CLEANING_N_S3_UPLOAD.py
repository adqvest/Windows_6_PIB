...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN'
    ...
    try:
        ...
        key1='GENERAL_INSURANCE_CORPUS_Q1FY26/'
        key2='GENERAL_INSURANCE_CORPUS_TO_BE_CHUNKED_Q1FY26/'
        path=r'C:/Users/Administrator/AdQvestDir/GENERAL_INSURANCE_Q1FY26/'

        # df_links=pd.read_sql("select * from GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN where Relevant_Date='2025-01-31' and Company='Zurich Kotak General Insurance Company Limited'",engine)
        # df_links=pd.read_sql("select * from GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN where S3_Upload='Not PDF'",engine)
        # df_links=df_links[:1500]

        df_links = pd.read_sql("select * from GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN where S3_Upload is NULL", engine)

        df_links['Url']=df_links['Url'].apply(lambda x:x.replace('\\','/'))

        df_links['Url']=df_links['Url'].apply(lambda x:x.replace(' ','%20'))
        ...
        for i, row in df_links.iterrows():
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 'application/pdf'  in content_type:
                        ...
                        query = """ UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET S3_Upload = 'Done'
                        WHERE File_ID = %s """
                        connection.execute(query, (num,))


                        # query6 = "UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET Is_PDF= %s WHERE File_ID = %s"
                        ...
                        connection=engine.connect()
                        query = """ UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET S3_Upload = 'Not PDF'
                        WHERE File_ID = %s """
                        connection.execute(query, (num,))
                        connection.execute("commit")
                    ...
                    connection=engine.connect()
                    query = """ UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET S3_Upload = 'Not Downloaded because of Requests' WHERE File_ID = %s """
                    connection.execute(query, (num,))
                    connection.execute("commit")
                    print('Updated Status as Not Downloaded, Response Status')
                    print('_'*88)
                ...
                #     soup = BeautifulSoup(content_type, 'lxml')
            
                #     if "Page Not Found" in soup.get_text():
                #         print(f"Page Not Found for {link}, skipping this URL.")
                #         connection=engine.connect()
                #         query = """ UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET S3_Upload = 'Not Available'
                #         WHERE Url = %s """
                ...
                #     connection.execute(query, (link,))
                    ...
                    # query1 = "UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET File_ID = %s WHERE Url = %s"
                    # connection.execute(query1, (num, link))
            
                    # query7 = "UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET Cleaned_File_Name= %s WHERE Url = %s"
            ...
            #     query3 = "UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET Report_Quarter = %s WHERE Url = %s"
            #     connection.execute(query3, (report_quarter, link))
            
            #     query4 = "UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET Report_Type = %s WHERE Url = %s"
            #     connection.execute(query4, (report_type, link))
            
            #     query5 = "UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET Company = %s WHERE Url = %s"
            #     connection.execute(query5, (company, link))
            # #
            except:
                # connection=engine.connect()
                # query = """ UPDATE GEN_INSURANCE_SCRAPY_CORPUS_FY26_Temp_Rithees_CLEAN SET S3_Upload = 'Failed'
                # WHERE File_ID = %s """
                # connection.execute(query, (num,))
                # connection.execute("commit")
