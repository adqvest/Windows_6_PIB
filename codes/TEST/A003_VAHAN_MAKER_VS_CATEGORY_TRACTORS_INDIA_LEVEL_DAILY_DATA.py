        ...
        def main_function():
            ...
            while True:
                try:
                        ...
                        for vc in vehicle_class_info.items():
                            ...
                            for jk in fuel_info.items():
                                ...
                                if('no records found' in str(soup).lower()):
                                    ...
                                    engine = adqvest_db.db_conn()
                                    connection = engine.connect()
                                    # vahan_df_maker_vs_cat_vs_fuel.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_INDIA_LEVEL_3W_MONTHLY_DATA", con=engine, index=False, if_exists='append')
                    ...
                    # query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_INDIA_LEVEL_TRACTORS_MONTHLY_DATA where Relevant_Date>='" + date_vahan + "'"
                    # connection.execute(query)
                    # connection.execute('commit')

                    vahan_maker_cat_fuel_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_INDIA_LEVEL_TRACTORS_MONTHLY_DATA", con=engine, index=False, if_exists='append')
