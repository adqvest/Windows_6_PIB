...
pdf_down_dir="C:/Users/Administrator/junk_nse"
...
#%%
        ...
        status_empty_data=pd.read_sql(f"select * from BANK_BASEL_III_QUARTERLY_LINKS where S3_Upload_Status is Null order by File_ID asc;", con=engine)
        ...
        # status_empty_data=status_empty_data[~status_empty_data['File_Link'].str.contains('.zip')].reset_index(drop=True)
        # status_empty_data=status_empty_data[status_empty_data['File_Link']!='-'].reset_index(drop=True)
        # status_empty_data = status_empty_data[:500]
        # print(status_empty_data)
        c=0
