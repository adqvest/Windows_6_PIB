...
response = [x for x in response if re.search(r'FY25', x)]
            ...
            if int(relevant_year[5:])>=18 and int(relevant_year[5:])<30:
                ...
                if (len(extracted_chunks)>0):
                    ...
                    try:
                        ...
                        query=f"Update NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS set Chunking_Status='Done',Chunking_Comments=Null where File_Name='{file_name.replace('.pdf','')}'"
                        print(query)
