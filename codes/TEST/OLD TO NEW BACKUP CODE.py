...
total_records = new_client.execute("SELECT COUNT(*) FROM thurro_pdf_documents_vector_db_final_2_backup_gokul_3")[0][0]
...
while offset < total_records:
    select_query = f"""
        SELECT 
            ...
            embedding,
            vector_db_status
        FROM thurro_pdf_documents_vector_db_final_2_backup_gokul_3
        LIMIT {batch_size} OFFSET {offset}
    """

    # Execute on old server
    rows = new_client.execute(select_query)
    ...
    if rows:
        ...
        new_client.execute(
            """
            INSERT INTO thurro_pdf_documents_vector_db_final_2_backup_gokul_4 (
                document_id,
                symbol,
                Sector,
                Industry,
    ...
    offset += batch_size
    time.sleep(15)  # optional cooldown
