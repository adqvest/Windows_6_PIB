...
# response = [x for x in response if re.search(r'FY19', x)]
...
# response=response[3001:6000]
print(len(response))

doc_ids = client.execute(f'''SELECT distinct Document_Id FROM NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS_CHUNKED;''')
...
for obj in response:
    ...
    try:
        try:
            ...
            # file_name_1=re.sub('.pdf','',file_name)
            query=f"Update NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS set Chunking_Status='ISSUE' where File_Name='{file_name}'"
            print(query)
            connection.execute(query)
            print("FILE IS ISSSUE")
            continue
            ...
            if int(relevant_year[5:])>=18 and int(relevant_year[5:])<30:
                ...
                if (len(extracted_chunks)>0):
                    ...
                    try:
                        ...
                        query=f"Update NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS set Chunking_Status='Done',Chunking_Comments=Null where File_Name='{file_name}'"
                        print(query)
                        connection.execute(query)
                    except:
                        file_name=re.sub('.pdf','',file_name)
                        query=f"Update NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS set Chunking_Status='Done',Chunking_Comments=Null where File_Id='{file_id}'"
                        print(query)
                        connection.execute(query)

                    if p==len(extracted_chunks):
                        print('==========================================================')
                        query=f"Update NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS set Chunking_Comments='No page extracted',Chunking_Status=Null where  File_Id='{file_id}'"
                        print(query)
                        connection.execute(query)
                    ...
                    query=f"Update NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS set Chunking_Comments='No page extracted' where  File_Id='{file_id}'"
                    print(query)
                    connection.execute(query)
        ...
        query=f"Update NSE_INVESTOR_INFORMATION_DAILY_DATA_CORPUS set Chunking_Comments='{error_msg}' where File_Name='{file_name}'"
        print(query)
        connection.execute(query)
        print("FILE IS TOO SMALL")

# connection = engine.connect()
# query = "select * from AdqvestDB.NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS ORDER BY File_Id ASC;"
# data = pd.read_sql(query, con=engine)

# doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
# print(len(doc))
...
#         link = data["Links"][x].replace("'", "\\'")  # Escape single quotes
...
#         query = f"""
#             ALTER TABLE NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS_CHUNKED
#             UPDATE 
...
# doc = client.execute('''SELECT distinct Document_Id as id FROM NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
# print(len(doc))
...
# doc = client.execute('''SELECT distinct Document_Id as id FROM NSE_INVESTOR_INFORMATION_PAST_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
# print(len(doc))
