...
def extract_text_from_pdf(files_content,file_name):
    ...
    report_type = report_type.split("NL")[0].strip()
...
password_string = 'Clickhouse@2024'
db_name = 'AdqvestDB'
client2 = Client(host, user=user_name, password=password_string, database=db_name)


bucket_name = 'adqvests3bucket'
folder_name = 'GENERAL_INSURANCE_CORPUS_TO_BE_CHUNKED_Q3_FY25'
...
response = [ x for x in response if 'GENERAL_INSURANCE_CORPUS_TO_BE_CHUNKED_Q3_FY25/' in x]
print(len(response))
response = response[1:]
print(len(response))
# response.reverse()

doc_ids = client2.execute(f'''SELECT distinct document_id FROM GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED;''')
...
for obj in response:
    ...
    try:
                ...
                if (len(extracted_chunks)>0):
                    ...
                    for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                        ...
                        try:
                            ...
                            client.execute(f"INSERT INTO GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED (document_id,document_company,document_type,document_year,document_date,document_content,document_content_modify,embedding) VALUES ('{file_id}','{company_name}','{report_type}','{relevant_year}','{document_date}','''{chunk}''','''{new_chunk}''',{chunk_embed});") 
                            
                            client2.execute(f"INSERT INTO GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED (document_id,document_company,document_type,document_year,document_date,document_content,document_content_modify,embedding) VALUES ('{file_id}','{company_name}','{report_type}','{relevant_year}','{document_date}','''{chunk}''','''{new_chunk}''',{chunk_embed});") 
                            
                            from sqlalchemy import text

                            query = f"""
                                UPDATE GEN_INSURANCE_SCRAPY_CORPUS_CLEAN_Temp_Rahul_2 
                                SET Chunk_Status='Done' WHERE Cleaned_File_Name='{file_name.replace('.pdf','')}';"""
        ...
        query=f"Update GEN_INSURANCE_SCRAPY_CORPUS_CLEAN_Temp_Rahul_2 set Chunk_Status='ISSUE' where File_Name='{file_name.replace('.pdf','')}'"
        print(query)
        connection.execute(query)
        print('Update as Done')
        print('_'*75)
        continue
...
connection = engine.connect()
query = "select * from AdqvestDB.GEN_INSURANCE_SCRAPY_CORPUS_CLEAN_Temp_Rahul_2 ORDER BY File_ID ASC;"
data = pd.read_sql(query, con=engine)

doc = client.execute('''SELECT distinct document_id as id , page_number as pn FROM GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED where document_link = '' ;''')
print(len(doc))
...
if len(doc) > 0:
    ...
    for batch_num in range(total_batches):
        ...
        query = f"""
            ALTER TABLE GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED
            UPDATE 
    ...
    query = """ALTER TABLE GEN_INSURANCE_QUARTERLY_DATA_CORPUS_CHUNKED UPDATE document_link = concat(document_link, '#page=', toString(page_number)) WHERE document_link LIKE '%.pdf%' AND document_link NOT LIKE '%.html%' AND document_link NOT LIKE '%.htm%' and document_link NOT LIKE '%#page=%';"""
