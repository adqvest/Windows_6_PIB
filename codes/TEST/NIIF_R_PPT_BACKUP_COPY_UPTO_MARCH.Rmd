...
#
#Database_Connection_CLICK_HOUSE
```{r}
...
get_data=function(mv_table,variable,act_amt='',Show_Date=FALSE,railway=FALSE,pow=FALSE,electricity = FALSE,bubble_map=FALSE,epfo=FALSE,vh_cat = '',fuel = '',toll=FALSE){
      ...
      if(electricity==TRUE){
        ...
        qu="select State,variable as Var,Relevant_Date from mv_table where Relevant_Date in (select max(Relevant_Date) from mv_table where Relevant_Date<='2023-06-30')"
      }
      else if(epfo==TRUE){
        ...
        if(fuel == ''){qu="select State,variable as Var,Relevant_Date from mv_table where Relevant_Date  in (select max(Relevant_Date) from mv_table where Relevant_Date<=prev_month) and Vehicle_Segment = 'vh_cat'"}
          ...
          qu="select State,variable as Var,Relevant_Date from mv_table where Relevant_Date  in (select max(Relevant_Date) from mv_table where Relevant_Date<=prev_month) and Vehicle_Segment = 'vh_cat' and Fuel = 'ELECTRIC(BOV)'"
        }
...
##Economic_indicator_page_1_Chart
```{r}
...
growth_df_1 = rbind(goi_gr,gross_tax_rev_gr,gst_coll_gr,iip_gr,core_8_gr,elec_gen_gr,steel_gr,
                    cement_gr,wpi_indx_gr,rail_freight_gr,port_cargo_gr,air_cargo_gr,
...
indicator_df_1 = rbind(goi,gross_tax_rev,gst_coll,
                       iip,core_8,elec_gen,steel,cement,wpi_indx,
...
##EConomic_indicator_Page_2_Charts
```{r}
...
##EConomic_indicator_old_arrangements_depricated
...
#
##Major_economic_indiacator_State
```{r}
...
##Major_economic_indiacator_State_abs
```{r}
gdp = get_data("GDP_STATEWISE_TEMP",'GDP')
gdp_rd =  gdp[2][[1]][1]
gdp = data.frame(gdp[1])
...
toll_traffic_yoy=get_data("mv_ihmcl_statewise_collection_niif",'Traffic')
toll_traffic_yoy_rd =  toll_traffic_yoy[2][[1]][1]
toll_traffic_yoy = data.frame(toll_traffic_yoy[1])
...
toll_revenue_yoy=get_data("mv_ihmcl_statewise_collection_niif",'Revenue')
toll_revenue_yoy_rd =  toll_revenue_yoy[2][[1]][1]
toll_revenue_yoy = data.frame(toll_revenue_yoy[1])
...
rail_cmnt = read_query(284821, 12)
...
# Rail Cargo | Coal
rail_coal = read_query(284836, 12)
...
rail_ferti = read_query(284815, 12)
...
#Rail Cargo | Iron Ore
rail_iron = read_query(284826, 12)
...
rail_mn_oil = read_query(284811, 12)
...
rail_ir_ft = read_query(284831, 12)
...
#Raw Material
rail_raw = read_query(2084624, 12)
...
#Rail Cargo
rail_total = read_query(284813, 12)
...
##HARD CODE
rail_df <- rail_df[order(as.numeric(rail_df$`Sep-23`),decreasing = TRUE),]
...
##Quarterly_balance_of_payments_(USD billion)_79_OLD_format
```{r eval=FALSE, include=FALSE}
...
```
#
##Quarterly_balance_of_payments_(USD billion)_new_format
```{r}
#Eco-Indicators | Current Account 
qtr_bop_mt=read_query(1633411,5,quarter=TRUE)
names(qtr_bop_mt)[1]<-"Relevant_Date"
names(qtr_bop_mt)[2]<-"Value"
qtr_bop_mt = table_preprocessing_annual(qtr_bop_mt,frequency_normalizer='quarter',
                                    period=1, 
...
names(qtr_bop_s)[1]<-"Relevant_Date"
names(qtr_bop_s)[2]<-"Value"
qtr_bop_s = table_preprocessing_annual(qtr_bop_s,frequency_normalizer='quarter',period=1, 
...
qtr_bop_i = table_preprocessing_annual(qtr_bop_i,frequency_normalizer='quarter',period=1, 
                                    variable = "e.  Income", 
                                    calculate_gr = FALSE,
                                    divisor =10^9, rounder = 1)
...
qtr_bop_t = table_preprocessing_annual(qtr_bop_t,frequency_normalizer='quarter',period=1, 
...
names(qtr_bop_caa)[1]<-"Relevant_Date"
names(qtr_bop_caa)[2]<-"Value"
qtr_bop_caa = table_preprocessing_annual(qtr_bop_caa,frequency_normalizer='quarter',period=1, 
...
qtr_bop_ca_gdp = table_preprocessing_annual(qtr_bop_ca_gdp,
                                    frequency_normalizer='quarter',period=1, 
                                    variable = "(as % of GDP) ", 
                                    calculate_gr = FALSE,
                                    divisor =1, rounder = 1)
...
qtr_bop_fdi = table_preprocessing_annual(qtr_bop_fdi,frequency_normalizer='quarter',
                                    period=1, variable = "h.  Foreign direct investment", 
                                    calculate_gr = FALSE,
                                    divisor =10^9, rounder = 1)
...
qtr_bop_pf = table_preprocessing_annual(qtr_bop_pf,frequency_normalizer='quarter',period=1, 
...
qtr_bop_bc = table_preprocessing_annual(qtr_bop_bc,frequency_normalizer='quarter',
                                    period=1, variable = "k.  Banking capital",
                                    calculate_gr = FALSE,
                                    divisor =10^9, rounder = 1)
...
qtr_bop_oth = table_preprocessing_annual(qtr_bop_oth,frequency_normalizer='quarter',period=1, 
                                    variable = "l.  Others", 
                                    calculate_gr = FALSE,
                                    divisor =10^9, rounder = 1)
...
names(qtr_bop_ra)[1]<-"Relevant_Date"
names(qtr_bop_ra)[2]<-"Value"
qtr_bop_ra = table_preprocessing_annual(qtr_bop_ra,frequency_normalizer='quarter',period=1, 
...
names(qtr_bop_eo)[1]<-"Relevant_Date"
names(qtr_bop_eo)[2]<-"Value"
qtr_bop_eo = table_preprocessing_annual(qtr_bop_eo,frequency_normalizer='quarter',period=1, 
...
```
#
##Quarterly_balance_of_payments_(USD billion)_Annual
```{r}
#Eco-Indicators | Current Account 
...
#
##Quarterly_equity_deployment_by_mutual_funds _75
```{r}
#MF | AUTO
...
##Annual_FDI_inflows_ by _sector_76
```{r}
...
##Annual_FDI_inflows_ by _country_77
```{r}
...
#BAR_LINE_CHART_COMBO_NIIF_UPDATED
```{r}
line_bar_chart_niif <- function(data1,data2,sales_heading,
  ...
  tryCatch({
   ...
   #############################Data_formatting##############################
    showtext_auto()
    data_final=create_df_for_chart(data1,data2,bar_line  =TRUE)[[1]]
...
#
#STACKED_BAR_LINE_COMBO_NIIF_UPDATED
```{r}
...
multi_line_chart_trend_niif <- function(data1,
                                  ...
                                  CPI_reference=FALSE,show_actual_lbls=FALSE,
...
#MULTI_LINE_CHART_NIIF_RAINFALL
```{r}
...
#STACKED_BAR_LINE COMBO_NIIF_SPECIAL_CASE_UPDATED
```{r}
...
##Scheduled_Commercial_Total_Deposit_gr Rate_16_31
```{r}
...
names(SCB_Deposit_gr)[2]='Growth including merger (RHS, % yoy)'
...
names(SCB_Depo_gr_exc_hdfc)[2]='Growth (RHS, % yoy)'
```
...
names(SCB_Credit_gr)[2]='Growth including merger (RHS, % yoy)'
...
names(SCB_cre_gr_exc_hdfc)[2]='Growth (RHS, % yoy)'
...
##Monthly_net_purchase/(sale)_US_dollars_RBI_28_63
```{r}
...
##Monthly_national_etc_road_toll_col_47_86
```{r}
...
##IHMCL_all-India toll collection
```{r}
#Revenue Collected
...
##Monthly_clearance_prices_on_IEX_DAM_51_95
```{r}
...
##Quarterly_real_GDP_growth_India_10
```{r} 
#Avg Daily Generation
...
##Services_Commerciasl_Real_estate_28
```{r}
...
##Quarterly_telecom_internet_subscribers_66
```{r}
...
#
##Global_services_modern_services
```{r eval=FALSE, include=FALSE}
...
#
##India_services_modern_services
```{r eval=FALSE, include=FALSE}
#World | Exports | Services
...
###Monthly_copper_and_iron_ore_prices_(World Bank)_42_80
```{r}
...
#
##Monthly     Fertilizer Sales_15_27
```{r}
#Product Wise
...
##Mon_outstanding_dues_of_discoms_46
```{r}
#O/S Dues For NIIF
...
##Quarterly_outstan_deployment_in_debt_69
```{r}
...
##Outstanding_central_govt_domestic_borrowings_63
```{r}
...
##Outstanding_central_external_govt_borrowings_63
```{r}
...
##Quarterly_mutual_fund_folios_68
```{r}
#MF | High Networth Individuals
...
##Quarterly_GVA_contribution_segments_6
...
##Monthly_Mon_debt_funds_mobilized_in_India_31
```{r}
...
##Mon_equity_fu_mob_India_(INR billion)_31
...
##Annual_net_DII_inv_in_India_37
...
##Monthly_net_DII_inv_in_India_37
...
##Monthly_net_inflows_mutual_funds_74
```{r}
...
##Quarterly_AIF_net_fundraises_44 
```{r}
...
##Quarterly_AIF_net_investments_45
```{r}
...
Qtr_AIF_net_invest_chart=stacked_bar_chart_niif(data_s,x_axis_interval="24 month",data_unit="",graph_lim=120,negative=TRUE,bar_thick=40,order_stack = TRUE)
...
##Quarterly_Category_I_AIF_net_fundraises_44
```{r}
# Amount Of Funds Raised
...
##Quarterly_Category_I_AIF_net_investments_45
```{r}
# Amount Of Investments Made
# AIF | Category I | Infrastructure Fund
...
Qtr_AIF_CI_net_inv_chart=stacked_bar_chart_niif(data_s,
                                        ...
                                        round_integer = TRUE,bar_thick=40)
...
```
#
##4W_registrations_Growth_top_cities
```{r}
#Road Construction in India
...
##Quarterly_real_GVA_growth_components_6 
...
##Quarterly_real_GDP_growth_segments_6
...
names(CD_Ratio)[2]="Credit-to-deposit ratio including merger (%)"
##
...
names(CD_Ratio_excl_hdfc)[2]="Credit-to-deposit ratio (%)"
```
...
##Monthly_Services_export_import_22
...
##India_G_sec_bench_trea_bill_yields_30_61
...
##Monthly_PMI_manufacturing_and_Services_8_13
...
##Monthly_PMI_Composite_Indices_across_major_economies_49
...
##Monthly_performance_Nifty-50_Sensex_other_global_indices_34_67
...
##Daily_market_prices_for_listed_REITs_InvITs_33
```{r include=FALSE}
#IRB InvIT
...
##Bi_monthly_median_real_GDP_projections_61
```{r}
...
gdp_trend_post=gdp_trend[gdp_trend$Relevant_Date>'2020-03-31',]
...
names(pfcf_trend)[2]='PFCF (INR trillion)'
...
my_legends_col=c("Pre pandemic trend"="GOLDEN ROD 1",
                 'Post pandemic trend'='#BF6E00',
                 'PFCF (INR trillion)'='Grey 48') 

my_line_type=c("Pre pandemic trend"="dashed",
               "Post pandemic trend"="dashed",
               'PFCF (INR trillion)'="solid")
...
pfcf_trend_chart=multi_line_chart_trend_niif(data_s,
              ...
              trend ='PFCF (INR trillion)',key_spacing=0.25)
...
##Quarterly_growth_merchandize_exports_64
...
##Quarterly_growth_merchandize_import_64
...
##Quarterly_survey_business_expectations_65
```{r}
...
##Quarterly_survey_business_cost_65
```{r}
...
#
##Quarterly_weighted_avg_lending_rates_67
```{r}
...
##Qtr_gross_non_performing_assets_67
```{r}
...
##Daily_market_prices_for_listed_new_age_companies_35
...
#
##Monthly_departure_Rainfall_map_14_25
```{r}
mysql  <- dbConnect(RMySQL:::MySQL(), dbname = DBName, host = hostname, port = portnum, user = username, password = password)
...
##Cumulative_Rainfall_departure_map_14_26
```{r}
mysql  <- dbConnect(RMySQL:::MySQL(), dbname = DBName, host = hostname, port = portnum, user = username, password = password)
...
##Cumulative_act_Rainfall_map_exp
```{r eval=FALSE, include=FALSE}
mysql  <- dbConnect(RMySQL:::MySQL(), dbname = DBName, host = hostname, port = portnum, user = username, password = password)
...
##Monthly_demand_deficit_of_power_51_94
```{r}
#DAM_Clearing_Price
...
##Monthly_silicon_silver_prices_52_101
```{r eval=FALSE, include=FALSE}
#SILVER
...
##Monthly_Credit_Deposit_Difference_18
```{r eval=FALSE, include=FALSE}
...
##Monthly_average_P/E_ratio_Nifty-50_companies_36_70
...
##Monthly_real_effective exchange_rate_INR 
```{r include=FALSE}
...
```
#
##Quarterly_outstanding_deployment_equity_69
```{r}
#MF | Equity
...
##Annual_net_FPI_investments_in_India_annual_37
```{r}
...
##Mon_net_FPI_investments_in_India_Monthly_37
```{r}
#Capital Flows | FPI | Equity
Mon_net_equi=data_query_clk_pg(id=1526330)
Mon_net_equi_src=Mon_net_equi[3][[1]]
...
#NIIF_ONTIME_CHARTS
##CEA_POWER_INSTALLED_CAPACITY_MW
```{r eval=FALSE, include=FALSE}
#Power | Wind
...
##CEA_POWER_INSTALL_CAPACITY_ADDITION_GW
...
#
##New_Company_reg_Growth_top_cities
```{r eval=FALSE, include=FALSE}
#Road Construction in India
...
##Airport_Foot_Fall_Growth_top_cities
```{r eval=FALSE, include=FALSE}
#Road Construction in India
...
my_ppt=Single_Chart("Sub title",elec_spot_pri_chart,
...
my_ppt=abdul_table_type1(
             ...
             economic_indicator_gr_st_54,
             "Header",
             paste0("Change in major economic indicators across major states (% yoy), ",format(as.Date(max_date_yoy), '%b %Y')),
...
my_ppt=abdul_table_type2(
         ...
         "Inflation",
         consumer_inflation_58,
         "Consumer inflation in June at 4.8% yoy, within RBI's target range of 4% +/-2%",
...
my_ppt=Two_Chart(
       ...
       "Median GDP growth projection for FY2025 lowered to 7.4%",
           
        paste0("Bi-monthly median real GDP projections for FY2025 by RBI (% yoy), ",bi_real_gva_gdp_title),
        "RBI's recent survey projects lower growth in FY2025",
           
        "Source: CMIE, NIIF Research",bi_real_gva_gdp_src,
        "Growth projections for FY2025 lowered as global economy expected slowdown",
...
my_ppt=Two_Chart(
      ...
      "CPI projections for FY2025 revised upwards",
...
my_ppt=Two_Chart(
       ...
       "Lending and deposit rates for scheduled commercial banks pick up",
...
#########################################################################
#************ CPI Sector inflation: Page 76 ****************************************
my_ppt=Single_Chart("Inflation",
                    Cpi_sect_chart,

                   paste0("Contribution of key components to consumer price index (CPI) inflation (% yoy/pp), ",Cpi_sect_title),
                    "Food and beverages contribute more than three-fifths of August CPI inflation, i.e., 4.2pp of 6.8% inflation",

                   Cpi_sect_src,
                   "Food inflation driving major part of consumer inflation in August",
...
my_ppt=Single_Chart("Inflation",
                   ...
                   paste0("Contribution of key components to consumer food price inflation (CPI food) (% yoy/pp), ",Cpi_com_title),
                    "Vegetables contribute 4pp of overall 9.9% food inflation in August, followed by cereals at 2.9pp, spices at 1.5pp, and milk at 1.3pp",

                   Cpi_com_src,
                   "Vegetables, cereals, spices, and milk prices driving food inflation in August",
...
my_ppt=Single_Chart("Inflation",
                   ...
                   paste0("Contribution of key components to wholesale price inflation (WPI) (% yoy/pp), ",wpi_sect_title),
                    "Manufactured products contribute (1.5)pp of (0.5%) WPI decline in August, followed by minerals at (0.8)pp; food inflation at 1.6pp",

                   wpi_sect_src,
                   "Deflation in manufactured products contributing majorly to the decline in WPI in August",
...
##########################################################################
#************ IIP Sector: Page 76 ****************************************
my_ppt=Single_Chart("High Frequency Indicators",
                    iip_sect_chart,
                  ...
                  "Manufacturing accounts for 3.5% of the 5.7% IIP growth in July, followed by mining at 1.5% and electricity at 0.6%",

                   iip_sect_src,
                   "Lower growth in manufacturing weighs on IIP",
...
my_ppt=Single_Chart("High Frequency Indicators",
                  ...
                  "Primary goods contribute 2.6% of 5.7% IIP growth in July, followed by consumer durables at 1.5% and non-durables at 1.1%",

                   iip_used_src,
                   "Primary goods and consumer durables driving IIP growth in July",
...
#####################################################################################################
#************ Real effective exchange rate/Doller index: Page 76 ****************************************
my_ppt=Two_Chart(
       ...
       "INR remains overvalued against currencies of trading partners",
       
       paste0("Monthly average dollar index (X), ",Mon_doller_indx_title),
       "Dollar index at a ten-month high",
       
       Mon_re_exch_rt_src,Mon_doller_indx_src,
       "Rupee overvalued vis-à-vis trading partner currencies; dollar strengthens against major currencies",
...
###############################################
setwd(ppt_dir)
print(my_ppt,target="NIIF_PPT_heat_maps_6.pptx")
```
...
tryCatch({
  ...
  ##--------------------------------------------------------------
  df1=data = dbGetQuery(clk,"select * from DPIIT_FDI_EQUITY_INFLOW_TOP_COUNTRY_YEARLY_DATA limit 10")
  df2= dbGetQuery(clk,"select * from DPIIT_FDI_EQUITY_INFLOW_TOP_COUNTRY_YEARLY_DATA  limit 10")
  df=rbind(df1,df2)

},error = function(e) {
    error_type = class(e)[1][[1]]
    error_msg =paste0(conditionMessage(e))
    print(error_msg)
    end_time = Sys.time()
    exe_time_sec = end_time - start_time
    val2=paste0(paste0("'",end_time,"',"),
