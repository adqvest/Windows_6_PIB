    ...
    try:
        ...
        def get_dates(cond):
            ...
            if(cond):
                ...
                dates_df = dates_df[:4] #change this later
        ...
        def main_function():
            ...
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_3W_MONTHLY_DATA where Relevant_Date>='" + date_vahan + "'"
            connection.execute(query)
            connection.execute('commit')

            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_3W_MONTHLY_STAGING where Relevant_Date>='" + date_vahan + "'", con=engine)
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [21,1,8,15,22]):#29 removed 29 as we are revising all the data
