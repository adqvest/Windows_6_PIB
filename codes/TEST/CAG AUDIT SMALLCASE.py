...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        # folder_name = 'GOI_BUDGET_ECONOMIC_SURVEY_TO_BE_CHUNKED/'
        
        # response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        # response = [ x for x in response if 'GOI_BUDGET_ECONOMIC_SURVEY_TO_BE_CHUNKED/' in x]
        # print(len(response))
        ...
        doc = client.execute('''SELECT * FROM thurro_pdf_documents_vector_db_cag_audit_reports where document_company_smallcase = '';''')
        print(len(doc))
        
        columns = client.execute('DESCRIBE TABLE thurro_pdf_documents_vector_db_cag_audit_reports')
        column_names = [col[0] for col in columns]
        # print(column_names)
        
        doc_company_index = column_names.index('document_company')
        doc_id_index = column_names.index('document_id')
        doc_embedding_index = column_names.index('embedding')
        doc_content_index = column_names.index('document_content')
        
        # doc_ids = [str(x).replace('(','').replace(',)','') for x in doc_ids]
        # print(len(doc_ids))
        
        for row in doc:
            chunk = row[doc_content_index]  
            company = row[doc_company_index]
            doc_id = row[doc_id_index]
            emb = row[doc_embedding_index]
            # print(chunk) 
            # print(company)
            ...
            #                     connection.execute("update INDIA_BUDGET_ECONOMIC_SURVEY_YEARLY_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
            ...
            #             connection=engine.connect()
            #             connection.execute("update INDIA_BUDGET_ECONOMIC_SURVEY_YEARLY_DATA_CORPUS set Chunking_Status='Issues' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
            #             connection.execute("commit")
            #             continue
            ...
            company_name_small = company.lower()  
            print(doc_id)                                
            # try:
            ...
            print(len(chunk_small))  

            chunk_small = chunk_small.replace("\\", "\\\\")  # Escaping backslashes
            chunk_small = chunk_small.replace("\n", "\\n")  # Escaping newlines
            chunk_small = chunk_small.replace("\r", "\\r")  # Escaping carriage returns
            chunk_small = chunk_small.replace("'","\\'") # Escaping single quotes
            # chunk_small = chunk_small.replace("&","and")
            ...
            # client.execute(f"INSERT INTO thurro_pdf_documents_vector_db_cag_audit_reports (document_company_smallcase,document_content_smallcase,embedding_smallcase) VALUES ('{company_name_small}',{chunk_small},{chunk_embed_small}) where document_content = {chunk};") 
            # print(f"ALTER TABLE thurro_pdf_documents_vector_db_cag_audit_reports UPDATE document_company_smallcase = '{company_name_small}', document_content_smallcase = '{chunk_small}', embedding_smallcase = {chunk_embed_small} WHERE document_content = '{chunk}' and document_id = '{doc_id}';")
            from clickhouse_driver import Client
            host = 'ec2-52-88-156-240.us-west-2.compute.amazonaws.com'
            user_name = 'default'
            password_string = 'Clickhouse@2024'
            ...
            client.execute(f"ALTER TABLE thurro_pdf_documents_vector_db_cag_audit_reports UPDATE document_company_smallcase = '{company_name_small}', document_content_smallcase = '{chunk_small}', embedding_smallcase = {chunk_embed_small} WHERE document_content = '{chunk}' and document_id = {doc_id};")
            

                # connection=engine.connect() 
                # connection.execute(text("UPDATE INDIA_BUDGET_ECONOMIC_SURVEY_YEARLY_DATA_CORPUS SET Chunking_Status='Done' WHERE File_Name = :file_name"),{"file_name": file_name})
                # connection.commit()
        ...
        # connection = engine.connect()
        # query = "select * from AdqvestDB.INDIA_BUDGET_ECONOMIC_SURVEY_YEARLY_DATA_CORPUS ORDER BY File_ID ASC;"
        # data = pd.read_sql(query, con=engine)

        # doc = client.execute('''SELECT distinct document_id as id , page_number as pn FROM INDIA_BUDGET_ECONOMIC_SURVEY_YEARLY_DATA_CORPUS_CHUNKED where document_link = '' ;''')
        # len(doc)
        ...
        #         client.execute(f'''ALTER TABLE INDIA_BUDGET_ECONOMIC_SURVEY_YEARLY_DATA_CORPUS_CHUNKED UPDATE document_link = '{updated_link}' WHERE document_id = '{document_id}' and page_number = '{page}' ;''')
        #         client.execute(f'''ALTER TABLE INDIA_BUDGET_ECONOMIC_SURVEY_YEARLY_DATA_CORPUS_CHUNKED UPDATE vector_db_status = 'LINK UPDATED' WHERE document_id = '{document_id}';''')
        #         print("COMPLETED LOOP:",count)
        #         count += 1
            
        # if count % 900 == 0:
        #     print("OPTIMIZING DATA")
        #     client.execute('''OPTIMIZE TABLE INDIA_BUDGET_ECONOMIC_SURVEY_YEARLY_DATA_CORPUS_CHUNKED FINAL;''')        
