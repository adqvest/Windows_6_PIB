    ...
    try:
        ...
        for obj in response:
            ...
            try:
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute("update ICRA_DAILY_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update ICRA_DAILY_DATA_CORPUS set Chunking_Status='ISSUE' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
                        connection.execute("commit")
                        continue
                ...
                connection=engine.connect()
                connection.execute("update ICRA_DAILY_DATA_CORPUS set Chunking_Status='ISSUE' where Generated_File_Name = '" +str(file_name)+"'")
                connection.execute("commit")
