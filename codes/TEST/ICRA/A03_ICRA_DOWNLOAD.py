# -*- coding: utf-8 -*-
"""
Created on Tue Mar 25 10:06:10 2025
...
def upload_to_s3(url,filename):
    ...
    os.chdir(r'C:\Users\Administrator\AdQvestDir\Junk_ICRA')
    print(url)
    r = requests.get(url)
    ...
    if r.status_code == 200:
        with open('C:/Users/Administrator/AdQvestDir/Junk_ICRA/'+filename, 'wb') as file:
            file.write(r.content)
            print("PDF downloaded successfully.")
            
        file_size = os.path.getsize('C:/Users/Administrator/AdQvestDir/Junk_ICRA/'+filename)
        ...
        if file_size > size_threshold:
            ...
            print('Done for ICRA_CORPUS')
            
            key_2='ICRA_CORPUS_TO_BE_CHUNKED/'
            s3 = boto3.resource('s3', aws_access_key_id=ACCESS_KEY_ID, aws_secret_access_key=ACCESS_SECRET_KEY,
                config=Config(signature_version='s3v4', region_name='ap-south-1')
                )
            data_s3 =  open(filename, 'rb')
            try:
                ...
                print("Credentials not available")
            print('Done for ICRA_CORPUS_TO_BE_CHUNKED')
            return 'Done'
        else:
            return 'SIZE ISSUE'
    else:
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        links_df = pd.read_sql("select * from ICRA_DAILY_DATA_CORPUS_NEW where Chunking_Status is Null;",engine)
        ...
        for x in range(len(links_df)):
            ...
            print(file_name)
            filepath = os.path.join(r'C:\Users\Administrator\AdQvestDir\Junk_ICRA', file_name)
            print(filepath)
            file_url = str(links_df.loc[x]["Links"])
            try:
            ...
            try:
                if status_val=='Done':
                    query = f'Update ICRA_DAILY_DATA_CORPUS_NEW set S3_Upload_Status="Done" where Generated_File_Name="{links_df.loc[x]["Generated_File_Name"]}" and File_ID={int(links_df.loc[x]["File_ID"])}'

                    connection.execute(query)
                    time.sleep(2)
                    print('Done updates')
                else:
                    query = f'Update ICRA_DAILY_DATA_CORPUS_NEW set S3_Upload_Status="{status_val}" where Generated_File_Name="{links_df.loc[x]["Generated_File_Name"]}" and File_ID={int(links_df.loc[x]["File_ID"])}'
