# -*- coding: utf-8 -*-
"""
Created on Tue Mar 25 09:48:53 2025
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        links_df = pd.read_sql("select * from CARE_RATINGS_DAILY_DATA_CORPUS_NEW where Chunking_Status = 'File Issue' or Chunking_Status is Null;",engine)
        # print(len(links_df))
        links_df = links_df.reset_index(drop=True)
        links_df = links_df[601:]
        print(len(links_df))
        ...
        for x in range(len(links_df)):
            ...
            try:
                if status_val=='Done':
                    query = f'Update CARE_RATINGS_DAILY_DATA_CORPUS_NEW set S3_Upload_Status="Done" where Generated_File_Name="{links_df.loc[x]["Generated_File_Name"]}" and File_ID={int(links_df.loc[x]["File_ID"])}'

                    connection.execute(query)
                    time.sleep(2)
                    print('Done updates')
                else:
                    query = f'Update CARE_RATINGS_DAILY_DATA_CORPUS_NEW set S3_Upload_Status="{status_val}" where Generated_File_Name="{links_df.loc[x]["Generated_File_Name"]}" and File_ID={int(links_df.loc[x]["File_ID"])}'
