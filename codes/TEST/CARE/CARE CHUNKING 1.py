...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        doc_ids = client.execute('''SELECT distinct document_id FROM CARE_DAILY_DATA_CORPUS_CHUNKED_1;''')
        ...
        for obj in response:
            ...
            try:
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute("update CARE_RATINGS_DAILY_DATA_CORPUS_NEW set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update CARE_RATINGS_DAILY_DATA_CORPUS_NEW set Chunking_Status='Issues' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
                        connection.execute("commit")
                        continue
                ...
                delete_file_from_s3(obj)
                connection=engine.connect() 
                connection.execute(text("UPDATE CARE_RATINGS_DAILY_DATA_CORPUS_NEW SET Chunking_Status='File Issue' WHERE Generated_File_Name = :file_name"),{"file_name": file_name})
        
        # connection = engine.connect()
        # query = "select * from AdqvestDB.CARE_RATINGS_DAILY_DATA_CORPUS_NEW ORDER BY File_ID limit 13000;"
        # data = pd.read_sql(query, con=engine)

        # doc = client.execute('''SELECT distinct document_id as id , page_number as pn FROM CARE_DAILY_DATA_CORPUS_CHUNKED_1 where document_link = '' ;''')
        # len(doc)
        ...
        #         client.execute(f'''ALTER TABLE CARE_DAILY_DATA_CORPUS_CHUNKED_1 UPDATE document_link = '{updated_link}' WHERE document_id = '{document_id}' and page_number = '{page}' ;''')
        #         client.execute(f'''ALTER TABLE CARE_DAILY_DATA_CORPUS_CHUNKED_1 UPDATE vector_db_status = 'LINK UPDATED' WHERE document_id = '{document_id}';''')
        #         print("COMPLETED LOOP:",count)
        #         count += 1
            
        # if count % 900 == 0:
        #     print("OPTIMIZING DATA")
        #     client.execute('''OPTIMIZE TABLE CARE_DAILY_DATA_CORPUS_CHUNKED_1 FINAL;''')        
