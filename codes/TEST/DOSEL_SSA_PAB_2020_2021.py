...
links = links_df[links_df.Financial_Year.str.lower().str.contains('2020-2021')]
# links = links[~(links.State.str.lower().str.contains('Lakshadweep|uttar pradesh|tamil Nadu|sikkim|punjab|puducherry|odisha|nagaland|ncpcr|ncert'))]
...
for row, vals in links.iloc[:, :].iterrows():
    ...
    try:
        query = f'Insert into DOSEL_SAMAGRA_SHIKSHA_DATA_COLLECTION_STATUS value ("{vals.State.strip()}", null, null, null, null, null, null, null, null, "{str(vals.Link.strip())}","{rel_date}", "{runtime.strftime("%Y-%m-%d %H:%M:%S")}")'
    ...
    # break
        ...
        query = f'Insert into DOSEL_SAMAGRA_SHIKSHA_DATA_COLLECTION_STATUS value ("{vals.State.strip()}", null, null, null, null, null, null, null, null, null ,"{rel_date}", "{runtime.strftime("%Y-%m-%d %H:%M:%S")}")'
        print(query)
        connection.execute(query)
        connection.execute('commit')
    tables = []
    try:
        tables = camelot.read_pdf(vals.Link, pages = '1-300', strip_text = '\n', shift_text = [])
    except:
        try:
