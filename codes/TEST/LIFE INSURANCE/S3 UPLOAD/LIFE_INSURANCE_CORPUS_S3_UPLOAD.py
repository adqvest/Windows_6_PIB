...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        path=r'C:/Users/Administrator/AdQvestDir/LIFE_INSURANCE_1/'

        df_links=pd.read_sql("select * from AdqvestDB.LIFE_INSURANCE_QUARTERLY_DATA_CORPUS where Is_PDF is not Null and S3_Upload = 'Not Downloaded because of Requests'",engine)
        print(len(df_links))
        df_links['Year'] = df_links['Year'].apply(validate_year)

        df_links=df_links[:6000]
        print(len(df_links))
        import time
        time.sleep(10)
        ...
        for i, row in df_links.iterrows():
            ...
            try:
                ...
                session.headers.update({
                    ...
                    'X-Browser-Year': '2025',
                    'X-Client-Data': 'CJO2yQEIorbJAQipncoBCL+XywEIlKHLAQj/ossBCIWgzQEIxdvOAQjV284BCJDfzgEY9MnNAQ==',
                ...
                if response.status_code == 200:
                    ...
                    if 'application/pdf' in content_type:
                     # if 'application/pdf' or 'text/plain' in content_type:    
                        ...
                        # query1 = "UPDATE LIFE_INSURANCE_QUARTERLY_DATA_CORPUS_RUN_FINAL_GOKUL SET File_ID = %s WHERE Url = %s"
                        # connection.execute(query1, (num, link))
                        
                        # query2 = "UPDATE LIFE_INSURANCE_QUARTERLY_DATA_CORPUS_RUN_FINAL_GOKUL SET Doc_Number = %s WHERE Url = %s"
                        # connection.execute(query2, (doc, link))
                        
                        # query3 = "UPDATE LIFE_INSURANCE_QUARTERLY_DATA_CORPUS_RUN_FINAL_GOKUL SET Report_Quarter = %s WHERE Url = %s"
                        # connection.execute(query3, (report_quarter, link))
                        
                        # query4 = "UPDATE LIFE_INSURANCE_QUARTERLY_DATA_CORPUS_RUN_FINAL_GOKUL SET Report_Type = %s WHERE Url = %s"
                        # connection.execute(query4, (report_type, link))
                        
                        # query5 = "UPDATE LIFE_INSURANCE_QUARTERLY_DATA_CORPUS_RUN_FINAL_GOKUL SET Company = %s WHERE Url = %s"
                        # connection.execute(query5, (company, link))
                        
                        # query6 = "UPDATE LIFE_INSURANCE_QUARTERLY_DATA_CORPUS_RUN_FINAL_GOKUL SET Is_PDF= %s WHERE Url = %s"
                ...
                #     print(f"Skipping file. The link does not point to a PDF. Content-Type: {content_type}")
                        ...
                        connection=engine.connect()
                        query = """ UPDATE LIFE_INSURANCE_QUARTERLY_DATA_CORPUS SET S3_Upload = 'EXCEL' WHERE Url = %s """
                        connection.execute(query, (link,))
                        connection.execute("commit")
                        ...
                        connection=engine.connect()
                        query = """ UPDATE LIFE_INSURANCE_QUARTERLY_DATA_CORPUS SET S3_Upload = 'HTML' WHERE Url = %s """
                        connection.execute(query, (link,))
                        connection.execute("commit")
