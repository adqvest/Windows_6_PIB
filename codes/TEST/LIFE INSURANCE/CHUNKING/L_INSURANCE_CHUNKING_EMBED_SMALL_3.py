...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'LIFE_INSURANCE_CORPUS_TO_BE_CHUNKED_9'
        ...
        response = [ x for x in response if 'LIFE_INSURANCE_CORPUS_TO_BE_CHUNKED_9/' in x]
        
        response = response[12001:18000]
        print(len(response))
        
        doc_ids = client.execute(f'''SELECT distinct document_id FROM thurro_pdf_documents_vector_db_life_insurance_corpus_final_3;''')
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute("update LIFE_INSURANCE_QUARTERLY_DATA_CORPUS set Chunk_Status='Not Relevant' where Cleaned_File_Name = '" +str(file_name_without_extension)+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update LIFE_INSURANCE_QUARTERLY_DATA_CORPUS set Chunk_Status='Issues' where Cleaned_File_Name = '" +str(file_name_without_extension)+"'")
                        connection.execute("commit")
                        continue
        ...
        # query = "select * from AdqvestDB.LIFE_INSURANCE_QUARTERLY_DATA_CORPUS ORDER BY Company ASC;"
