...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS,ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        for obj in response:
            ...
            try:
                ...
                try:
                    ...
                    connection.execute("update BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA set Chunking_Status='ISSUE' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection=engine.connect()
                                connection.execute("update BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name)+"'")
                                connection.execute("commit")
                                continue      
                        ...
                        connection=engine.connect()
                        connection.execute("update BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA set Chunking_Status='ISSUE' where File_Name = '" +str(file_name)+"'")
                        connection.execute("commit")
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                client.execute(f"INSERT INTO BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED (document_id,document_company,document_company_small_case,document_type,document_year,document_date,page_number,document_content,document_content_small_case,document_content_modify,embedding,embedding_small_case) VALUES ('{file_id}','{company_name}','{company_name_small}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''','''{chunk_small}''','''{new_chunk}''',{chunk_embed},{chunk_embed_small});") 
                                connection=engine.connect()
                                connection.execute("update BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA set Chunking_Status='Done' where File_Name = '" +str(file_name)+"'")
                                connection.execute("commit")
                            except:
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA ORDER BY File_ID ASC;"
        ...
        if len(doc) > 0:
            ...
            for x in range(len(data)):
                doc_id = int(float(data["File_ID"][x]))
            ...
            for batch_num in range(total_batches):
                ...
                # Build a CASE statement for document_link updates
                case_parts = []
                for doc_id, link in batch:
                    case_parts.append(f"WHEN document_id = {doc_id} THEN '{link}'")
                
                case_statement_link = "CASE " + " ".join(case_parts) + " ELSE document_link END"
                
                # Collect document IDs for the WHERE clause
                doc_ids = ", ".join([str(doc_id) for doc_id, _ in batch])
            ...
            query = """
                    ...
                    WHERE document_link NOT LIKE '%#page=%'
                    AND document_link LIKE '%.pdf%' 
        ...
        # connection = engine.connect()
        # query = "select * from AdqvestDB.ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS ORDER BY File_ID ASC;"
        # data = pd.read_sql(query, con=engine)

        # doc = client.execute('''SELECT distinct document_id,page_number FROM ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED WHERE document_link = '';''')
        # len(doc)
        ...
        #     document_link = data["Links"][x]
            
        #     client.execute(f'''ALTER TABLE ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED UPDATE document_link = '{document_link}' WHERE document_id = '{document_id}' ;''')
        #     client.execute(f'''ALTER TABLE ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED UPDATE vector_db_status = 'LINK UPDATED' WHERE document_id = '{document_id}';''')
        #     print("COMPLETED LOOP:",count)
        #     count += 1
            
        #     if count % 900 == 0:
        #         print("OPTIMIZING DATA")
        #         client.execute('''OPTIMIZE TABLE ANAROCK_MARKET_VIEWPOINTS_MONTHLY_DATA_CORPUS_CHUNKED FINAL;''')
