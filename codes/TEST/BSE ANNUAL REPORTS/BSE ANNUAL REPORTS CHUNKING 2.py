# -*- coding: utf-8 -*-
"""
Created on Thu Apr  3 11:16:01 2025
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS,BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        folder_name = 'BSE_ANNUAL_REPORTS_CORPUS_TO_BE_CHUNKED_NEW'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'BSE_ANNUAL_REPORTS_CORPUS_TO_BE_CHUNKED_NEW/' in x]
        
        import re

        response = [x for x in response if re.search(r'FY16', x)]
        response = response[250:450]

        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED;''')
            ...
            try:
                ...
                try:
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS SET Chunking_Comments = 'File Not Available' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    elif 'Failed to load page' in str(e):
                        query = f"UPDATE BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS SET Chunking_Comments = 'Corrupt Document' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    else:
                        query = f"UPDATE BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    # connection.commit()
                    time.sleep(2)
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 16 and int(relevant_year[5:]) < 30:
                                ...
                                connection=engine.connect()
                                connection.execute("update BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name)+"'")
                                connection.execute("commit")
                                continue      
                        elif int(relevant_year[2:]) >= 16 and int(relevant_year[2:]) < 30:
                                company_name = company_name.replace("'", "\\'")  # Escaping single quotes
                                document_date = parse_quarter_year(relevant_year)
                        else:
                            print("FILE NOT RELEVANT ") 
                        ...
                        connection=engine.connect()
                        connection.execute("update BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '" +str(file_name)+"'")
                        connection.execute("commit")
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                # client.execute(f"INSERT INTO BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED (Document_Id,Chunk_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.str.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{datetime.datetime.now(india_time)}');")   
                                client.execute(f"INSERT INTO BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")
                                connection=engine.connect()
                                connection.execute("update BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS set Chunking_Status='Done' where File_Name = '" +str(file_name)+"'")
                                connection.execute("commit")
                            except:
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM BSE_ANNOUNCEMENT_ANNUAL_REPORT_COMPANY_WISE_YEARLY_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
