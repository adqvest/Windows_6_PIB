# -*- coding: utf-8 -*-
"""
Created on Thu Mar 27 08:38:50 2025
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):    
    ...
    try:
        ...
        query = "SELECT * FROM CRISIL_DAILY_DATA_CORPUS_NEW where Chunking_Status in ('Not Relevant','Check File');"
        raw_df = pd.read_sql(query, con=engine)
        raw_df = raw_df[900:1700].reset_index(drop =True)
        print(len(raw_df))
            ...
            try:
                ...
                if b'file not found' in r.content.lower():
                    ...
                    with engine.begin() as connection:  # Automatically handles commit/rollback
                        query = text("""
                            ...
                            SET S3_Upload_Status = 'File Not Available' 
                ...
                try:
                    ...
                    with engine.begin() as connection:  # Automatically handles commit/rollback
                        query = text("""
                            UPDATE CRISIL_DAILY_DATA_CORPUS_NEW 
                            SET Moved_And_Deleted = 'Move and Delete Done' 
                            WHERE Rating_File_Link = :link 
                            AND Rating_File_Name = :old 
                            AND File_ID = :id 
                            AND Generated_File_Name = :new
                        ...
                        result = connection.execute(query, {"link": link, "old": old_file_name, "id": file_id,"new": new_file_name,})
                        print("Rows updated as Moved and Deleted:", result.rowcount)  # To check if the update worked
                    ...
                    with engine.begin() as connection:  # Automatically handles commit/rollback
                        query = text("""
                            UPDATE CRISIL_DAILY_DATA_CORPUS_NEW 
                            SET S3_Upload_Comments = 'Download Failed' 
                            WHERE Rating_File_Link = :link 
                            AND Rating_File_Name = :old 
                            AND File_ID = :id 
                            AND Generated_File_Name = :new
