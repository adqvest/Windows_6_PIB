...
def data_collection(table_name,table_name2,table_name3):
    ...
    driver.get('https://nhai.gov.in/#/project-informations')
    robot.add_link('https://nhai.gov.in/#/project-informations')
    time.sleep(5)
    driver.find_element("xpath","//a[text()='English']").click()
    ...
    if file_available == True:
        ...
        dbfunctions.to_clickhouse(table_name)
        ...
        if(month != 4):
            ...
            dbfunctions.to_clickhouse(table_name3)
        else:
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try :
        ...
        logfunctions.start_log(run_by,table_name2,py_file_name,job_start_time,scheduler)
        session = requests.Session()
        
        data_collection(table_name,table_name2,table_name3)

        log.job_end_log(table_name,job_start_time, no_of_ping)
        log.job_end_log(table_name2,job_start_time, no_of_ping)

    except:
        logfunctions.error_log(table_name,job_start_time, no_of_ping)
        logfunctions.error_log(table_name2,job_start_time, no_of_ping)
