...
def extract_text_from_pdf(files_content,file_name):
    ...
    for page_number in range(len(pdf)):
        ...
        text_content += '\n\n'
    text_content = re.sub('\n\n\n','\n\n', text_content)
    return company_name,report_type.upper(),relevant_year,file_id,text_content  
    
def extract_text_from_html(files_content, file_name):
    ...
    formatted_text += f'REPORT TYPE: {report_type.upper().strip()}\n\n'
    ...
    return company_name, report_type.upper(), relevant_year, file_id, formatted_text
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):    
    ...
    table_name = 'TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS,TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        folder_name = 'TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_TO_BE_CHUNKED/'

        query = "select * from TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS where Chunking_Status is Null and S3_Upload_Status = 'Done'"
        transunion_reports_df = pd.read_sql(query,con=engine)

        doc_links = client.execute('''SELECT distinct Document_Link FROM TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED;''')
        doc_links = [link for (link,) in doc_links]
        print(len(doc_links))

        doc_ids = client.execute('''SELECT distinct Document_Id FROM TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED;''')
        doc_ids = [str(row[0]).strip().strip("'") for row in doc_ids if row[0] is not None]
        print(len(doc_ids))

        print("Setting up S3 client...")
        ACCESS_key_ID = 'AKIAYCVSRU2U3XP2DB75'  # Your access key
        ACCESS_SECRET_KEY = '2icYcvBViEnFyXs7eHF30WMAhm8hGWvfm5f394xK' # Your secret key
        s3 = boto3.client('s3', aws_access_key_id=ACCESS_key_ID, aws_secret_access_key=ACCESS_SECRET_KEY, 
                          config=Config(signature_version='s3v4', region_name='ap-south-1'))
        # ---------------------------------------------------------

        for i in range(len(transunion_reports_df)):
            source_name = transunion_reports_df['Source'][i]
            link = transunion_reports_df['File_Link'][i]
            table_file_id = transunion_reports_df['File_ID'][i]
            report_name = transunion_reports_df['Report_Name'][i]
            base_file_name = transunion_reports_df['File_Name'][i] 
            # This 'new_file_name' line was removed as it was causing an error. We define it later.
            rel_Date = transunion_reports_df['Relevant_Date'][i]
            year = transunion_reports_df['Relevant_Date'][i].year
            month = transunion_reports_df['Relevant_Date'][i].month
            ...
            # --- START: NEW LOGIC TO FIND THE CORRECT FILE IN S3 ---
            new_file_name = None # This will store the full filename with extension
            for ext in ['.pdf', '.html']:
                potential_file_name = base_file_name + ext
                try:
                    # Use a lightweight check to see if the file exists
                    s3.head_object(Bucket='adqvests3bucket', Key=folder_name + potential_file_name)
                    new_file_name = potential_file_name
                    print(f"Found file in S3: {new_file_name}")
                    break  # Exit the loop once the file is found
                except Exception:
                    continue # If not found, try the next extension

            if not new_file_name:
                # If neither .pdf nor .html was found, log an error and skip
                error_msg = f"File '{base_file_name}' not found in S3 with .pdf or .html extension."
                print(error_msg)
                error_msg_sql = error_msg.replace("'", "''") # Make error safe for SQL
                query = f"UPDATE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS SET Chunking_Comments = '{error_msg_sql}' WHERE File_ID = '{table_file_id}'"
                connection.execute(sql_text(query))
                continue # Go to the next file in the main loop
            # --- END: NEW LOGIC ---

            if(link not in doc_links):                
                try:
                    # --- MODIFIED: Pass the full, correct new_file_name and the bucket name ---
                    files_content,file_name = read_all_file_content(folder_name, new_file_name, bucket_name='adqvests3bucket')
                    
                except Exception as e:
                    print(e)
                    # --- MODIFIED: Sanitize the error and use the base_file_name for the query ---
                    error_message_for_sql = str(e).replace("'", "''")
                    base_file_name_sql = base_file_name.replace("'", "''")
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS SET Chunking_Comments = 'File Not Available' WHERE Relevant_Date= '{rel_Date}' and File_Name = '{base_file_name_sql}'"
                        print(query)
                        connection.execute(query)
                    else:
                        query = f"UPDATE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS SET Chunking_Comments = '{error_message_for_sql}' where Relevant_Date= '{rel_Date}' and File_Name = '{base_file_name_sql}'"
                        print(query)
                        connection.execute(query)
                    time.sleep(2)
                    continue
                ...
                try:
                    try:
                        ...
                        if('.html' in file_name):
                            company_name,report_type,relevant_year,file_id,text_content = extract_text_from_html(files_content,file_name)
                    except Exception as e:
                        print(e)
                        if 'Failed to load document (PDFium: Data format error)' in str(e):
                            query = f"UPDATE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS SET Chunking_Comments = 'File Not Available' WHERE Relevant_Date= '{rel_Date}' and File_Name = '{new_file_name}'"
                            print(query)
                            connection.execute(query)
                        elif 'Failed to load page' in str(e): 
                            query = f"UPDATE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS SET Chunking_Comments = 'Corrupt Document' WHERE Relevant_Date= '{rel_Date}' and File_Name = '{new_file_name}'"
                            print(query)
                            connection.execute(query)   
                        else:
                            query = f"UPDATE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' where Relevant_Date= '{rel_Date}' and File_Name = '{new_file_name}'"
                            print(query)
                            connection.execute(query)
                        time.sleep(2)
                        continue
                    ...
                    if str(file_id) not in doc_ids :
                        ...
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 10 and int(relevant_year[5:]) < 30:
                                ...
                                connection = engine.connect()
                                update_query = f"update TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS set Chunking_Comments = 'Not Relevant' where File_Link = '{link}' and File_Name = '{file_name}' and Relevant_Date = '{transunion_reports_df['Relevant_Date'][i]}'"
                                connection.execute(sql_text(update_query))
                                print('Table Updated as Done')
                                #delete_file_from_s3(file_name)
                                print('_' * 80)
                                continue
                        elif int(relevant_year[3:])>=10 and int(relevant_year[3:])<30:
                            company_name = company_name.replace("'","\\'") # Escaping single quotes
                        ...
                        if (len(extracted_chunks)>0):
                            ...
                            try:
                                # --- Prepare all the data variables for the INSERT statement ---
                                chunk_id = f"{file_id}_{page}"
                                
                                # Safely escape the link for the query
                                safe_link = link.replace("'", "''") 
                                
                                # Get the scraped timestamp from the dataframe
                                runtime_scraped = transunion_reports_df['Runtime'][i]

                                # Format the publish date from the dataframe's Relevant_Date
                                publish_date = str(rel_Date) + ' 12:00:00'

                                # Create the embeddings
                                chunk_embed = str(create_embeddings(chunk)).replace('[[','[').replace(']]',']')
                                
                                # --- This is the new, corrected INSERT statement matching your schema ---
                                client.execute(f"""
                                    INSERT INTO TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED (
                                        Document_Id,
                                        Chunk_Id,
                                        Document_Company,
                                        Document_Type,
                                        ...
                                        '{relevant_year}',
                                        '{document_date}',
                                        toDateTime('{publish_date}'),
                                        '{safe_link}',
                                        {page},
                                        '''{chunk}''',
                                        {chunk_embed},
                                        'CHUNKED',
                                        toDateTime('{runtime_scraped}'),
                                        '{timestamp}'
                                    );
                                """)
                                
                                print(f'Page {page} for Document {file_id} inserted into Clickhouse.')
                                
                                connection = engine.connect()                
                                base_file_name_sql = base_file_name.replace("'", "''")
                                update_query = f"""
                                    UPDATE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS 
                                    SET Chunking_Status = 'Done' 
                                    WHERE File_Name = '{base_file_name_sql}'
                                      AND Relevant_Date = '{rel_Date}'
                                """
                                connection.execute(sql_text(update_query))
                                print('MySQL Table Updated to Done.')
                                ...
                                print(f"****** PAGE {page_number} WAS SKIPPED DUE TO AN ERROR ******")
                        ...
                        update_query = f"""
                                        UPDATE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS 
                                        SET Chunking_Status = 'Done' 
                                        WHERE File_Link = '{link.replace("'", "''")}' 
                                          AND File_Name = '{file_name}' 
                                          AND Relevant_Date = '{transunion_reports_df['Relevant_Date'][i]}'
                                    """
                ...
                connection = engine.connect()
                update_query = f"update TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS set Chunking_Status = 'Done' where File_Link = '{link}' and File_ID = '{table_file_id}' and Relevant_Date = '{transunion_reports_df['Relevant_Date'][i]}'"
                connection.execute(sql_text(update_query))
                print('Table Updated as Done')
                print('_' * 80) 
                continue
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM TRANSUNION_CIBIL_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED where Published_Date IS Null ;''')
        print(len(doc))
