...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'FRC_GUJARAT_DIST_SCHOOL_WISE_FEE_STRUCT_YEARLY_DATA_ALL'
    ...
    try:
        ...
        # districts = [i.text for i in soup.find('select', {'id':'currentDistrict'}).findAll('option')][1:]
        districts = pd.read_sql('SELECT District from FRC_GUJARAT_DIST_SCHOOL_WISE_FEE_STRUCT_YEARLY_DATA_STATUS_ALL WHERE Status is null', con = engine)['District']
        final_fee_df = pd.DataFrame()
        # districts = ['The Dangs'] ### Remove at 10 pm
        for dist in districts:
            try:
                ...
                print(f'No Of Records collected: {len(df_denorm.School_Number.unique())}')
                status_df = pd.DataFrame({'District':[dist], 'Total_Records':[total_entries], 'Last_Collected_Page':[pg], 'Records_Collected': len(df_denorm.School_Number.unique()), 'Status':[coll_status], 'Relevant_Date':[today.date()], 'Runtime':[datetime.datetime.now(india_time)]})
                df_denorm.to_sql('FRC_GUJARAT_DIST_SCHOOL_WISE_FEE_STRUCT_YEARLY_DATA_ALL', if_exists='append', index=False, con = engine)
                query = f'UPDATE FRC_GUJARAT_DIST_SCHOOL_WISE_FEE_STRUCT_YEARLY_DATA_STATUS_ALL SET Total_Records = {status_df.Total_Records[0]}, Last_Collected_Page = {status_df.Last_Collected_Page[0]}, Records_Collected = {status_df.Records_Collected[0]}, Status = "{status_df.Status[0]}", Relevant_Date = "{status_df.Relevant_Date[0]}", Runtime = "{status_df.Runtime[0]}" WHERE District = "{status_df.District[0]}"'
                connection.execute(query)
            except Exception as e:
                error = str(e)
                print('Failed for ---> ', dist, 'error ', error)
                query = f'UPDATE FRC_GUJARAT_DIST_SCHOOL_WISE_FEE_STRUCT_YEARLY_DATA_STATUS_ALL SET Total_Records = Null, Last_Collected_Page = Null, Records_Collected = Null, Status = "Failed", Relevant_Date = "{today.date()}", Runtime = "{datetime.datetime.now(india_time)}" WHERE District = "{dist}"'
                connection.execute(query)
