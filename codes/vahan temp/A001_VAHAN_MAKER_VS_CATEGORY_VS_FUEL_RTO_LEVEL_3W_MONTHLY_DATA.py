...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA'
    ...
    try:
        ...
        def click_submit(driver):
            ...
            while True:
                try:
                    ...
                    element = driver.find_element(By.XPATH,"//*[@class = 'button-section']/button[@type = 'submit']")
        ...
        def get_dates(cond):
            ...
            distinct_date = [datetime.date(2021,7,31)]


            #un comment the following lines aafter data collection

            query = "select min(Relevant_Date) as Min FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS"
            min_date = pd.read_sql(query, engine)
            min_date = min_date["Min"][0]

            distinct_date.append(min_date)

            min_date=datetime.date(2024,7,31)
            ...
            dates_df = pd.DataFrame({"Relevant_Date":[min_date]}) #comment out this line after data collection
            ...
            ''' Uncomment the below 2 lines after manual data collection''' 
        ...
        def data_check_step1():
            rto_df = pd.read_sql("Select  State, Sum(Total) as RTO_Total, Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING group by State, Relevant_Date order by State",engine)

            state_df = pd.read_sql("Select  State, Sum(Total) as State_Total, Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_3W_MONTHLY_DATA  group by State, Relevant_Date order by State",engine)
        ...
        def data_check():
            ...
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING  group by RTO_Office_Raw,Relevant_Date",engine)
            ...
            if(error_df.empty):
                ...
                for index,i in error_df.iterrows():
                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                    connection.execute("commit")

                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS set State_Status = Null where State = '" +i["State"] +"'")
                    connection.execute("commit")


                    connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS", con=engine)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS", con=engine, index=False, if_exists='append')
                    ...
                    for st in state_info:
                        ...
                        for x in rto_info.keys():
                            ...
                            for i in range(len(dates_df)):
                                ...
                                for vc in vehicle_class_info.items():
                                    driver.find_element(By.XPATH,"//*[@id='VhClass']/tbody/tr["+str(vc[1])+"]/td/label").click()
                                    print('loop begins')
                                    ...
                                    for jk in other_fuels:
                                        '''here'''
                                        # element = driver.find_element(By.XPATH,"//*[@id='fuel']/tbody/tr["+str(jk)+"]/td/label")
                                        # driver.execute_script("arguments[0].click();", element)
                            ...
                            vahan_maker_cat_fuel_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING", con=engine, index=False, if_exists='append')


                            connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS set RTO_Status = 'Done' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")

                            connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS set Runtime = '"+datetime.datetime.now(india_time).strftime("%Y-%m-%d %H:%M:%S")+"' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")
                        ...
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
            ...
            data_check_step1()        
            data_check()
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS"
            connection.execute(query)
            connection.execute('commit')

            #maker vs category
            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING", con=engine)
            ...
            ''' Uncomment thes 3 lines below after data collection'''

            # query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA where Relevant_Date>='" + date_vahan + "'"  #Uncomment these 3 lines
            # connection.execute(query)
            # connection.execute('commit')


            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING where Relevant_Date>='" + date_vahan + "'", con=engine)
                df.drop_duplicates(subset=['Maker', 'Maker_Clean', 'Total', 'Vehicle_Category', 'Vehicle_Class',
                                   'Fuel', 'State', 'RTO_Office_Raw', 'RTO_Date', 'RTO_Code', 'RTO_Office',
                                   'Relevant_Date'], inplace = True)
                df['Fuel_Clean']=df['Fuel']
                df['Fuel_Clean']=df['Fuel_Clean'].apply(lambda x:x.replace("ELECTRIC(BOV)","ELECTRIC").replace("PURE EV","ELECTRIC"))
                df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA', con=engine, index=False, if_exists='append')
                
                #ClickHouse

            except:
                print('ERROR 3')
                raise Exception('error in VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING')


            query = "drop table VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING"
            connection.execute(query)
            connection.execute('commit')


        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING')"
        ...
        query = "SELECT * FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA_STATUS"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            max_date=pd.read_sql('select max(Relevant_Date) as max from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA',engine)['max'][0]
            
            if (today.day in [2] or (today.date()-max_date).days>=31):   #change it to 2 after this data collection
                print('B')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_3W_MONTHLY_DATA_STAGING select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_3W_MONTHLY_DATA limit 0"
                connection.execute(query)
                connection.execute('commit')
