...
def run_program(run_by='Adqvest_Bot', py_file_name=None):    
    ...
    table_name = 'CRISIL_DAILY_DATA_CORPUS,CRISIL_DAILY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        query = "select * from CRISIL_DAILY_DATA_CORPUS where Chunking_Status is Null and S3_Upload_Status = 'Done'"
            ...
            if(link not in doc_links):                
                try:
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE CRISIL_DAILY_DATA_CORPUS SET Chunking_Comments = 'File Not Available' WHERE Relevant_Date= '{rel_Date}' and Generated_File_Name = '{new_file_name}'"
                        print(query)
                        connection.execute(query)
                    else:
                        query = f"UPDATE CRISIL_DAILY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' where Relevant_Date= '{rel_Date}' and Generated_File_Name = '{new_file_name}'"
                        print(query)
                        connection.execute(query)
                    # connection.commit()
                    time.sleep(2)
                ...
                try:
                    try:
                        ...
                        if 'Failed to load document (PDFium: Data format error)' in str(e):
                            ...
                            query = f"UPDATE CRISIL_DAILY_DATA_CORPUS SET Chunking_Comments = 'Corrupt Document' WHERE Relevant_Date= '{rel_Date}' and Generated_File_Name = '{new_file_name}'"
                            print(query)
                            connection.execute(query)   
                        else:
                            query = f"UPDATE CRISIL_DAILY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' where Relevant_Date= '{rel_Date}' and Generated_File_Name = '{new_file_name}'"
                    ...
                    if str(file_id) not in doc_ids :
                        ...
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection = engine.connect()                
                                update_query = f"update CRISIL_DAILY_DATA_CORPUS set Chunking_Comments = 'Not Relevant' where Rating_File_Link = '{link}' and Generated_File_Name = '{file_name}' and Relevant_Date = '{crisil_ratings['Relevant_Date'][i]}'"
                                ...
                                print('_' * 80)
                                continue
                        elif int(relevant_year[3:])>=18 and int(relevant_year[3:])<30:
                            company_name = company_name.replace("'","\\'") # Escaping single quotes
                        ...
                        if (len(extracted_chunks)>0):
                            for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                                ...
                                try:
                                    ...
                                    client.execute(f"INSERT INTO CRISIL_DAILY_DATA_CORPUS_CHUNKED (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")
                                    # client.execute(f"INSERT INTO CRISIL_DAILY_DATA_CORPUS_CHUNKED (Document_Id,Chunk_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.str.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{datetime.datetime.now(india_time)}');")   
                                    ...
                                    update_query = f"""
                                        ...
                                        WHERE Rating_File_Link = '{link.replace("'", "''")}' 
                                          AND Generated_File_Name = '{file_name}' 
                                          AND Relevant_Date = '{crisil_ratings['Relevant_Date'][i]}'
                                    """
                ...
                connection = engine.connect()                
                update_query = f"update CRISIL_DAILY_DATA_CORPUS set Chunking_Status = 'Done' where Rating_File_Link = '{link}' and File_ID = '{table_file_id}' and Relevant_Date = '{crisil_ratings['Relevant_Date'][i]}'"
                connection.execute(sql_text(update_query))
                # connection.commit()
                print('Table Updated as Done')
                print('_' * 80) 
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.CRISIL_DAILY_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM CRISIL_DAILY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE CRISIL_DAILY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM CRISIL_DAILY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM CRISIL_DAILY_DATA_CORPUS_CHUNKED where Published_Date IS Null ;''')
        print(len(doc))
