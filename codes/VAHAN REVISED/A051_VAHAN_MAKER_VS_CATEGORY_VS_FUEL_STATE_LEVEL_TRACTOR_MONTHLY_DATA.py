...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        def download_file(driver):
            global no_of_ping
            os.chdir(r"C:\Users\Administrator\Junk_Vahan_Tractors")
            limit = 0
            while True:
                try:
                    no_of_ping += 1
        ...
        def data_check():
            ...
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING  group by RTO_Office_Raw,Relevant_Date",engine)
            ...
            if(error_df.empty):
                ...
                for index,i in error_df.iterrows():
                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTHLY_STATUS set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                    connection.execute("commit")

                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTHLY_STATUS set State_Status = Null where State = '" +i["State"] +"'")
                    connection.execute("commit")


                    connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    download_file_path = r"C:\Users\Administrator\Junk_Vahan_Tractors"
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTHLY_STATUS", con=engine)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTHLY_STATUS", con=engine, index=False, if_exists='append')

                    #########################################
                    query = "select min(Relevant_Date) as Min FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTHLY_STATUS"
                    ...
                    for st in state_info:
                        ...
                        vahan_maker_cat_fuel_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING", con=engine, index=False, if_exists='append')
                        ...
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTHLY_STATUS set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
                ...
                try:
                    ...
                    pass
            data_check()
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTHLY_STATUS"
            connection.execute(query)
            connection.execute('commit')

            #maker vs category
            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING", con=engine)
            ...
            date_vahan = pd.read_sql("select max(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING", con=engine)
            ...
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_TRACTOR_MONTHLY_DATA where Relevant_Date>='" + min_date_vahan + "' and Relevant_Date<='" + max_date_vahan + "'"
            connection.execute(query)
            connection.execute('commit')

            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING where Relevant_Date>='" + min_date_vahan + "'", con=engine)
                df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_TRACTOR_MONTHLY_DATA', con=engine, index=False, if_exists='append')

            except:
                print('ERROR 3')
                raise Exception('error in VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING')


            query = "drop table VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING"
            connection.execute(query)
            connection.execute('commit')
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING')"
        ...
        query = "SELECT * FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTHLY_STATUS"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [21,1,8,15,22,29,9]): #25
                

                print('B')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAGING select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_TRACTOR_MONTHLY_DATA limit 0"
                connection.execute(query)
                connection.execute('commit')
