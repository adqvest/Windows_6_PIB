...
class ExtractorLoader:
    ...
    def _get_extractor(self, function_name: str) -> Optional[Callable]:
    ...
    def reload(self):
        """Reload all extractors from file"""
    ...
    def _create_config_from_json(self,site_data:Dict[str, Any]) -> SiteConfig:
        ...
        try:
            config = SiteConfig(
                ...
                #--------------------------SELECTOR INFO----------------------------------------------------------
                state_selector=site_data.get("selectors",{})['state_selector'],
                ...
                skip_city_selector=site_data.get("options",{}).get('skip_state_selector', False),
                skip_locality_selector=site_data.get("options",{}).get('skip_state_selector', False),
...
class CascadingDataCollector:
    ...
    async def _collect_cascading_data(self, context: PlaywrightCrawlingContext):
        ...
        for state_index, state in enumerate(states):
            ...
            try:
                    ...
                    for city_index, city in enumerate(cities):
                        ...
                        try:
                            ...
                            if (self.config.locality_selector==None and self.config.search_selector):
                                print('-----------------------')
                                click_success = await self._open_by_javascript(context.page, self.config.search_selector)
                            ...
                            print(f"Error processing city {city}: {self._log_strategy_error('_collect_cascading_data')}")
                            continue
                ...
                print(f"Error processing state {state}: {self._log_strategy_error('_collect_cascading_data')}")
                continue
        ...
        for locality_index, locality in enumerate(localities):
            ...
            try:
                ...
                if self.config.search_selector:
                        ...
                        click_success = await self._try_multiple_click_strategies_with_context(page, self.config.search_selector)
                        print('hi-----------------------')
    ...
    async def _get_dropdown_options(self, page, selector: str) -> List[str]:
        ...
        try:
            ...
            if not options:
                options=self._extract_options_from_page()
            return options
        ...
        try:
            ...
            for container in containers:
                ...
                if self.config.custom_data_extractor:
                    record = self.config.custom_data_extractor(container, state, city, locality,self.config.address_selector,self.config.filter_flag,self.base_record)
                else:
...
def _zudio_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
    """Custom data extractor for Zudio with specific business logic"""
    try:
...
def _yousta_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
    """Custom data extractor for Zudio with specific business logic"""
    try:
        all_address=[]
        ...
        for adr in address_elem:
            ...
            all_address.append({**base_record,
                                'Address': address,
                                'State': state,
                                'City': city,
                                'Locality': locality or '',
                                'Pincode': _get_pincode_from_address(address),
                                'Latitude':latitude,
                                'Longitude':longitude
                            })
...
def _pngadgil_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
    """Custom data extractor for Zudio with specific business logic"""
    try:
        # print(container)
        all_address=[]
...
def _popeyes_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
...
def _melorra_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
    """Custom data extractor for POPEYES with specific business logic"""
    try:
        print(container)
        all_address=[]
...
def _maxfashion_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
    """Custom data extractor for POPEYES with specific business logic"""
    try:
        print(container)
        all_address=[]
...
def _malabar_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
    """Custom data extractor for POPEYES with specific business logic"""
    try:
        # print(container)
        all_address=[]
        address_elem =container.select("div.str_adcont")
        # print(pngadgil_config.address_selector)
        # address_elem= [i.find('p') for i in address_elem]
        # print(address_elem)
...
def _lifestyle_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
    """Custom data extractor for POPEYES with specific business logic"""
    try:
        # print(container)
        all_address=[]
        address_elem =container.select("div.store-info-box")
        if not address_elem:
                return None

        for adr in address_elem:
            print("---------------------------------------------------------------------------------------------------")
            adr=adr.select("ul.list-unstyled.outlet-detail")
            if adr:
                address=adr[0].get_text(separator=' ', strip=True)
                # address = adr.text.replace('\n', '').replace('  ', '').strip()
                print(address)
                if not _validate_address(address,filter_flag,state, city, locality):
                  continue
...
def _reliance_custom_extractor(container, state, city, locality,address_id,filter_flag,base_record):
...
def _Vmart_custom_extractor(container, state, city, locality,address_id,filter_flag,base_record):
    """custom_data_extractor"""
    try:
        # print(filter_flag)
        all_address=[]
...
def _Vishal_custom_extractor(container, state, city, locality,address_id,filter_flag,base_record):
    """custom_data_extractor"""
    try:
        all_address=[]
        address_elem =container.find_all(id='storecards')
        if not address_elem:
                return None

        for adr in address_elem:
            address_com=adr.get('value')
            address_com=json.loads(address_com)
            print(address_com)
            for j in address_com:
                address=j['address']
                latitude=j['lat']
                longitude=j['lng']
        ...
        return all_address
            ...
            _log_strategy_error('_melorra_custom_extractor')
            return None

def _styleBazar_custom_extractor(container, state, city, locality, address_id,filter_flag,base_record):
    """Custom data extractor for POPEYES with specific business logic"""
    try:
        # print(container)
        all_address=[]
...
def _Kalyan_custom_extractor(container, state, city, locality,address_id,filter_flag,base_record):
    """custom_data_extractor"""
    try:
        all_address=[]
        address_elem =container.select(address_id)
            ...
            for ci in adr.select('ul.placeList li'):
                ...
                for store in ci.select('ul.storeList li'):
                    ...
                    if store_link:
                        ...
                        print(city,state,store,latitude,longitude,address)
                        if filter_flag is not None:
                           if not _validate_address(address,filter_flag,state, city, locality):
                              continue
...
async def run_program():
    ...
    malabar_config=SiteConfig(
        url="https://www.malabargoldanddiamonds.com/stores/india",
        company_name="Malabar Group",
        brand_name="Malabar",
        country="India",
        state_selector="select#state",
        city_selector="select#city",
        search_selector="input#store_submit",

        data_container_selector="div.addr_strinner",
        address_selector="address",
        custom_data_extractor=_malabar_custom_extractor , # Optional custom logic
        cascade_wait_time=3
    ...
    lifestyle_config=SiteConfig(
            ...
            country="India",
            state_selector="select#customState",
            city_selector="select#customCity",
            search_selector="button.btn",

            data_container_selector="div.col-lg-4.col-md-12:nth-child(2)",
            address_selector="address",
            custom_data_extractor=_lifestyle_custom_extractor , # Optional custom logic
            cascade_wait_time=3
            
        )
    
    relianceSmtBazar_config=SiteConfig(
            url="https://stores.reliancesmartbazaar.com/location/maharashtra/mumbai",
            company_name="Reliance Retail",
            brand_name="Reliance Smart",
            country="India",
            state_selector="select.input[required]",
            city_selector="select.input[name='city']",
            locality_selector="select.input[name='locality']",
            search_selector="li.actions button.btn",
            skip_locality_selector=True,

            data_container_selector="div.outlet-list",
            address_selector="div.store-info-box",
            custom_data_extractor=_reliance_custom_extractor , # Optional custom logic
            cascade_wait_time=3,
            page_load_timeout=100000,
            filter_flag='locality',
            
            
        )

    Vmart_config=SiteConfig(
            url="https://stores.vmartretail.com/",
            company_name="Vmart Retail",
            brand_name="Vmart",
            country="India",
            state_selector="select[id='state']",
            city_selector="select[id='city']",
            locality_selector="select[id='locality']",
            search_selector="button[type='submit']",
            skip_locality_selector=True,

            data_container_selector="div.card-deck.mt-3",
            address_selector="div.card-body.pt-3.px-3.pb-2",
            custom_data_extractor=_Vmart_custom_extractor , # Optional custom logic
            cascade_wait_time=3,
            page_load_timeout=100000,
            filter_flag='locality',
    ...
    VishalMegaMart_config=SiteConfig(
            url="https://stores.vishalmegamart.com/map",
            company_name="Vishal Megamart",
            brand_name="Vishal Megamart",
            country="India",
            state_selector="select[id='state']",
            city_selector="select[id='city']",
            locality_selector="select[id='locality']",
            search_selector="button.btn.mb-3.idx-str-srch-btn.search-button",
            skip_locality_selector=True,

            data_container_selector="div.container-fluid.p-0",
            address_selector="div.container-fluid.p-0",
            custom_data_extractor=_Vishal_custom_extractor , # Optional custom logic
            cascade_wait_time=3,
            page_load_timeout=100000,
            filter_flag='locality',
    ...
    StyleBazar_config=SiteConfig(
            url="https://stylebaazar.in/store-locator/",
            company_name="Baazar Style Retail Limited",
            brand_name="Style Bazaar",
            country="India",
            state_selector="li.item-state[data-id] a[data-parent='#p-statelist'] span",
            # city_selector="li.item-state[data-id] a[data-parent='#item-city-8'] span",
            skip_state_selector=True,
            skip_city_selector=True,

            data_container_selector="ul.sl-list[id='p-statelist']",
            address_selector="li.sl-addr span",
            custom_data_extractor=_styleBazar_custom_extractor , # Optional custom logic
            cascade_wait_time=3,
            page_load_timeout=100000,
            search_limit=1  #Because All DATA WE CAN GET IN A PAGE NO NEED TO GO IN A LOOP
            
        )


    Kalyan_config=SiteConfig(
            url="https://www.kalyanjewellers.net/store-locator.php",
            company_name="Kalyan Jewellers India Ltd",
            brand_name="Kalyan Jewellers",
            country="India",
            state_selector="ul.do-locator-menu[role='tablist'] li[role='tab'] ul[id='collapese1'] a[href]",

            skip_state_selector=True,
            skip_city_selector=True,

            data_container_selector="ul[id='collapese1'].areaList.collapse.show",
            address_selector="li",
            custom_data_extractor=_Kalyan_custom_extractor , # Optional custom logic
            cascade_wait_time=3,
            page_load_timeout=100000,
            search_limit=1 
            # filter_flag=['city','state']
    ...
    styleup_config=SiteConfig(
            url="https://styleup.in/store-locator",
            company_name="Aditya Birla Fashion and Retail Ltd",
            brand_name="Style Up",
            country="India",
            state_selector="select.jet-select__control.depth-0",
            city_selector="select.jet-select__control.depth-1",
            # locality_selector="select.jet-select__control.depth-2",
            search_selector="button.apply-filters__button",

            data_container_selector="div.elementor-loop-container.elementor-grid",
            address_selector="div.elementor-widget-container",
            # custom_data_extractor=_cityKart_custom_extractor , # Optional custom logic
            cascade_wait_time=3,
            page_load_timeout=100000
    ...
    citycart_config=SiteConfig(
            url="https://citykart.co.in/store/",
            company_name="Citykart Retail Private Limited",
            brand_name="City Kart",
            country="India",
            state_selector="select.jet-select__control.depth-0",
            city_selector="select.jet-select__control.depth-1",
            # locality_selector="select.jet-select__control.depth-2",
    ...
    # Firstcry_config=SiteConfig(
    #         url="https://stores.vishalmegamart.com/map",
    #         company_name="Brainbees Solutions Limited",
    #         brand_name="Firstcry",
    #         country="India",
    #         state_selector="select[id='storetype']",
    #         city_selector="select[id='city']",
    #         locality_selector="select[id='locality']",
    #         search_selector="button.btn.mb-3.idx-str-srch-btn.search-button",
    #         skip_locality_selector=True,
    ...
    # print(all_configs)
    Kalyan_config_2=all_configs["kalyan_jewellers"]
    ...
    #----------------------------------------------------------------------------------------------------------------------------------
    all_configs = [yousta_config, pngadgil_config, popeyes_config,malabar_config,lifestyle_config,relianceSmtBazar_config,Vmart_config,VishalMegaMart_config] 
    all_configs = [Kalyan_config_2] 
