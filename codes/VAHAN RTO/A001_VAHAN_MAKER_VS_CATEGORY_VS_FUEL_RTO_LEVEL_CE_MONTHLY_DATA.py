...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA'
    ...
    try:
        ...
        def get_dates(cond):
            ...
            distinct_date = [datetime.date(2023,7,31)]

            query = "select min(Relevant_Date) as Min FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS"
            ...
            dates_df=pd.DataFrame({"Relevant_Date":[datetime.date(2024,9,30)]})# comment out after the manual run
            ...
            if(cond):
                ...
                dates_df = dates_df[:1] #change this later to 4
        ...
        def data_check():
            ...
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING  group by RTO_Office_Raw,Relevant_Date",engine)
            ...
            if(error_df.empty):
                ...
                for index,i in error_df.iterrows():
                    connection = engine.connect()
                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                    connection.execute("commit")

                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS set State_Status = Null where State = '" +i["State"] +"'")
                    connection.execute("commit")


                    connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS", con=engine)
                    if(maker_df.empty):
                        engine = adqvest_db.db_conn()
                        maker_done.to_sql("VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS", con=engine, index=False, if_exists='append')
                    ...
                    for st in state_info:
                        ...
                        for x in rto_info.keys():
                            ...
                            engine = adqvest_db.db_conn()    
                            vahan_maker_cat_fuel_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING", con=engine, index=False, if_exists='append')


                            connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS set RTO_Status = 'Done' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            time.sleep(2)
                            connection.execute("commit")

                            connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS set Runtime = '"+datetime.datetime.now(india_time).strftime("%Y-%m-%d %H:%M:%S")+"' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            time.sleep(2)
                            connection.execute("commit")
                        ...
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS set State_Status = 'Done' where State = '"+state+"'")
                        time.sleep(2)
                        connection.execute("commit")
                ...
                try:
                    ...
                    pass
            data_check()
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS"
            connection.execute(query)
            connection.execute('commit')

            #maker vs category
            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING", con=engine)
            ...
            # query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA where Relevant_Date>='" + date_vahan + "'"  #Uncomment these 3 lines
            # connection.execute(query)
            # connection.execute('commit')
            ...
            try:
                engine = adqvest_db.db_conn()
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING where Relevant_Date>='" + date_vahan + "'", con=engine)
                ...
                df['Fuel_Clean']=df['Fuel_Clean'].apply(lambda x:x.replace("PURE EV","ELECRIC").replace("ELECTRIC(BOV)","ELECTRIC"))
                df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA', con=engine, index=False, if_exists='append')
                #ClickHouse

            except:
                print('ERROR 3')
                raise Exception('error in VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING')


            query = "drop table VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING"
            connection.execute(query)
            connection.execute('commit')
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING')"
        ...
        query = "SELECT * FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA_STATUS"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            max_date=pd.read_sql('select max(Relevant_Date) as max from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA',engine)['max'][0]
            if (today.day in [2,23] or (today.date()-max_date).days>=31):
                print('B')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_CE_MONTHLY_DATA_STAGING select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_CE_MONTHLY_DATA limit 0"
                connection.execute(query)
                connection.execute('commit')
