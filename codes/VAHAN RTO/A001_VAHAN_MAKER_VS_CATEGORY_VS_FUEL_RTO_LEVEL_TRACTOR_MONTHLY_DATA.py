...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_DATA'
    ...
    try:
        ...
        def get_dates(cond):
            ...
            query = "select min(Relevant_Date) as Min FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS"
            ...
            dates_df=pd.DataFrame({"Relevant_Date":[datetime.date(2024,10,31)]})# comment out after the manual run
        ...
        def data_check_step1():
            rto_df = pd.read_sql("Select  State, Sum(Total) as RTO_Total, Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING group by State, Relevant_Date order by State",engine)

            state_df = pd.read_sql("Select  State, Sum(Total) as State_Total, Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_TRACTOR_MONTHLY_DATA  group by State, Relevant_Date order by State",engine)
        ...
        def data_check():
            ...
            data_df = pd.read_sql("Select  Sum(Total) as RTO ,RTO_Office_Raw,State,RTO_Code,Relevant_Date from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING  group by RTO_Office_Raw,Relevant_Date",engine)
            
            data_df.to_csv('vahan_error_data.csv')
            ...
            if(error_df.empty):
                ...
                error_df.to_csv('vahan_error.csv')

                for index,i in error_df.iterrows():
                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS set RTO_Status = Null,State_Status = Null where RTO_Office_Raw = '" +i["RTO_Office_Raw"]+"'")
                    connection.execute("commit")

                    connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS set State_Status = Null where State = '" +i["State"] +"'")
                    connection.execute("commit")


                    connection.execute("delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING where RTO_Code = '" +i["RTO_Code"]+"' and Relevant_Date >= '"+error_df["Relevant_Date"].min().strftime("%Y-%m-%d")+"' and Relevant_Date <= '"+error_df["Relevant_Date"].max().strftime("%Y-%m-%d")+"'")
                    connection.execute("commit")
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    maker_df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS", con=engine)
                    if(maker_df.empty):
                        maker_done.to_sql("VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS", con=engine, index=False, if_exists='append')
                    ...
                    for st in state_info:
                        ...
                        for x in rto_info.keys():
                            ...
                            print('passing if')
                            print(vahan_maker_cat_fuel_rto_level)
                            print("Data Frame shape: ",vahan_maker_cat_fuel_rto_level.shape)
                            ...
                            vahan_maker_cat_fuel_rto_level.to_sql(name = "VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING", con=engine, index=False, if_exists='append')

                            print("reached here")
                            connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS set RTO_Status = 'Done' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")

                            print("reached here 2")

                            connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS set Runtime = '"+datetime.datetime.now(india_time).strftime("%Y-%m-%d %H:%M:%S")+"' where State = '"+state+"' and RTO_Office_Raw = '" +rto+"'")
                            connection.execute("commit")
                        ...
                        connection.execute("update VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS set State_Status = 'Done' where State = '"+state+"'")
                        connection.execute("commit")
            ...
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS"
            connection.execute(query)
            connection.execute('commit')

            #maker vs category
            date_vahan = pd.read_sql("select min(Relevant_Date) as Date from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING", con=engine)
            ...
            # query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_DATA where Relevant_Date>='" + date_vahan + "'"  #Uncomment these 3 lines
            # connection.execute(query)
            # connection.execute('commit')


            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING where Relevant_Date>='" + date_vahan + "'", con=engine)
                ...
                df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_DATA', con=engine, index=False, if_exists='append')
                #ClickHouse

            except:
                print('ERROR 3')
                raise Exception('error in VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING')


            query = "drop table VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING"
            connection.execute(query)
            connection.execute('commit')
            ...
            # vahan_table_list = ["VAHAN_MAKER_WISE_RTO_LEVEL_MONTHLY_PRORATA_DATA"]
        ...
        def table_check():
            query = "SELECT * FROM information_schema.tables WHERE table_schema = 'AdqvestDB' AND TABLE_NAME in ('VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING')"
        ...
        query = "SELECT * FROM VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_STATUS"
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            max_date=pd.read_sql('select max(Relevant_Date) as max from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_DATA',engine)['max'][0]
            if (today.day in [2,22] or (today.date()-max_date).days>=31):
                print('B')

                query = "create table AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LVL_TRACTOR_MONTHLY_STAGING select * from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_TRACTOR_MONTHLY_DATA limit 0"
                connection.execute(query)
                connection.execute('commit')
