...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
                ...
                try:
                    ...
                    for x in state_option:
                        ...
                        if(val == '-1'):
                            ...
                            if val in ['LD', 'MZ', 'NL', 'OR', 'PB','PY', 'SK', 'TN']:
                                state_info[val] = x.text

                    # filtered_data = {key: state_info[key] for key in ['Sikkim','Mizoram','Nagaland','Punjab','Tamil Nadu','Puducherry','Lakshadweep','Odisha'] if key in state_info}        
                    # print()
                    # state_info=filtered_data.copy()
                    time.sleep(5)
                    break
                except:
        ...
        def get_dates(cond):
            ...
            distinct_date = [datetime.date(2021,8,31)]
            ...
            if(cond):
                dates_df = dates_df[:1] #change it back to 4
            ...
            '''uncomment these 2 lines after manual data collection '''
        ...
        def main_function():
            ...
            while True:
                try:
                    ...
                    #delete the line after the manual data collection
                    # maker_done=maker_done[maker_done['State'].isin(['Sikkim','Mizoram','Nagaland','Punjab','Tamil Nadu','Puducherry','Lakshadweep','Odisha'])]
            ...
            #uncomment theses lines after manual data collection
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            max_date=pd.read_sql('select max(Relevant_Date) as max from AdqvestDB.VAHAN_MAKER_VS_CATEGORY_VS_FUEL_RTO_LEVEL_MONTHLY_DATA',engine)['max'][0]
            
            if (today.day in [2,26] or (today.date()-max_date).days>=31):   #change it to 2 after this data collection
                print('B')
