...
class PibProcessor:
    ...
    def process_single_file(self, row_data, ch_client, timestamp):
...
def process_single_pdf(obj, custom_processor=None,table_config=None):
    ...
    try:
        ...
        try:
            ...
            # print(f"🔍 FILE SIZE: {len(files_content.get(file_name, b''))} bytes")
        ...
        if str(file_id) not in doc_ids:
            ...
            if len(extracted_chunks) > 0:
                if chunk_table == "SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED_3":
                    Report_Category=pd.read_sql(f"select Report_Category from AdqvestDB.SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS where File_Name='{file_name}' order by Relevant_Date DESC",engine)['Report_Category'][0]
                    title = pd.read_sql(f"select Title from AdqvestDB.SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS where File_Name='{file_name}' order by Relevant_Date DESC",engine)['Title'][0]
                    ...
                    for chunk, page_number in zip(extracted_chunks, range(len(extracted_chunks))):
                        ...
                        try:
                            ...
                            ch_client.execute(f"INSERT INTO {chunk_table} (Document_Id,Document_Company,Document_Type,Document_Category,Title,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{Report_Category}','{title}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")
...
def process_single_table(table_config):
    ...
    table_name = table_config["table"]
    processor_name = table_config.get("custom_processor", None)
    
    # Create processor inside the process (for ProcessPoolExecutor compatibility)
    custom_processor = None
    if processor_name == "CareRatingsProcessor":
        custom_processor = CareRatingsProcessor()
    elif processor_name == "CrisilProcessor":
        custom_processor = CrisilProcessor()
    elif processor_name == "PibProcessor":
        custom_processor = PibProcessor()
    
    try:
        print(f"Starting chunking for table: {table_name}")
        if custom_processor:
            print(f"Starting: {table_name} ({processor_name})")
        else:
...
def run_parallel_for_response(response_list, custom_processor=None, table_config=None, max_workers=MAX_WORKERS):
    ...
    for idx, k in enumerate(response_list):
        try:
            ...
            results.append((file_name, logs))
            if (idx + 1) % 10 == 0:
                print(f"  Progress: {idx+1}/{total} files done")
        except Exception as e:
            results.append((k, [f"ERROR: {str(e)}"]))            
...
def get_table_configs():
    ...
    return [
        ...
        {
            "table": "CARE_RATINGS_DAILY_DATA_CORPUS",
            "s3_folder": "CARE_RATINGS_CORPUS_TO_BE_CHUNKED/",
            "chunk_table": "CARE_DAILY_DATA_CORPUS_CHUNKED",
            "custom_processor": "CareRatingsProcessor",
            "s3_filters": None
        },
        {
            "table": "CRISIL_DAILY_DATA_CORPUS",
            "s3_folder": "CRISIL_RATINGS_CORPUS_TO_BE_CHUNKED/",
            "chunk_table": "CRISIL_DAILY_DATA_CORPUS_CHUNKED",
            "custom_processor": "CrisilProcessor",
            "s3_filters": None
        },
        
        {
            "table": "PIB_REPORTS_DAILY_DATA_CORPUS",
            "s3_folder": "PIB_CORPUS_TO_BE_CHUNKED/",
            "chunk_table": "PIB_REPORTS_DAILY_DATA_CORPUS_CHUNKED",
            "custom_processor": "PibProcessor",
            "s3_filters": None
        },
        ...
        {
            "table": 'BANK_BASEL_III_QUARTERLY_DATA_CORPUS',
        ...
        
        {
            ...
            "chunk_table": "SEBI_LEGAL_CIRCULARS_RANDOM_DATA_CORPUS_CHUNKED_3",
            "custom_processor": None,
            "s3_filters": None
        },
...
def run_program(run_by='Adqvest_Bot', py_file_name=None, scheduler_name=None):
    ...
    try:
        if (py_file_name is None):
            ...
            py_file_name = os.path.basename(sys.argv[0]).replace('.py', '')
        ...
        table_results = []
        table_max_workers = 4  # 3 tables in parallel
        print(f"Processing {len(tables_to_run)} tables with {table_max_workers} parallel processes")
        
        # Use ProcessPoolExecutor - separate processes have separate memory
        # pypdfium2 in different processes won't conflict
        with ProcessPoolExecutor(max_workers=table_max_workers) as exe:
            future_to_table = {exe.submit(process_single_table, table_config): table_config["table"] 
                               for table_config in tables_to_run}
            ...
            for future in as_completed(future_to_table):
                ...
                try:
                    table_name, status, result = future.result()
                    table_results.append((table_name, status, result))
                    print(f"[{completed_count}/{len(tables_to_run)}] Table {table_name} completed: {status}")
                except Exception as e:
                    print(f"[{completed_count}/{len(tables_to_run)}] Table {table_name} failed: {str(e)}")
                    table_results.append((table_name, "ERROR", str(e)))
...
if __name__ == "__main__":
    ...
    # Check if arguments passed from scheduler
    if len(sys.argv) >= 4:
        py_file_name = sys.argv[1]
        scheduler_name = sys.argv[2]
        run_by = sys.argv[3]
        run_program(run_by=run_by, py_file_name=py_file_name, scheduler_name=scheduler_name)
    else:
