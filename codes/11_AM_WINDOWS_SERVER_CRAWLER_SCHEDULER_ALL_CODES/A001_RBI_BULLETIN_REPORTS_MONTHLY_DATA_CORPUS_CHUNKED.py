...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS,RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        folder_name = 'RBI_BULLETIN_REPORT_CORPUS_TO_BE_CHUNKED'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'RBI_BULLETIN_REPORT_CORPUS_TO_BE_CHUNKED/' in x]
        # response = response[:20]
        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED;''')
        ...
        for obj in response:
            ...
            try:
                ...
                try:
                    ...
                    connection.execute("update RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '" +str(file_name).replace('.pdf', '')+"'")
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection=engine.connect()
                                connection.execute("update RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name)+"'")
                                connection.execute("commit")
                                continue      
                        ...
                        connection=engine.connect()
                        connection.execute("update RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '" +str(file_name)+"'")
                        connection.execute("commit")
                        continue
                    ...
                    if (len(extracted_chunks)>0):
                        ...
                        for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                            ...
                            try:
                                ...
                                connection=engine.connect()
                                connection.execute("update RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS set Chunking_Status='Done' where File_Name = '" +str(file_name)+"'")
                                connection.execute("commit")
                            except:
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM RBI_BULLETIN_REPORTS_MONTHLY_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
