...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        doc_ids = client.execute('''SELECT distinct document_id FROM RBI_PRESS_RELEASES_DAILY_CORPUS_CHUNKED;''')
            ...
            try:
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if relevant_year is not None:
                        ...
                        connection=engine.connect()
                        connection.execute("update RBI_PRESS_RELEASES_DAILY_DATA_CORPUS set Chunking_Status='Issue' where File_Name = '" +str(file_name)+"'")
                        connection.execute("commit")
                        continue
        ...
        doc = client.execute('''SELECT distinct document_id as id , page_number as pn FROM RBI_PRESS_RELEASES_DAILY_CORPUS_CHUNKED where document_link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE RBI_PRESS_RELEASES_DAILY_CORPUS_CHUNKED
                    UPDATE 
