...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        def get_dates(cond):
            ...
            if(cond):
                dates_df = dates_df[:12]

            else:
                dates_df = dates_df[:12] #change this later
        ...
        def main_function():
            ...
            query = "delete from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_TRAC_MONTHLY_DATA_MP where Relevant_Date>='" + date_vahan + "'"
            connection.execute(query)
            connection.execute('commit')

            try:
                df = pd.read_sql("select * from VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LVL_TRACTOR_MONTH_STAG_MP where Relevant_Date>='" + date_vahan + "'", con=engine)
                df.to_sql(name='VAHAN_MAKER_VS_CATEGORY_VS_FUEL_STATE_LEVEL_TRAC_MONTHLY_DATA_MP', con=engine, index=False, if_exists='append')
        ...
        if(( vahan_maker_state_done.shape[0]>0 ) | (table_check())):
            ...
            if(today.day in [14]):   #1,4,11,12,16,22,27
