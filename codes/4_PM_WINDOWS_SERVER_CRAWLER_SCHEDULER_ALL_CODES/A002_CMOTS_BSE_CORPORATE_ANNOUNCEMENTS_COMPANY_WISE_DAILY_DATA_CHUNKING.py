...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    try:
        ...
        folder_name = 'CMOTS_BSE_CORPUS_TO_BE_CHUNKED'

        # not_tried_files=pd.read_sql("select File_Name  from  AdqvestDB.CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS where  S3_Upload_Status = 'Done' and  Chunking_Status is null and Chunking_Comments is  null and Relevant_Date>='2025-11-25';",con=engine)
        # print(len(not_tried_files))
        ...
        response = [ x for x in response if 'CMOTS_BSE_CORPUS_TO_BE_CHUNKED/' in x]
        print(len(response))
        ...
        # print(response)
        # response = response[:2]
        # response = [x for x in response if re.search(r'Q2_FY26', x)]
        # response = [x for x in response if x.split('/')[-1] in not_tried_files['File_Name'].to_list()]
        print("-"*20)
        print(len(response))
        # response = response[:2]
        # response = [x for x in response if re.search(r'Hdfc_Bank_Limited_0_Investor_Presentation_Q2_FY26_AZ402', x)]

        doc_ids = client.execute('''SELECT distinct Document_Id FROM CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS_CHUNKED;''')
            ...
            try:
                try:
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = 'File Not Available' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    elif 'Failed to load document (PDFium: Incorrect password error)' in str(e):
                        query = f"UPDATE CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = 'Password Issue' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)  
                    elif 'Failed to load page' in str(e):
                        query = f"UPDATE CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = 'Corrupt Document' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)       
                    else:
                        query = f"UPDATE CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    # connection.commit()
                    time.sleep(2)
                ...
                if is_whole_doc_scanned:
                    query = f"UPDATE CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS SET Chunking_Comments = 'Scanned Document' WHERE File_Name = '{file_name}'"
                    print(query)
                    connection.execute(query)
                    continue
                ...
                if str(file_id) not in doc_ids :
                    ...
                    if int(relevant_year[5:])>=18 and int(relevant_year[5:])<30:
                        ...
                        if (len(extracted_chunks)>0):
                            ...
                            for chunk,page_number in zip(extracted_chunks,range(len(extracted_chunks))):
                                ...
                                try:
                                    ...
                                    client.execute(f"INSERT INTO CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS_CHUNKED (Document_Id,Document_Company,Document_Type,Document_Year,Document_Date,Page_Number,Document_Content,Embedding,Runtime_Chunking) VALUES ('{file_id}','{company_name.title()}','{report_type}','{relevant_year}','{document_date}',{page},'''{chunk}''',{chunk_embed},'{timestamp}');")
                                except:
                            ...
                            # connection = engine.connect()
                            # query='Update CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS set Chunking_Status="Done" where Company_Name="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                            query=f"Update CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS set Chunking_Status='Done' where Company_Name_Clean='{company_name}' and File_ID='{str(file_id)}' and Relevant_Quarter='{relevant_year}';"
                            print(query)
                        ...
                        query='Update CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS set Chunking_Status="Not Relevant" where Company_Name_Clean="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                        connection = engine.connect()
                        # connection.execute(text(query))
                        connection.execute(query)
                        # connection.commit()
                    ...
                    delete_file_from_s3(obj)
                    connection = engine.connect()
                    query='Update CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS set Chunking_Status="Done" where Company_Name_Clean="'+company_name+'" and File_ID="'+str(file_id)+'" and Relevant_Quarter="'+relevant_year+'"'
                    # connection.execute(text(query))
                    connection.execute(query)
                    # connection.commit()
                    time.sleep(2)
                ...
                print(error_msg)
                # query=f"Update CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '{file_name}'"
                # print(query)
                # try:
                #     connection.execute(query)
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc)>1:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
        ...
        dcomp = client.execute('''SELECT distinct Document_Company as dcomp FROM CMOTS_BSE_CORPORATE_ANNOUNCEMENTS_COMPANY_WISE_DAILY_DATA_CORPUS_CHUNKED where Symbol = '';''')
        print(len(dcomp))
