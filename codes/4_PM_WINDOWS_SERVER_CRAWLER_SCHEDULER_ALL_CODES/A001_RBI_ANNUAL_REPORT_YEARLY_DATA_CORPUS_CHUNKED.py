# -*- coding: utf-8 -*-
"""
Created on Tue Jul 29 16:27:28 2025
...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS, RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        folder_name = 'RBI_ANNUAL_REPORT_CORPUS_TO_BE_CHUNKED'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'RBI_ANNUAL_REPORT_CORPUS_TO_BE_CHUNKED/' in x]
        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS_CHUNKED;''')
            ...
            try:
                ...
                try:
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE Generated_File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    else:
                        query = f"UPDATE RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE Generateed_File_Name = '{file_name}'"
                        print(query)
                        connection.execute(query)
                    # connection.commit()
                    time.sleep(2)
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection=engine.connect()
                                connection.execute("update RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS set Chunking_Status='Not Relevant' where Generated_File_Name = '" +str(file_name)+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS set Chunking_Comments='ISSUE' where Generated_File_Name = '" +str(file_name)+"'")
                        connection.execute("commit")
                        continue
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc) > 0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM RBI_ANNUAL_REPORT_YEARLY_DATA_CORPUS_CHUNKED where Published_Date is Null ;''')
        print(len(doc))
