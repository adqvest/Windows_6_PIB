...
def run_program(run_by = 'Adqvest_Bot', py_file_name = None):
   ...
   try :
      ...
      time.sleep(2)
      page.locator('//*[contains(text(), "Monthly Monetary Bulletin")]').click()
      html = page.content()
      ...
      dates = []
      for i in table:
         link = "http://www.qcb.gov.qa" + i.find('a').get('href')
         links_list.append(link)
         text = i.find('div').text.strip().split('-',1)[-1].lower().strip()
         date = parser.parse(text).date() + relativedelta(months=1, days = -1, day = 1)
         dates.append(date)

      for link, date in zip(links_list, dates):
         if date > last_date:
            url = link
            present_url = [url.replace(' ','%20')]
            headers = {
            ...
            # * Modified by Joe *
            try:
               r = requests.get(present_url[0], verify=False,headers=headers, timeout = 60)
               #         no_of_ping +=1
               present_date = pd.read_excel(r.content,sheet_name = 'Title',header = None)
               present_date.dropna(axis=0,how='all',inplace=True)
               present_date.reset_index(drop=True,inplace=True)
               try:
                     format_date = present_date.iloc[3,3].date()
               except:
                     format_date = present_date.iloc[3, -1].date()
               #   format_date = present_date.iloc[3,0].date()
               latest_date = last_day_of_month(format_date)
               file = "QATAR_CENTRAL_BANK_"+str(latest_date)+".xls"
               print('passes')
               ...
               r = requests.get(present_url[0], verify=False,headers=headers, timeout = 60)
               #         no_of_ping +=1
               latest_date = pd.to_datetime('2023-11-30').date()
               file = "QATAR_CENTRAL_BANK_2023-11-30.xls"
               print('passes')
               with open(file, 'wb') as f:
                     f.write(r.content)
                     f.close()
            ...
            if latest_date > last_date:
               ...
               s3.Bucket(BUCKET_NAME).put_object(Key='QATAR_CENTRAL_BANK/' + file, Body=data)
               ...
               table_1['Relevant_Date'] = latest_date
               table_1['Runtime'] = pd.to_datetime(today.strftime("%Y-%m-%d %H:%M:%S"))
               table_1['Unit'] = 'Millions of QR'
               table_1['Value'] = table_1['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_1 = table_1[['Table_No','Table_Name','Variable_Name','Value','Unit','Relevant_Date','Runtime']]
               table_1.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA",con = engine,index = False,if_exists = 'append')
               ...
               table_2['Value'] = table_2['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_2.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)

               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Currency Issued'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Currency Issued' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_2.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_2.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_3['Value'] = table_3['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_3.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)

               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Monetary Survey of the Banking System'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Monetary Survey of the Banking System' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_3.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_3.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_4['Value'] = table_4['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_4.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)

               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Commercial Banks Claims on Govt. & Private Sector'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Commercial Banks Claims on Govt. & Private Sector' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_4.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_4.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_5['Value'] = table_5['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_5.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Money Supply'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Money Supply' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_5.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_5.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_6['Value'] = table_6['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_6.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Changes in Money Supply'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Changes in Money Supply' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_6.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_6.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_7['Value'] = table_7['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_7.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Changes in Factors Affecting Money Supply'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Changes in Factors Affecting Money Supply' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_7.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_7.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_8['Value'] = table_8['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_8.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Financial Statement of QCB Assets'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Financial Statement of QCB Assets' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_8.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_8.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_9['Value'] = table_9['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_9.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Financial Statement of QCB Liabilities'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Financial Statement of QCB Liabilities' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_9.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_9.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_10['Value'] = table_10['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_10.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Reserve Money M0 and The Counterpart Assets'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Reserve Money M0 and The Counterpart Assets' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_10.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_10.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_11['Value'] = table_11['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_11.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'QCBs International Reserves and Foreign Currency Liquidity'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'QCBs International Reserves and Foreign Currency Liquidity' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_11.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_11.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_12['Value'] = table_12['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_12.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Commercial Banks Assets'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Commercial Banks Assets' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_12.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_12.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_13['Value'] = table_13['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_13.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Commercial Banks Liabilities'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Commercial Banks Liabilities' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_13.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_13.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_14['Value'] = table_14['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_14.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Commercial Banks Deposits'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Commercial Banks Deposits' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_14.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_14.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_15['Value'] = table_15['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_15.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Commercial Banks Deposits-Public Sector'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Commercial Banks Deposits-Public Sector' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_15.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_15.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_16['Value'] = table_16['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_16.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Commercial Banks Deposits-Private sector'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Commercial Banks Deposits-Private sector' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_16.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_16.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_17['Value'] = table_17['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_17.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Distribution of Credit Facilities'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Distribution of Credit Facilities' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_17.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_17.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_18['Value'] = table_18['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_18.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Commercial Banks Credit Facilities-Public sector'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Commercial Banks Credit Facilities-Public sector' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_18.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_18.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_19['Value'] = table_19['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_19.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Commercial Banks Credit Facilities-Private Sector'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Commercial Banks Credit Facilities-Private Sector' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_19.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_19.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_20['Value'] = table_20['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_20.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Banks Clearings'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Banks Clearings' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_20.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_20.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_21['Value'] = table_21['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_21 = table_21[['Table_No','Table_Name','Variable_Name','Value','Unit','Relevant_Date','Runtime']]
               table_21.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA",con = engine,index = False,if_exists = 'append')
               ...
               table_22['Value'] = table_22['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_22 = table_22[['Table_No','Table_Name','Variable_Name','Value','Unit','Relevant_Date','Runtime']]
               table_22.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA",con = engine,index = False,if_exists = 'append')
               ...
               table_23['Value'] = table_23['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_23 = table_23[['Table_No','Table_Name','Variable_Name','Value','Unit','Relevant_Date','Runtime']]
               table_23.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA",con = engine,index = False,if_exists = 'append')
               ...
               table_24['Value'] = table_24['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_24 = table_24[['Table_No','Table_Name','Variable_Name','Value','Unit','Relevant_Date','Runtime']]
               table_24.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA",con = engine,index = False,if_exists = 'append')
               ...
               table_25['Value'] = table_25['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_25.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Interbanks Interest Rates Weighted Average'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Interbanks Interest Rates Weighted Average' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_25.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_25.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_26['Value'] = table_26['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_26.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Interest Rates on Customers Deposits in QAR Weighted Average'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Interest Rates on Customers Deposits in QAR Weighted Average' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_26.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_26.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_27['Value'] = table_27['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_27.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Interest Rates on Credit Facilities in QAR Weighted Average'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Interest Rates on Credit Facilities in QAR Weighted Average' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_27.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_27.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_28['Value'] = table_28['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_28.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'QMR & Repo Operations'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'QMR & Repo Operations' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_28.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_28.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_29['Unit'] = 'QR'
               table_29['Value'] = table_29['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_29.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Exchange Rate of GCC and Major Currencies against QR'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Exchange Rate of GCC and Major Currencies against QR' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_29.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_29.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               ...
               table_30['Value'] = table_30['Value'].apply(lambda x: int(str(x).replace(',', '')))
               table_30.to_sql(name = "QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING",con = engine,if_exists = 'append',index = False)
               try:
                  query = "Select min(Relevant_Date) as RD from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING where Table_Name = 'Qatar Stock Exchange'"
                  RD = pd.read_sql(query,con=engine)['RD'].iloc[0]
                  query1 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA where Table_Name = 'Qatar Stock Exchange' and Relevant_Date >= '"+str(RD)+"';"
                  engine.execute(query1)
                  table_30.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               except:
                  table_30.to_sql(name='QATAR_CENTRAL_BANK_MONTHLY_DATA',con=engine,if_exists='append',index=False)
               query2 = "Delete from AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA_STAGING"
               engine.execute(query2)
               try:
                     try:
                        query="ALTER TABLE AdqvestDB.QATAR_CENTRAL_BANK_MONTHLY_DATA DELETE WHERE Relevant_Date>='" + str(RD) + "'"
                        client.execute(query)
                     except:
                        print('ClickHouse delete')
                        pass
                     try:
                        sql_df = pd.read_sql("select * from QATAR_CENTRAL_BANK_MONTHLY_DATA where Relevant_Date>='" + str(RD)+ "'", con=engine)
                        client.execute("INSERT INTO QATAR_CENTRAL_BANK_MONTHLY_DATA VALUES", sql_df.values.tolist())
                     except:
                        pass
               except:
                     print('ERROR')
                     raise Exception('error in QATAR_CENTRAL_BANK_MONTHLY_DATA')
         else:
