...
def run_program(run_by='Adqvest_Bot', py_file_name=None):
    ...
    table_name = 'SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS,SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS_CHUNKED'
    ...
    try:
        ...
        folder_name = 'SEBI_INVIT_CORPUS_TO_BE_CHUNKED'
        
        response = read_all_file_names_from_s3_folder(bucket_name, folder_name)
        response = [ x for x in response if 'SEBI_INVIT_CORPUS_TO_BE_CHUNKED/' in x]
        response = [ x for x in response if 'Final_Offer_Document' in x]
        # response = response[:50]
        print(len(response))
        
        doc_ids = client.execute('''SELECT distinct Document_Id FROM SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS_CHUNKED;''')
            ...
            try:
                try:
                    ...
                    if 'Failed to load document (PDFium: Data format error)' in str(e):
                        query = f"UPDATE SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS SET Chunking_Comments = '{str(e)}' WHERE File_Name = '{str(file_name)}'"
                        print(query)
                    ...
                    if relevant_year is not None:
                        if 'Q' in relevant_year: 
                            if int(relevant_year[5:]) >= 18 and int(relevant_year[5:]) < 30:
                                ...
                                connection.execute("update SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS set Chunking_Status='Not Relevant' where File_Name = '" +str(file_name)+"'")
                        ...
                        connection=engine.connect()
                        connection.execute("update SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS set Chunking_Comments='ISSUE' where File_Name = '" +str(file_name)+"'")
                        connection.execute("commit")
                        continue
        ...
        connection = engine.connect()
        query = "select * from AdqvestDB.SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS ORDER BY File_ID ASC;"
        data = pd.read_sql(query, con=engine)

        doc = client.execute('''SELECT distinct Document_Id as id , Page_Number as pn FROM SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS_CHUNKED where Document_Link = '' ;''')
        print(len(doc))
        ...
        if len(doc)>0:
            ...
            for batch_num in range(total_batches):
                ...
                query = f"""
                    ALTER TABLE SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS_CHUNKED
                    UPDATE 
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS_CHUNKED where Runtime_Scraped is Null ;''')
        print(len(doc))
        ...
        doc = client.execute('''SELECT distinct Document_Id as id FROM SEBI_INVIT_ISSUES_DAILY_DATA_CORPUS_CHUNKED where Published_Date is Null;''')
        print(len(doc))
